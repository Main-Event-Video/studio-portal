...
export async function POST(request) {
  ...
  if (action === 'done') {
    ...
    try {
      const { buffer, count } = await buildCharacterSheet(db, client.id);
      if (buffer && count > 0) {
        await sendCharacterSheetReady({ client, buffer, count }).catch((e) => console.error('sheet email failed:', e?.message));
      }
    } catch (e) {
