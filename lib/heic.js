...
// COLOUR NORMALISE (Josh 9/15 — "pink suit plays salmon"). iPhone JPEGs carry a
// DISPLAY P3 profile. Creatomate ignores embedded profiles and reads the pixel
// numbers as sRGB, which drains ~15% of the saturation out of every vivid
// colour before the video is even encoded (measured on render 098HR Seg A).
// sharp honours the profile: a plain re-encode converts the pixels to true
// sRGB and drops the profile, so Creatomate then sees the right numbers.
// Only touches files that HAVE a non-sRGB profile; everything else is returned
// untouched (changed:false) so ordinary sRGB uploads pay nothing.
export async function toSrgb(inputBuffer, { quality = 92 } = {}) {
  const type = sniffImageType(inputBuffer);
  if (type !== 'jpeg' && type !== 'png' && type !== 'webp') return { buffer: inputBuffer, changed: false, profile: null };
  const meta = await sharp(inputBuffer).metadata();
  if (!meta.icc || !meta.icc.length) return { buffer: inputBuffer, changed: false, profile: null };
  const desc = iccDescription(meta.icc);
  if (/srgb/i.test(desc)) return { buffer: inputBuffer, changed: false, profile: desc };
  const out = await sharp(inputBuffer).rotate().jpeg({ quality }).toBuffer(); // rotate() bakes EXIF orientation
  return { buffer: out, changed: true, profile: desc || 'unknown' };
}

// The human name inside an ICC profile ('Display P3', 'sRGB IEC61966-2.1', …).
// Reads the tag table (starts at byte 128), finds the 'desc' tag and decodes
// its data: v2 profiles store 'desc' + ASCII, v4 profiles store 'mluc' + UTF-16.
function iccDescription(icc) {
  try {
    const b = Buffer.from(icc);
    const n = b.readUInt32BE(128);
    for (let i = 0; i < n && i < 256; i++) {
      const e = 132 + i * 12;
      if (b.toString('latin1', e, e + 4) !== 'desc') continue;
      const off = b.readUInt32BE(e + 4), len = b.readUInt32BE(e + 8);
      const type = b.toString('latin1', off, off + 4);
      if (type === 'desc') {
        const count = b.readUInt32BE(off + 8);
        return b.toString('latin1', off + 12, off + 12 + Math.max(0, count - 1)).replace(/\0+$/, '').trim();
      }
      if (type === 'mluc') {
        const recLen = b.readUInt32BE(off + 12), rOff = off + 16;
        const sLen = b.readUInt32BE(rOff + 4), sOff = b.readUInt32BE(rOff + 8);
        return b.slice(off + sOff, off + sOff + sLen).swap16().toString('utf16le').replace(/\0+$/, '').trim();
      }
      return '';
    }
    return '';
