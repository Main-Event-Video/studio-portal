...
async function loadClientMedia(db, clientId) {
  const base = 'id, r2_key, filename, content_type, folder_path, sort_number, created_at, size_bytes';
  // crop_key / crop_rect are newer than timeline_pos, so they get their own rung
  // on the ladder. Collapsing them into one attempt would mean a portal without
  // the crop migration ALSO losing timeline_pos, which silently reorders every
  // client's montage — a much worse failure than not knowing about a crop.
  const sel = (cols) => db.from('studio_media').select(cols).eq('client_id', clientId).eq('kind', 'client_upload');
  const full = `${base}, timeline_pos, crop_key, crop_rect`;
  const withPos = `${base}, timeline_pos`;
  let { data, error } = await sel(full).is('hidden_at', null);
  if (error) ({ data, error } = await sel(full));
  if (error) ({ data, error } = await sel(withPos).is('hidden_at', null));
  if (error) ({ data, error } = await sel(withPos));
  if (error) ({ data, error } = await sel(base));
  return { data, error };
}
