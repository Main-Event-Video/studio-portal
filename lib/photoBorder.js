...
export function normalizeStyleBorder(sb) {
  ...
  // Already normalized ({ on, w, color, at } — no `mode`): pass it through.
  // The montage route normalizes once up top and then hands THAT to
  // resolveBorder, which normalized it again, saw no `mode`, and returned
  // null — so the montage-wide border never reached a single photo (Josh 9/9:
  // "this had a 2.0 white frame selected — why didn't it work").
  if (sb.mode === undefined && typeof sb.on === 'boolean' && Number.isFinite(Number(sb.w))) return normalizeBorder(sb);
  const mode = sb.mode;
