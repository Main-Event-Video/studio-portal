...
import { deleteFile, rotateStoredImage, cropStoredImage } from '@/lib/r2';
...
export async function applyMediaAction(db, clientId, body, { allowDelete = false, actor = 'client' } = {}) {
  ...
  if (action === 'rotate') {
    // Rotate a stored photo 90° (default) — non-destructive edit, allowed for
    // clients too. Overwrites the same R2 object so it updates everywhere.
    // Target the photo by studio_media id OR by r2_key (the montage editor keys
    // photos by r2_key). Either way it's scoped to this client's own uploads.
    const { id, key } = body;
    if (!id && !key) return { error: 'Missing file id', status: 400 };
    const angle = body.angle == null ? 90 : Number(body.angle);
    const sel = scope(db.from('studio_media').select('r2_key, content_type'));
    const { data: row } = await (id ? sel.eq('id', id) : sel.eq('r2_key', key)).maybeSingle();
    if (!row?.r2_key) return { error: 'File not found', status: 404 };
    if (!(row.content_type || '').startsWith('image/')) return { error: 'Only photos can be rotated', status: 400 };
    try { await rotateStoredImage(row.r2_key, row.content_type, angle); }
    catch (e) { return { error: 'Could not rotate photo', detail: e.message, status: 500 }; }
    return { ok: true };
  }

  if (action === 'crop' || action === 'uncrop') {
    // Client-facing crop. THE ORIGINAL IS NEVER TOUCHED — the crop is written to
    // a sibling key and the row points at it; 'uncrop' just forgets the pointer,
    // so undo is instant and the full frame is always recoverable.
    const { id, key } = body;
    if (!id && !key) return { error: 'Missing file id', status: 400 };
    const sel = scope(db.from('studio_media').select('id, r2_key, content_type'));
    const { data: row } = await (id ? sel.eq('id', id) : sel.eq('r2_key', key)).maybeSingle();
    if (!row?.r2_key) return { error: 'File not found', status: 404 };
    if (!(row.content_type || '').startsWith('image/')) return { error: 'Only photos can be cropped', status: 400 };

    if (action === 'uncrop') {
      const { error } = await scope(db.from('studio_media').update({ crop_key: null, crop_rect: null })).eq('id', row.id);
      if (error) return { error: 'Could not restore the photo', detail: error.message, status: 500 };
      return { ok: true };
    }

    const r = body.rect || {};
    const nums = ['x', 'y', 'w', 'h'].map((k) => Number(r[k]));
    if (nums.some((n) => !Number.isFinite(n))) return { error: 'Bad crop', status: 400 };
    const [x, y, w, h] = nums;
    // A crop that keeps almost nothing is a mis-drag, not an intention.
    if (w <= 0.02 || h <= 0.02 || x < -0.001 || y < -0.001 || x + w > 1.001 || y + h > 1.001) {
      return { error: 'Bad crop', status: 400 };
    }
    let made;
    try { made = await cropStoredImage(row.r2_key, row.content_type, { x, y, w, h }); }
    catch (e) { return { error: 'Could not crop photo', detail: e.message, status: 500 }; }
    const { error } = await scope(db.from('studio_media').update({
      crop_key: made.key,
      // `by` says whose framing this is: the admin's badge reads differently for
      // a crop the CLIENT chose (leave it alone) and one Josh made himself.
      crop_rect: { x, y, w, h, ratio: body.ratio === '9:16' ? '9:16' : '16:9', by: actor === 'admin' ? 'admin' : 'client' },
    })).eq('id', row.id);
    if (error) return { error: 'Could not save the crop', detail: error.message, status: 500 };
    return { ok: true, cropKey: made.key };
  }
...
// Persist the WHOLE timeline arrangement in one call. SHARED by the portal
// reorder AND the admin montage reorder so both write the exact same order the
// client reads back via buildTimeline — that's what makes admin<->client sync.
//   top    : [{ type:'media', id } | { type:'album', name }]  (top-level order)
//   albums : { "<albumName>": ["<mediaId>", ...] }            (order inside each album)
// Loose media get folder_path=null + timeline_pos=rank; albums get
// studio_boxes.position=rank; media inside an album get folder_path=album,
// sort_number=rank-inside, timeline_pos=null. Renumbered 1..N every save.
export async function applyArrangement(db, clientId, { top, albums } = {}) {
  const topItems = Array.isArray(top) ? top : [];
  const albumMap = albums && typeof albums === 'object' ? albums : {};
  ...
  const writeAll = async (withTLP) => {
    ...
    for (const [name, ids] of Object.entries(albumMap)) {
      ...
      for (const id of ids) {
        const pos = j, mid = id;
        mediaWrites.push(async () => {
          const patch = withTLP ? { folder_path: albumName, sort_number: pos, timeline_pos: null } : { folder_path: albumName, sort_number: pos };
          const { error } = await db.from('studio_media').update(patch)
            .eq('client_id', clientId).eq('kind', 'client_upload').eq('id', mid);
          return error ? error.message : null;
        });
        j++;
      }
  ...
  try {
    try { await writeAll(true); }
    catch (e) {
      if (/timeline_pos/i.test(String(e?.message || e))) await writeAll(false);
      else throw e;
    }
  } catch (e) {
    return { error: 'Could not save the order', detail: String(e.message || e), status: 500 };
  }
  return { ok: true };
}
