...
export function planStills(project, clips, opts = {}) {
  ...
  scenes.forEach((scene, k) => {
    ...
    // ALWAYS take the auto pick, even when this photo has its own. The cycle is
    // a fixed walk by POSITION: photo 5 gets the fifth move whatever photo 4 was
    // given. Skipping the draw for a picked photo shifted every Auto after it by
    // one — Josh 9/13: "when I change a transition it passes it all down …
    // if I change 1 it then moves to the next and right down the line".
    const auto = pick()
    let key = picked ? ov : auto
