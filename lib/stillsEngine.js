...
function openWindow(name, els, D, track, x0, y0, w, h, t0, s0, t1, s1, easing = E.qOut, props = {}) {
  const NS = 12, fn = EASE_FN[easing] || EASE_FN[E.qOut], MIN = 0.05
...
export function buildStillsElements(project, clips, ctx) {
  ...
  scenes.forEach((scene, k) => {
    ...
    if (key && STILLS_FX[key]) {
      ...
      if (ctx.keySafe) fxEls = keySafe(fxEls, aSet, bSet, coverScale(prev, scene, ctx), D)
...
// images". A pixel that is PARTLY transparent over the key colour blends
// with it, and that tint survives the key — so on a keyable backdrop nothing
// in an FX scene may be semi-transparent where the backdrop shows:
//   • decorative shapes (flash white, dim black, sweep bar, drops) are dropped
//   • blur_radius is dropped (blurred edges are semi-transparent fringes)
//   • opacity keyframes become hard steps at the midpoint of each fade, so a
//     layer is always fully on or fully off (a fade-out ends as a cut-out,
//     a fade-in as a cut-in); the moves themselves (slides, spins, pieces)
//     are untouched because opaque motion never tints
//   • the one fade allowed to stay is B fading in (without moving) over A
//     when B's photo box sits INSIDE A's and B is layered above A — then A is
//     held still and opaque underneath, so everything behind the fade is real
//     picture (the portal's proven "cover" rule)
// Elements are told apart by identity: the stack arrays A()/B() returned.
// → the zoom factor A needs (about the frame centre, on top of its 106% drift
//   end) so its picture covers B's box; 1 when B already sits inside A; null
//   for screens (no cover possible).
function coverScale(prev, scene, ctx) {
  ...
  const a = photoBox(prev.clips[0], ctx.width, ctx.height, null, insetFor(prev.clips[0]))
  const b = photoBox(scene.clips[0], ctx.width, ctx.height, null, insetFor(scene.clips[0]))
  const grow = DRIFT.scale1 / 100
  const H = ctx.height, W = ctx.width
  // A's centre after its drift sits DRIFT.dy/2 above centre, B's the same
  // amount below; scaling A about the frame centre moves A's centre too.
  const off = DRIFT.dy / 2 / 100 // fraction of frame height
  const aH = (a.boxH * grow) / H, bH = b.boxH / H, aW = (a.boxW * grow) / W, bW = b.boxW / W
  const kH = (off + bH / 2 + 0.01) / Math.max(0.05, aH / 2 - off)
  const kW = (bW / 2 + 0.01) / Math.max(0.05, aW / 2)
  const k = Math.max(1, kH, kW)
  if (k > 3) return null
  // the boxes as frame fractions (B at its start, A at its drift end)
  const boxes = { a: { w: aW, h: aH, cx: 0.5, cy: 0.5 - off }, b: { w: bW, h: bH, cx: 0.5, cy: 0.5 + off } }
  return { k: Number(k.toFixed(3)), boxes }
}
...
function keySafe(els, aSet, bSet, cover, D) {
  const side = el => {
  ...
  // "Cover" is only possible when B sits ABOVE A (then A can be held still and
  // opaque under B's fade). When A's pieces fly off to reveal B underneath, A
  // must keep moving — its fades become cuts instead.
  const aTracks = comps.filter(el => side(el) === 'A').map(el => el.track)
  const bTracks = comps.filter(el => side(el) === 'B').map(el => el.track)
  // …and only when something on B actually fades without moving (otherwise
  // A keeps its own motion — opaque motion never tints).
  ...
  const hold = needHold && cover != null && bOnTop
  const k = cover ? cover.k : 1
  // When B's box is bigger than A's, A first zooms up (opaque, about the frame
  // centre) until it covers B's box, and B's fade is pushed after that zoom —
  // a zoom-then-dissolve instead of a pop (Josh 9/9: "background popping").
  const zoomEnd = hold && k > 1 ? r3(D * 0.45) : 0
  // REVEAL-BENEATH moves (A's pieces fly off and B waits underneath): B's box
  // is a different shape from A's, so the part of B outside A's footprint used
  // to be there from frame one — "the background pops". Now a rectangular
  // window over B starts at A's footprint and opens out to B's full box over
  // the second half, so B's world grows out of A's frame instead.
  const reveal = !bOnTop && cover != null && !!bSet.size
  const bx = cover && cover.boxes.b, ab = cover && cover.boxes.a
  return comps.map(el => {
    const { blur_radius, blur_mode, ...rest } = el
    const s = side(el)
    if (s === 'A' && hold) {
      for (const k of TRANSFORM_KEYS) if (Array.isArray(rest[k])) delete rest[k]
      delete rest.opacity
      if (k > 1) { rest.x_scale = kf(0, '100%', zoomEnd, pct(k * 100), E.qIO); rest.y_scale = kf(0, '100%', zoomEnd, pct(k * 100), E.qIO) }
    } else if (s === 'B' && reveal && bSet.has(rest.elements) && (ab.w < bx.w - 0.005 || ab.h < bx.h - 0.005)) {
      const s0 = { x: Math.min(1, ab.w / bx.w), y: Math.min(1, ab.h / bx.h) }
      // the unit's own motion (drop, spin-zoom…) rides on a wrapper INSIDE the window
      const mover = { ...rest, track: 1 }
      delete mover.opacity
      const win = openWindow(rest.name, [mover], D, rest.track, bx.cx - bx.w / 2, bx.cy - bx.h / 2, bx.w, bx.h, r3(D * 0.5), s0, D, 1, E.qOut)
      if ('opacity' in rest) win.opacity = stepOpacity(rest.opacity)
      return win
    } else if (s === 'B' && hold && !moves(rest) && Array.isArray(rest.opacity)) {
      if (zoomEnd) rest.opacity = rest.opacity.map(k => ({ ...k, time: r3(zoomEnd + Number(k.time) * (D - zoomEnd) / D) }))
    } else if ('opacity' in rest) {
