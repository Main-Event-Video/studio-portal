...
export function buildStillsElements(project, clips, ctx) {
  ...
  scenes.forEach((scene, k) => {
    ...
    if (key && STILLS_FX[key]) {
      ...
      if (ctx.keySafe) fxEls = keySafe(fxEls, aSet, bSet, bInsideA(prev, scene, ctx))
...
function bInsideA(prev, scene, ctx) {
  if (!prev || !scene || prev.clips.length !== 1 || scene.clips.length !== 1) return false
  ...
  const grow = DRIFT.scale1 / 100, dy = (DRIFT.dy / 100) * ctx.height
  return b.boxW <= a.boxW * grow - 2 && b.boxH + 2 * dy <= a.boxH * grow - 2
}
...
function keySafe(els, aSet, bSet, bInside) {
  const side = el => {
  ...
  const hold = needHold && bInside && aTracks.length && bTracks.length && Math.max(...aTracks) < Math.min(...bTracks)
  return comps.map(el => {
    ...
    if (s === 'A' && hold) {
      ...
      delete rest.opacity
    } else if ('opacity' in rest && !(s === 'B' && hold && !moves(rest))) {
