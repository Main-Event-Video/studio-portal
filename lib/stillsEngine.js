...
export function buildStillsElements(project, clips, ctx) {
  ...
  scenes.forEach((scene, k) => {
    ...
    if (key && STILLS_FX[key]) {
      ...
      if (ctx.keySafe) fxEls = keySafe(fxEls, aSet, bSet, sceneBoxes(prev, scene, ctx), D)
...
function sceneBoxes(prev, scene, ctx) {
  ...
  // B at its drift START (100%, centre below middle)
  return {
    a: { w: (a.boxW * grow) / ctx.width, h: (a.boxH * grow) / ctx.height, cx: 0.5, cy: 0.5 - off },
    b: { w: b.boxW / ctx.width, h: b.boxH / ctx.height, cx: 0.5, cy: 0.5 + off },
  }
}
...
function keySafe(els, aSet, bSet, geo, D) {
  const side = el => {
  ...
  return comps.map(el => {
    ...
    if (s === 'A' && isUnit(rest) && geo && bOnTop && (hold || !moves(rest))) {
      ...
      delete rest.opacity
      if (sameBox(geo.a, geo.b)) return rest
      return morph(rest.name, rest.elements, D, rest.track, geo.a, geo.b, 1, coverScale(geo.a, geo.b), 0, zoomEnd)
    }
    ...
    if (s === 'B' && reveal && isUnit(rest)) {
      ...
      const win = morph(rest.name, [mover], D, rest.track, geo.a, geo.b, coverScale(geo.b, geo.a), 1, settle, D)
    ...
    if ('opacity' in rest) {
      ...
      if (s === 'A' && isUnit(rest) && last < 50 && Array.isArray(rest.x_scale) && lastScale <= 100) {
        ...
        rest.y_scale = kf(start, rest.x_scale[0].value, end, '0%', E.qIn)
        delete rest.opacity
      } else {
