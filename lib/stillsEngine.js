...
export const STILLS_FX = {
  ...
  cutfly: { label: 'Cut-out fly-in', family: 'Cut-out', D: 1.4, needs: 'cutoutB', fallback: 'push', build: ({ A, B, D }) => [
    unit('A', A('end'), D, 1),
    unit('Bbed', B('start', { bedOnly: true }), D, 2, { opacity: kf(0, '0%', D * 0.25, '100%', E.qIO) }),
    unit('Bfull', B('start'), D, 3, { opacity: kf(D * 0.68, '0%', D, '100%', E.qIO) }),
...
// Everything the B-side pull-down may offer (native first, then FX).
export const ALL_TRANSITION_KEYS = [...Object.keys(NATIVE), ...FX_KEYS]
export const FX_LABELS = Object.fromEntries([...Object.keys(NATIVE).map(k => [k, k]), ...FX_KEYS.map(k => [k, STILLS_FX[k].label])])
