...
// Copy a file from a URL into R2 WITHOUT holding it in memory: streamed in
// 16 MB parts through a multipart upload. Built for the watermarked rough cuts —
// a 9-minute 720p cut is a few hundred MB, and the old path (fetch the whole
// thing into a Buffer, then one PUT) blew the serverless function's memory and
// time, so the render finished but the delivery never happened.
export async function putFileFromUrl(key, url, contentType) {
  const res = await fetch(url);
  if (!res.ok || !res.body) throw new Error(`fetch ${res.status}`);
  const PART = 16 * 1024 * 1024;
  const { UploadId } = await R2.send(new CreateMultipartUploadCommand({ Bucket: BUCKET, Key: key, ContentType: contentType }));
  const parts = [];
  let pending = [], size = 0, n = 0;
  const flush = async () => {
    if (!size) return;
    const body = Buffer.concat(pending, size); pending = []; size = 0; n += 1;
    const { ETag } = await R2.send(new UploadPartCommand({ Bucket: BUCKET, Key: key, UploadId, PartNumber: n, Body: body }));
  ...
  try {
    ...
    for (;;) {
      ...
      if (done) break;
      pending.push(Buffer.from(value)); size += value.byteLength;
      if (size >= PART) await flush();
    }
    await flush();
    if (!parts.length) throw new Error('empty file');
    await R2.send(new CompleteMultipartUploadCommand({ Bucket: BUCKET, Key: key, UploadId, MultipartUpload: { Parts: parts } }));
  } catch (e) {
    try { await R2.send(new AbortMultipartUploadCommand({ Bucket: BUCKET, Key: key, UploadId })); } catch { /* best effort */ }
    throw e;
  }
}
