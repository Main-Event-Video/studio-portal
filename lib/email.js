...
// -------------------------------------------------------------------
export async function sendCharacterSheetReady({ client, buffer, count = 0, profile = null }) {
  const to = process.env.INTAKE_NOTIFY_EMAIL || process.env.ADMIN_EMAIL;
  if (!to) return false;

  const adminUrl = `${siteUrl()}/admin`;
  const filename = `character-build-${String(client.display_name || 'client').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}.png`;
  ...
  const html = shell(`
    ...
    <p style="color:#555;font-size:15px;line-height:1.6;margin:0 0 8px;">
      ${escapeHtml(client.display_name)} has ${count} character reference shot${count === 1 ? '' : 's'} in.
  ...
  const profileText = profile
    ...
    : '';
  const text = `${client.display_name} has ${count} character reference shot(s) in.
...
Open admin: ${adminUrl}`;
  ...
  try {
    await getClient().sendEmail({
      From: fromAddress(),
      To: to,
      Subject: `Character build sheet — ${client.display_name}`,
      ...STREAM,
      TextBody: text,
      HtmlBody: html,
      Attachments: buffer
