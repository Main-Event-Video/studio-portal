...
// -------------------------------------------------------------------
export async function sendFinalSetReady({ client, count = 1, note = '', to = '', watchUrl = '' }) {
  const portalUrl = `${siteUrl()}/p/${client.portal_token}`;
  ...
  const html = shell(`
    ...
    <p style="color:#555;font-size:16px;line-height:1.7;margin:0 0 8px;">
      Your final ${many ? `videos are` : `video is`} ready — ${count} ${many ? 'versions' : 'version'} on one page, ready to watch and download.
    </p>
    ${noteBlockHtml}
...
Main Event Studio — maineventstudio.com`;
  ...
  await getClient().sendEmail({
    ...
    To: recipient,
    ReplyTo: process.env.POSTMARK_REPLY_TO || undefined,
    Subject: `Your final ${many ? 'videos are' : 'video is'} ready — Main Event Studio`,
    ...STREAM,
    TextBody: text,
    HtmlBody: html,
  });
