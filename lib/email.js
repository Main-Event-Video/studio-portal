...
// -------------------------------------------------------------------
// Final SET delivery: multiple final files (e.g. different aspect ratios) sent
// together as ONE email → one no-login page listing every file. Filenames are the
// labels (kept as-is). Throws on send failure so the admin UI can surface it.
// -------------------------------------------------------------------
export async function sendFinalSetReady({ client, count = 1, note = '', to = '', watchUrl = '', version = '' }) {
  // "FINAL" is the default label and reads as plain "final"; anything else ("FINAL Revised") is spelled out.
  const vl = version && String(version).trim() && String(version).trim().toUpperCase() !== 'FINAL' ? String(version).trim() : '';
  const finalWord = vl ? `${vl} ` : 'final ';
  const portalUrl = `${siteUrl()}/p/${client.portal_token}`;
  const link = watchUrl || portalUrl;
  const recipient = to && String(to).trim() ? String(to).trim() : `${client.display_name} <${client.email}>`;
  const many = count !== 1;
  const noteBlockHtml = note
    ? `<div style="background:#f6f8ff;border-left:4px solid #3d7bff;padding:14px 18px;margin:22px 0;border-radius:0 8px 8px 0;">
  ...
  const html = shell(`
    ...
    <p style="color:#555;font-size:16px;line-height:1.7;margin:0 0 8px;">
      Your ${finalWord}${many ? `videos are` : `video is`} ready — ${many ? `${count} files on one page` : 'on one page'}, ready to watch and download.
    </p>
    ${noteBlockHtml}
    ${button(link, many ? 'View & download your videos' : 'View & download your video')}
    ${watchUrl ? '' : `<p style="color:#888;font-size:13px;line-height:1.6;text-align:center;margin:8px 0 0;">
      You'll be asked for your portal password — the one Main Event Studio gave you.
    </p>`}
  `);
...
Main Event Studio — maineventstudio.com`;
  ...
  await getClient().sendEmail({
    ...
    To: recipient,
    ReplyTo: process.env.POSTMARK_REPLY_TO || undefined,
    Subject: `Your ${finalWord}${many ? 'videos are' : 'video is'} ready — Main Event Studio`,
    ...STREAM,
    TextBody: text,
    HtmlBody: html,
  });
...
export async function sendCharacterSheetReady({ client, buffer, count = 0, profile = null, name = '' }) {
  ...
  const to = process.env.CHARACTER_SHEET_EMAIL || 'info@maineventstudio.com';
  if (!to) return false;
