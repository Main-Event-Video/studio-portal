...
// Target AI tool this character is being built for. Keep the keys in sync with
// PROGRAMS in app/api/admin/character/route.js and the admin dropdown. `label`
// is stamped on the sheet header; `promptNote` is a short usage hint under the
// AI prompt.
const PROGRAM_INFO = {
  openart: { label: 'OpenArt — Character', promptNote: 'Create a Character in OpenArt from these shots, then use this prompt.' },
  higgsfield: { label: 'Higgsfield — Soul ID', promptNote: 'Train a Soul ID from these shots, then use this prompt.' },
  midjourney: { label: 'Midjourney — cref / oref', promptNote: 'Attach these shots as --cref / --oref, then use this prompt.' },
  lora: { label: 'LoRA training', promptNote: 'Use these shots as the training set with this as the base caption.' },
};
...
export async function buildCharacterSheet(db, characterId, opts = {}) {
  ...
  // Optional target AI program (stamped in the header + tweaks the prompt hint).
  const progKey = opts.program ? String(opts.program).toLowerCase() : '';
  const progInfo = PROGRAM_INFO[progKey] || null;
  ...
  s.push(`<text x="${nameX}" y="150" font-family="sans-serif" font-size="23" font-weight="600" fill="${MUT}">Character Build Sheet${progInfo ? ` · for ${esc(progInfo.label)}` : ''}</text>`);
  ...
  if (profile) {
    ...
    if (promptLines.length) {
      ...
      s.push(`<text x="${panelX + padX}" y="${boxY + 26}" font-family="sans-serif" font-size="14" font-weight="700" letter-spacing="1" fill="${RED}">AI PROMPT — ${esc(progInfo ? progInfo.promptNote : 'paste into your image tool')}</text>`);
    ...
    s.push(`<text x="${panelX + padX}" y="${panelY + 72}" font-family="sans-serif" font-size="16" fill="${MUT}">Your character write-up will appear here once it’s ready.</text>`);
  }
