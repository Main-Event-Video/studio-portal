...
export async function buildCharacterSheet(db, clientId, opts = {}) {
  let { data: clientRow } = await db.from('studio_clients').select('display_name, character_name').eq('id', clientId).single();
  if (!clientRow) ({ data: clientRow } = await db.from('studio_clients').select('display_name').eq('id', clientId).single());
  const clientName = (opts.name && String(opts.name).trim())
    || (clientRow?.character_name && String(clientRow.character_name).trim())
  ...
  // Best-effort read of the cache. Wrapped so a not-yet-migrated DB (columns
  // missing) simply behaves as a cache miss instead of throwing.
  let cached = null, cachedSig = null;
  if (!profile) {
    try {
      const { data: cRow } = await db.from('studio_clients')
        .select('character_profile, character_profile_sig').eq('id', clientId).single();
      cached = cRow?.character_profile || null;
      cachedSig = cRow?.character_profile_sig || null;
    } catch { /* columns not present yet — treat as miss */ }
  }
  ...
  if (!profile && !opts.force && cached && cachedSig && cachedSig === sig) {
    ...
    if (profile) {
      try {
        await db.from('studio_clients')
          .update({ character_profile: profile, character_profile_sig: sig })
          .eq('id', clientId);
      } catch { /* columns not present yet — skip caching */ }
    }
  }
