...
// Trash and the client's Character Build holding areas. Character shots live
// under '__character_build__' (the first/legacy character) OR
// '__character_build__::<id>' (additional characters), so we match the whole
// family by prefix. Kept literal here so this helper has zero imports (shared by
// client + server) — MUST stay in sync with CHAR_FOLDER in lib/characterPoses.js.
const CHAR_PREFIX = '__character_build__';
function isExcludedFolder(fp) {
  return fp === TRASH_FOLDER || fp === CHAR_PREFIX || fp.startsWith(`${CHAR_PREFIX}::`);
}
...
export function buildTimeline(mediaRows, boxRows) {
  ...
  for (const b of rawBoxes) {
    ...
    if (isExcludedFolder(String(b.name).trim())) continue;   // never seed a character/trash box as an album
  ...
  for (const m of media) {
    ...
    if (isExcludedFolder(fp)) continue;           // trashed or character-build — never plays
    if (fp) {
