...
function epicVintageSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, assetBase, perPhoto = null }) {
  ...
  const SC     = [230, 132, 100, 60, 33, 15];
  const ROT    = [-6, -2, 6, 26, 46, 64];                // ×dir — always turning
  const TX     = [0, 3, 10, 28, 54, 78];                  // ×dir — always drifting
  ...
  photos.forEach((it, i) => {
    ...
    const bgPrints = SPOTS.map((s) => {
      ...
      return {
        ...
        elements: [
          ...
          { type: 'image', source: it.url, x: '50%', y: '50%', width: `${(100 - 2 * 1.2).toFixed(1)}%`, height: `${(100 - 2 * 1.2).toFixed(1)}%`, x_anchor: '50%', y_anchor: '50%', fit: 'cover', color_filter: 'sepia', color_filter_value: '58%', blur_radius: 6, blur_mode: 'stack' },
          ...
          { type: 'shape', x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', path: rect, fill_color: '#2A1B0C', blend_mode: 'multiply', opacity: '18%' },
        ],
      };
    });
    ...
    const heroWrap = {
      ...
      z_rotation: [{ time: 0, value: '0°', easing: 'linear' }, { time: D, value: `${(dir * 10).toFixed(1)}°` }],
    ...
    elements.push({
      ...
      elements: [
        ...
        { type: 'image', source: it.url, x: '50%', y: '50%', width: '128%', height: '128%', x_anchor: '50%', y_anchor: '50%', fit: 'cover', color_filter: 'sepia', color_filter_value: '70%', blur_radius: 24, blur_mode: 'stack', opacity: '100%' },
        { type: 'shape', x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', path: rect, fill_color: '#221606', blend_mode: 'multiply', opacity: '26%' },
        ...bgPrints,
        heroWrap,
      ],
    });
  ...
  const glow = (w, x0, y0, x1, y1, op) => ({
    ...
    blend_mode: 'screen', blur_radius: 40, blur_mode: 'stack', opacity: `${op}%`,
  ...
  for (let k = 1; k < n; k++) {
    ...
    elements.push({ name: `Bloom-flash-${k}`, type: 'shape', track: 19, time: t0, duration: dur, path: rect, width: '100%', height: '100%', x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%', fill_color: '#FFEECB', blend_mode: 'screen', blur_radius: 8, blur_mode: 'stack', opacity: ramp(20) });
    // wide amber halo from the corner
    elements.push({ name: `Bloom-halo-${k}`, type: 'shape', track: 20, time: t0, duration: dur, path: circle, width: '100%', height: '100%', x: `${c.x}%`, y: `${c.y}%`, x_anchor: '50%', y_anchor: '50%', fill_color: '#FFD79A', blend_mode: 'screen', blur_radius: 44, blur_mode: 'stack', opacity: ramp(72) });
    // intense warm-white core
    elements.push({ name: `Bloom-core-${k}`, type: 'shape', track: 21, time: t0, duration: dur, path: circle, width: '56%', height: '56%', x: `${c.x}%`, y: `${c.y}%`, x_anchor: '50%', y_anchor: '50%', fill_color: '#FFFCF4', blend_mode: 'screen', blur_radius: 30, blur_mode: 'stack', opacity: ramp(85) });
