...
function glassDrift(el, k, dur, p) {
  const dir = k % 2 ? -1 : 1;
...
function glassPart(el, k, dur, acts, p, behindCopy) {
  glassDrift(el, k, dur, p);
...
function glassSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null, light = true }) {
  ...
  items.forEach((it, i) => {
    ...
    const inner = [];
    const frontPanes = L.panes.filter((p) => !p.behind);
    ...
    // copy of each blade.
    L.panes.filter((p) => p.behind).forEach((p, k) => inner.push(glassDrift(glassPane(p, it, W, H, S), k, step, p)));
    frontPanes.forEach((p, k) => inner.push(glassPart(glassPane(p, it, W, H, S), k + 1, step, acts, p, true)));
    inner.push(glassHero(glassPane(L.hero, it, W, H, S), step, acts, L.hero));
    frontPanes.forEach((p, k) => inner.push(glassPart(glassPane(p, it, W, H, S), k + 1, step, acts, p, false)));
