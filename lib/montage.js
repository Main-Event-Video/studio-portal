import { stillsSource } from './stillsEngine';
...
export const STYLES = {
  ...
  party2: {
    label: 'Party 2 (energetic — drift; slide + circular-wipe + wipe + fade)',
    greenDefault: true,
    bg: '#140A22',
    text: '#FFFFFF',
    dim: '#FF6EB4',
    ...
    // uncovers the frame for the green to bleed through.
    // 2026-09-11: Josh — "make party 3 now party 2". The straight wipe that was
    // Party 3's only difference moves in here; Party 3 below is now just an
    // alias so old render rows carrying its id keep exporting the same look.
  ...
  duotone: {
    finishedOnly: true,   // the montage IS the picture — never keyed, never a matte (Josh 9/16)
  ...
  // Party 3 RETIRED 2026-09-11 — its look (Party 2 plus a straight wipe) IS
  // Party 2 now. The id stays because render rows store the style id and an
  // unknown id silently falls back to Hollywood; filled in right after STYLES
  // is built so it can never drift from party2. Hidden from the admin picker.
  comic_book: {
  ...
  // MEVID STILLS (Josh 9/9) — the MEvid Stills transition engine as a Montage
  // Maker style: one photo at a time (or 2/3/4-up screens), cycling through all
  // 26 transitions; per-photo picks + manual screens come from the segment's
  // `stills` settings. Engine + adapter live in lib/stillsEngine.js (a COPY of
  // MEvid's lib/stills-fx.js — the repos never share a file).
  stills: {
    label: 'MEvid Stills (one at a time, 26 transitions, screens)',
    bg: '#000000', text: '#FFFFFF', dim: '#F5B301', font: 'Montserrat',
    kicker: 'MAIN EVENT STUDIO', photoS: 2.6, fadeS: 0.6,
    transitions: ['fade'], zoom: ['100%', '106%'],
    stills: true,
  },
};
STYLES.party3 = { ...STYLES.party2, label: 'Party 3 (retired — same as Party 2)' };
...
// cannot miss any.
// ---------------------------------------------------------------------------
// DELIVERY. Josh 9/16: "finished look should be for Duotone and collages and
// gallery 150 and epic vintage and trendy wall and glass — meaning these
// should not even have an option for keyable", and "anything keyable should
// default keyable, and vice versa". A style is keyable unless it is marked
// finishedOnly above; the admin derives the per-segment default from this.
export function styleIsKeyable(st) { return !!st && !st.finishedOnly; }

export const KEY_COLORS = [
  // ALPHA (Josh 9/15, the default): a BLACK backdrop for the two-pass alpha export
  // (colour pass + matte pass → one .mov with a real alpha channel, see
  // applyMattePass + tools/Alpha Merge.command). Nothing is chroma-keyed, so no
  // colour of clothing can ever be pulled, and dissolves fade toward black.
  { value: '#000000', label: 'Alpha (black backdrop)', note: 'The default. Export with alpha renders a colour pass and a matte pass; merged, they give a .mov with a true alpha channel — no chroma key, any colour clothing.' },
...
// MATTE PASS (Josh 9/15 — the magenta key was pulling the pink ski suit and a
// purple baby outfit). Turns a FINISHED source into its own luma matte: every
// photo/video becomes solid WHITE (color_overlay keeps the element's exact
// geometry, motion, masks and any cut-out alpha), the key-colour backdrop
// becomes BLACK, mats/borders/cards/text stay white, drop shadows are dropped
// (they would print as grey halo), decorative overlays (dust, leaks) are
// removed, audio is removed. Same timing, same motion — drop the render on the
// timeline as a track/luma matte over the colour render and no chroma key is
// needed at all, whatever colour the clothes are.
export function applyMattePass(source, keyColor) {
  if (!source) return source;
  const KEY = normalizeKeyColor(keyColor);
  const isKey = (v) => typeof v === 'string' && v.trim().toUpperCase() === KEY;
  const isKeyAsset = (u) => typeof u === 'string' && /\/(green|key-[0-9a-f]{6})\.png(\?|$)/i.test(u);
  ...
  const walk = (node) => {
    ...
    if (Array.isArray(node)) {
      // strip audio + decorative overlays in place
      for (let i = node.length - 1; i >= 0; i--) {
        const el = node[i];
        if (el && typeof el === 'object' && !Array.isArray(el)) {
          if (el.type === 'audio') { node.splice(i, 1); continue; }
          if (/^(OvlDust|OvlLeak|Leak-|Dust)/.test(el.name || '')) { node.splice(i, 1); continue; }
          if (el.name === 'Watermark') { node.splice(i, 1); continue; } // a white watermark would punch an opaque hole in the matte
        }
      }
      node.forEach(walk); return;
    }
    for (const k of Object.keys(node)) if (/^shadow_/.test(k)) delete node[k];
    if (node.type === 'image' || node.type === 'video') {
      node.color_overlay = isKeyAsset(node.source) ? BLACK : WHITE;
    }
    if (node.type === 'shape' || node.type === 'composition' || node.type === 'text') {
      if (typeof node.fill_color === 'string') node.fill_color = isKey(node.fill_color) ? BLACK : WHITE;
      if (typeof node.stroke_color === 'string') node.stroke_color = isKey(node.stroke_color) ? BLACK : WHITE;
      if (typeof node.background_color === 'string') node.background_color = isKey(node.background_color) ? BLACK : WHITE;
    }
    Object.values(node).forEach(walk);
  };
  walk(source.elements);
...
function bgLayers(background, S, { track = 1, time = 0, duration, shots = null }) {
  ...
  if (b.photoBlur && Array.isArray(shots) && shots.length) {
    ...
    // A shot may name the seam that leads INTO it (MEvid Stills sets it to the
    // transition's length, so the bed crossfades exactly under the move instead
    // of after it — Josh 9/9: "image pops behind").
    const seamInto = (i) => (shots[i] && Number(shots[i].seam) > 0 ? Number(shots[i].seam) : seam);
    const beds = shots.map((sh, i) => {
      const first = i === 0, last = i === shots.length - 1;
      const dur = (Number(sh.dur) || 0) + (first ? lead : 0) + (last ? trail : seamInto(i + 1));
      return {
        name: i === 0 ? 'Backdrop' : `Backdrop-${i + 1}`,
        type: 'image', source: sh.url, track: 2,
        x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%',
        fit: 'cover',
        // When the shot carries its item, aim the bed like every other cover-
        // crop: on the faces when known, else the top (Josh 9/9: "use the eye
        // engine — or at least the upper third, not the middle groin area").
        ...(sh.it && sh.it.w > 0 && sh.it.h > 0 ? coverBox(bedShot(sh.it, 1920, 1080), 1920, 1080) : {}),
        ...
        ...(first ? {} : { animations: [{ time: 'start', duration: Number(seamInto(i).toFixed(3)), transition: true, type: 'fade' }] }),
      };
    });
    return wrap([
...
function fullFrameMotion(S, it, { zoomIn, sizePct, amp, drift, frameW, frameH }) {
  ...
  // EYES, when the face job has seen the photo. bedShot turns the detected
  // faces into a framing bias, so a 4:3 or 4:5 photo cover-cropped into 16:9
  // is cropped around the people rather than pinned to the top edge. Same
  // helper Glass / Collage / Framed Box / Multi Page / Two Panel use. A photo
  // with no faces on record is unchanged (top-biased, as before), and a manual
  // drag (posX/posY) still wins inside coverBox. Josh: "does the revised Party
  // 2 have the eye tracker for the resize?" — now it does.
  // Only when Josh has not framed it himself: an explicit Center/Bottom/Left/
  // Right anchor, a Fix-framing adjustment (object), or a drag (posX/posY)
  // is his call and beats the detector. The default 'top' (or nothing) is the
  // house rule, and that is what the eyes refine.
  const manual = (it.framing && it.framing !== 'top') || Number.isFinite(it.posX) || Number.isFinite(it.posY);
  const aimed = manual ? it : bedShot(it, 1920, 1080);
  const img = applyPhotoColor({ type: 'image', source: it.url, ...coverBox(aimed, 1920, 1080) }, it);
  const comp = {
...
function multiPageSource({ S, seq, watermarkUrl, width, height, background = null, mp = {} }) {
  const rect = 'M 0 0 L 100 0 L 100 100 L 0 100 Z';
  const AREA = ['a', 'b', 'c', 'd'];
  ...
  // FACE-ANCHORED. bedShot returns the item with a framing override that puts
  // the faces where a cover-crop into THIS cell can still see them; with no
  // face data it returns the item untouched and the house top-bias applies, so
  // a photo the detector has not reached is exactly as it was.
  const headSafe = (it, c) => {
    const bw = (c.w / 100) * width, bh = (c.h / 100) * height;
    return coverBox(bedShot(it, bw, bh), bw, bh);
  };
  ...
  const elements = [];
  // THE BACKGROUND CONTROL HAS TO REACH HERE. This was a hard-coded shape in
  // the style's own colour, so the "Background" dropdown did nothing on these
  // two styles — and worse than nothing once the key colour became a setting:
  // the shape is green by default, so choosing a gradient and a magenta key
  // gave Josh a magenta screen. "In other words - the background pulldown
  // didn't work." On these two, it did not.
  //
  // bgLayers with no background returns exactly the same single fill in the
  // style's own colour, so nothing changes for anyone who has not chosen one.
  // The duration is patched below, as before.
  const bgEls = bgLayers(background, S, { track: 1, time: 0, duration: 1 });
  const bg = bgEls[0];
  bgEls.forEach((el) => elements.push(el));
...
export function styleNeedsDims(st) {
  ...
  return !!(s.collage || s.epic || s.trendy || s.wholePhoto || s.story || s.multipage || s.duotone || s.pan
    ...
    || s.framedBox
    // MEvid Stills frames every photo at its native shape and aligns cut-outs
    // to the pixel — it needs real dimensions.
    || s.stills);
}
...
export function styleNeedsFaces(st) {
  ...
  // Multi Page and Two Panel join them, and for the same reason as Glass:
  // Josh, on a Multi Page render, "can you add the eye detector to the narrow
  // sindows in montages like the 2 multi-page?" Their layouts put a landscape
  // photograph into a TALL narrow cell, and a cover-crop of a wide picture into
  // a narrow window throws most of the width away and keeps the middle — which
  // on a person is the forehead. It is the identical failure the Glass accent
  // panes had, in a different frame.
  // The one-at-a-time fill styles (Hollywood, Timeless, Party, Party 2, and any
  // photo set to Fill elsewhere) join too: fullFrameMotion now aims its
  // cover-crop at the eyes.
  return !!(st && (st.glass || st.collage || st.framedBox || st.multipage || st.twoPanel || !usesOwnBuilder(st)));
}
...
// THE PANEL IS THE PHOTO'S OWN SHAPE. Josh 2026-09-11: "restricting images to
// only 16x9 and 9x16 is not working. Have Two Panel retain the original image
// aspect ratio." Every panel used to be a fixed 16:9 or 9:16 box with the photo
// cover-cropped into it, so a 4:5 or 4:3 phone photo lost its top and bottom.
// Now the box takes the photo's real aspect and is FITTED inside the slot the
// fixed box occupied: same width and margins, and a height cap a little above
// the fixed box's height, so a squarer photo shows whole, a bit smaller, and
// never spills over its partner. A true 16:9 or 9:16 lands exactly where it
// did before, so the pairs Josh signed off on render 014 are unchanged.
function twoPanelFit(it, widthPct, slotAspect, hSide, hIns, vSide, vIns, W, H) {
  const A = (it && it.w > 0 && it.h > 0) ? it.w / it.h : slotAspect;
  const slot = twoPanelBox(widthPct, slotAspect, hSide, hIns, vSide, vIns, W, H);
  const hCap = Math.min(slot.h * 1.18, 100 - 2 * vIns);
  let w = widthPct, h = ((w / 100) * W / A) / H * 100;
  if (h > hCap) { h = hCap; w = ((h / 100) * H * A) / W * 100; }
  const x = hSide === 'left' ? hIns : (100 - hIns - w);
  const y = vSide === 'top' ? vIns : (100 - vIns - h);
  return { x, y, w, h, from: hSide };
}
...
function twoPanelCard(items, variant, W, H) {
  ...
  if (items.length === 1) {
    ...
    const box = twoPanelFit(it, w, p ? AR_916 : AR_169, 'left', (100 - w) / 2, 'top', 0, W, H);
    box.x = (100 - box.w) / 2; box.y = (100 - box.h) / 2; box.from = flip ? 'right' : 'left';
    return [{ it, box }];
  }
  const [a, b] = items, pa = twoPanelIsP(a), pb = twoPanelIsP(b);
  if (pa && pb) { // two portraits — tall pair, offset high/low
    const left = twoPanelFit(a, 29, AR_916, 'left', 19, flip ? 'bottom' : 'top', 4, W, H);
    const right = twoPanelFit(b, 29, AR_916, 'right', 19, flip ? 'top' : 'bottom', 4, W, H);
    return [{ it: a, box: left }, { it: b, box: right }];
  }
  if (!pa && !pb) { // two landscapes — wide pair on a diagonal
    const left = twoPanelFit(a, 59, AR_169, 'left', 3, flip ? 'bottom' : 'top', 5, W, H);
    const right = twoPanelFit(b, 59, AR_169, 'right', 3, flip ? 'top' : 'bottom', 5, W, H);
    return [{ it: a, box: left }, { it: b, box: right }];
  }
  // mixed — landscape high, portrait low; alternate which side each sits on
  const land = pa ? b : a, port = pa ? a : b, landLeft = !flip;
  const lBox = twoPanelFit(land, 58, AR_169, landLeft ? 'left' : 'right', 4, 'top', 6, W, H);
  const pBox = twoPanelFit(port, 29, AR_916, landLeft ? 'right' : 'left', 12, 'bottom', 6, W, H);
  return [{ it: a, box: a === land ? lBox : pBox }, { it: b, box: b === land ? lBox : pBox }]; // keep play order for stagger
}
function twoPanelSource({ S, seq, watermarkUrl, width, height, background = null, mp = {} }) {
  const rect = 'M 0 0 L 100 0 L 100 100 L 0 100 Z';
  const transition = mp.transition || 'record-fwd';
  ...
  cards.forEach((group, ci) => {
    ...
    const photoEls = placed.map((pl, idx) => {
      ...
      // Face-anchored, same as Multi Page: a narrow panel showing the middle of
      // a landscape photograph is showing a forehead.
      const pbw = (c.w / 100) * width, pbh = (c.h / 100) * height;
      const inner = withBorder([applyPhotoColor({ type: 'image', source: pl.it.url, ...coverBox(bedShot(pl.it, pbw, pbh), pbw, pbh) }, pl.it)], pl.it, pbw, pbh, Math.min(width, height));
...
export function twoPanelLayout({ items } = {}) {
  ...
  for (let i = 0; i < photos.length; i += 2) {
    ...
    for (let j = 0; j < cells; j++) {
      ...
      out[i + j] = { page: i / 2 + 1, cell: j + 1, cells, cellAspect: A };
    }
  }
  return out;
}
...
function buildMontageSourceInner({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, totalSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true, assetBase = null, mpTransition = null, mpStagger = null, mpHold = null, mpSpeed = null, duoPalette = null, duoTreatment: duoTreat = null, glassLight = true, glassRefl = true, atmosphere = true, atmoOpts = null, frameW = null, frameColor = null, stillsOpts = null, neonOpts = null, keyColor = null }) {
  ...
  if (S.twoPanel) return twoPanelSource({ S, seq, watermarkUrl, width, height, background, mp: { transition: mpTransition, stagger: mpStagger, hold: mpHold, speed: mpSpeed } });
  if (S.multipage) return multiPageSource({ S, seq, watermarkUrl, width, height, background, mp: { transition: mpTransition, stagger: mpStagger, hold: mpHold, speed: mpSpeed } });
  ...
  // MEvid Stills: its own shell (bed, cards, green bookends, watermark) inside
  // the adapter; finish() only — green is an injected photo like the slide family.
  if (S.stills) return finish(stillsSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background, perPhoto, stills: stillsOpts,
    lib: { bgLayers, card, CARD_S, greenSlot, applyPhotoColor, backdropIsGreen, borderIsOn: (bd) => !!(bd && bd.on && Number(bd.w) > 0) } }))
  ...
  gseq.forEach((it, i) => {
    ...
    if (S.pan && eff === 'fill') {
      ...
      // The box is built for a border, or when NEON is on — the universal neon
      // can only trace a composition that holds a picture, so a borderless Fit
      // photo (every 9:16 on Party 2) went out as a bare <image> and never got
      // a light (Josh: "the neon selector isn't hitting on every image").
      //
      // NOT for every Fit photo, though. 2026-09-12 that was tried, and the
      // very next renders lost their first picture whenever it was a vertical
      // (Reagan, photo 148: "148 is a vertical"). The bare image is the
      // construction every earlier render proved; the box is only used where
      // it is needed. Not render-proven either way — if a vertical still goes
      // missing with neon OFF, the box was not the cause.
      const wantBox = !!(neonOpts && neonOpts.on);
      const rect = (borderPx(it, Math.min(width, height)) || wantBox) ? containRectPct(it, width, height) : null;
      if (rect) {
        photoEl = {
          type: 'composition',
  ...
  // ---- HELD FADES for the movement styles under the ALPHA key ----------------
  // Josh 9/16, on Party exported with alpha: "every transition has an alpha in
  // the middle of the transition that shows the lower level." Creatomate's
  // `fade` transition fades the OUTGOING photo out while the incoming fades in,
  // so halfway through neither is fully there: coverage — and therefore the
  // matte — dips to ~75% and the track below shows through the merged clip.
  // Over a black backdrop it only ever looked like a dissolve, which is why it
  // was never noticed before alpha.
  //
  // holdFade (below) solves this for the pure-fade styles by re-laying EVERY
  // shot on two tracks, which flattens slides and wipes — so it cannot be used
  // here. This does the same thing for the fade transitions ONLY: the incoming
  // photo moves up one lane and fades in over the outgoing, which is left to
  // play out at full opacity underneath. Slides and wipes stay exactly as they
  // are (cover mode already holds the outgoing during those), auto-sequenced
  // on whatever lane the previous shot is on. Lanes climb by one per fade
  // (30, 31, 32 …) so the fading shot is always ABOVE the one it fades over;
  // the watermark moves to 99 to stay on top. Cards are pinned explicitly, as
  // holdFade pins them, because the photos no longer share their track.
  //
  // Only under the Alpha key: it is the alpha deliverable that exposes the dip,
  // and the signed-off chroma-key looks are left byte-for-byte alone.
  const alphaKey = normalizeKeyColor(keyColor) === '#000000';
  let usedHeldFades = false;
  if (alphaKey && S.coverTransitions && !S.holdFade) {
    const seqEls = elements.slice(seqStart);
    let lane = 30;
    let t = includeCards ? CARD_S : 0;
    seqEls.forEach((el, k) => {
      const dur = el.duration || photoS;
      const tr = Array.isArray(el.animations) ? el.animations.find((a) => a && a.transition) : null;
      const lap = tr ? (Number(tr.duration) || 0) : 0;
      const start = k === 0 ? t : t - lap;
      if (k === 0) {
        el.track = lane; el.time = +start.toFixed(3);
      } else if (tr && tr.type === 'fade') {
        lane += 1;
        el.track = lane; el.time = +start.toFixed(3);
        delete el.animations;
        el.opacity = [{ time: 0, value: '0%' }, { time: +lap.toFixed(3), value: '100%' }];
      } else {
        el.track = lane;                      // auto-sequences after the previous shot on this lane
        delete el.time;
      }
      t = start + dur;
    });
  ...
  if (includeCards) {
    elements.push({
      ...
      track: 2,
      duration: CARD_S,
      ...((usedHoldFade || usedHeldFades) ? { time: Math.max(0, total - CARD_S) } : {}),
...
export function applyOverlays(source, {
  ...
  // EXTRAS (Josh 9/9): the standing frame, bars, squiggles and echoes that join
  // in from 150% can be switched off, leaving only the tracing light at any
  // intensity. Undefined = on, so every montage saved before the switch keeps
  // its look.
  const neonExtras = !(neon && neon.extras === false);
  ...
  // THICKNESS has its own dial on top of that. Josh: "can we add a slider to
  // make it thicker too?" 100% = the thickness intensity alone would give;
  // it multiplies, so 200% doubles every stroke of the tube (and the squiggles)
  // without touching brightness, light count or speed. Same null trap as above.
  const neonThick = pc(neon && neon.thickness, 100);
  const neonWide = Math.pow(Math.max(0.15, neonMul), 0.7) * Math.max(0.25, neonThick);
  ...
  gaps.forEach(([t0, t1], gi) => {
    ...
    if (canAtmo && dustPct > 0) {
      add.push({
        ...
        // DEGREE SYMBOL, not "deg". Creatomate rejects the whole render with
        // "Shape.z_rotation: Expected a number ending with \u00B0". Every other
        // z_rotation in this file already uses \u00B0 — these two were written
        // in CSS habit and did not match the house convention.
        z_rotation: [{ time: 0, value: '0\u00B0', easing: 'linear' },
          { time: span, value: `${(6 * span / 12).toFixed(2)}\u00B0` }],
      });
    }
  ...
  if (neonOn && neonPct > 0) {
    ...
    let shotIdx = 0;
    // WHAT COUNTS AS A SHOT. Normally one top-level element. MEvid Stills
    // (and anything built the same way) puts its whole body in ONE top-level
    // "Clips" composition holding a SEQUENCE of scene compositions on one
    // track. Treating that container as the shot meant one 77-second "shot"
    // whose box search dived into every transition copy and every shatter
    // piece — 48 rings, a 1.7 MB source, and Creatomate refusing it outright
    // ("could not process this request", 9/9). So a sequence container is
    // opened up: each child scene is its own shot, with its own span, and the
    // ring holds steady across them instead of fading per scene.
    const shots = [];
    for (const e of source.elements) {
      ...
      const seq = kids.length >= 2 && kids.every((k) => k && k.type === 'composition' && Number(k.duration) > 0 && k.time == null && String(k.track) === String(kids[0].track));
      if (seq) {
        const inner = ovlResolveTimes(kids);
        kids.forEach((k) => { const s = inner.get(k); if (s) shots.push({ el: k, span: [sp[0] + s[0], sp[0] + s[1]], steady: true }); });
      } else shots.push({ el: e, span: sp, steady: false });
    }
    for (const { el: e, span: sp, steady } of shots) {
      const life = +(sp[1] - sp[0]).toFixed(3);
      if (life < 0.5) continue;

      // WHICH BOX IS THE PICTURE. Nesting in the top-level shot is only right
      // when that shot IS one photograph. Two Panel's Card-1 is 100% x 100% —
      // the whole frame — with its two photographs in compositions nested
      // inside it at 58% and 29%. So the ring traced the edge of the VIDEO and
      // neither picture. Josh: "the trace is gone." It was not gone; it was
      // drawn around the outside of the screen where half of it is off frame.
      //
      // Descend instead to the innermost composition that DIRECTLY holds an
      // image, and trace that. A one-photo shot is unchanged — its shot already
      // is that composition — and a multi-photo layout now gets a light on each
      // picture, which is what it should have been from the start. It was
      // capped at six "so a wall style cannot turn into a light show", which
      // meant Photo Ribbon, Gallery and Trendy lit half their pictures and
      // left the rest dark — Josh: "isn't hitting on every image". Every
      // picture gets its light; the cap is now only a runaway guard.
      const boxes = [];
      const findBoxes = (node, depth) => {
        if (!node || typeof node !== 'object' || boxes.length >= 48 || depth > 6) return;
        // A clipped window onto a copy of a scene (Stills' shatter/blinds/glitch
        // pieces, its wipe and puddle windows) is not a picture — a ring in
        // each fragment would draw dozens of partial arcs. The one exception is
        // a "-box" morph window, which holds the real photo comp during a move.
        if (node.clip === true && !/-box$/.test(node.name || '')) return;
        const kids = Array.isArray(node.elements) ? node.elements : [];
        const holdsPicture = kids.some((k) => (k.type === 'image' || k.type === 'video')
          && typeof k.source === 'string'
          && !/\/(green|key-[0-9a-f]{6})\.png(\?|$)/i.test(k.source)
          && !/-cutout\.png(\?|$)/i.test(k.source));   // a cut-out person is not a framed picture
        if (node.type === 'composition' && holdsPicture) { boxes.push(node); return; }
        kids.forEach((k) => findBoxes(k, depth + 1));
      };
      findBoxes(e, 0);
      if (!boxes.length) boxes.push(e);
      ...
      // The tracing light goes on EVERY picture in the shot.
      for (const box of boxes) {
        for (let li = 0; li < neonLights; li++) {
          box.elements.push(ring(px(0.022), Math.round(neonPct * 0.24), li));
          box.elements.push(ring(px(0.010), Math.round(neonPct * 0.48), li));
          box.elements.push(ring(px(0.004), neonPct, li));
        }
      }

      // ---- THE TWO-COLOUR FRAME, from 150% up ------------------------------
      // Every one of Josh's references is the same object: a complete neon
      // rectangle around the picture, lit in TWO colours that meet on opposite
      // corners — cyan through one half of the perimeter, magenta through the
      // other — with the tube glowing rather than a line being drawn.
      //
      // It is built from the property already proven on the probe: two arcs,
      // each trimmed to half the path (0-50% and 50-100%) in different palette
      // colours. Together they close the rectangle. No new capability, no new
      // risk — the same stroke trimming that makes the travelling light work.
      //
      // This is a STANDING frame, not a travelling one: no stroke_offset, so it
      // sits there and burns. The running light from above stays on top of it,
      // which is what gives the reference its highlight sweeping round a tube
      // that is already lit.
      if (neonExtras && neonMul >= 1.5) {
        const halves = [[0, 50], [50, 100]];
        const standing = (a0, a1, hi, width, opacity) => ({
          type: 'shape', path,
          x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
          width: '100%', height: '100%',
          fill_color: 'rgba(0,0,0,0)',
          stroke_color: hexOf(hi), stroke_width: width,
          stroke_cap: 'round', stroke_join: 'round', blend_mode: 'screen',
          stroke_start: `${a0}%`, stroke_end: `${a1}%`,
          // Strikes on rather than fading up, and holds for the shot.
          opacity: [
            { time: 0, value: '0%', easing: 'quadratic-out' },
            { time: +(life * 0.10).toFixed(3), value: `${opacity}%`, easing: 'linear' },
            { time: +(life * 0.86).toFixed(3), value: `${opacity}%`, easing: 'quadratic-in' },
            { time: life, value: '0%' },
          ],
        });
        boxes.forEach((box) => halves.forEach(([a0, a1], hi) => {
          box.elements.push(standing(a0, a1, hi, px(0.020), Math.round(neonPct * 0.20)));
          box.elements.push(standing(a0, a1, hi, px(0.009), Math.round(neonPct * 0.42)));
          box.elements.push(standing(a0, a1, hi, px(0.0035), Math.round(neonPct * 0.9)));
        }));
      }

      // ---- ECHO FRAMES, from 200% up ---------------------------------------
      // The offset second and third rectangles standing off the picture in the
      // references (and, pushed further, the receding tunnel in the third).
      // Larger than the shot's own box on purpose, so they sit OUTSIDE the
      // photograph — which is where they are in every reference.
      //
      // WORTH KNOWING, and said in the UI too: on a green-screen montage the
      // area they sit in IS the chroma backdrop, so these echoes key away with
      // it. They are for montages finished over a dark or imported background,
      // which is what the references all are.
      if (neonExtras && neonMul >= 2) {
        // ONE echo, two only when it is pushed all the way. Josh on the
        // references: "doesn't need to be as busy as these refs." The stacked
        // rectangles are the busiest thing in them.
        const echoes = Math.min(2, Math.max(1, Math.floor(neonMul) - 1));
        for (let ei = 0; ei < echoes; ei++) {
          const grow = 106 + ei * 7;
          const op = Math.round(neonPct * (0.34 - ei * 0.08));
          if (op <= 0) continue;
          [[0, 50], [50, 100]].forEach(([a0, a1], hi) => {
            boxes[0].elements.push({
              type: 'shape', path,
              x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
              width: `${grow}%`, height: `${grow}%`,
              fill_color: 'rgba(0,0,0,0)',
              stroke_color: hexOf(hi + ei + 1), stroke_width: px(0.004),
              stroke_cap: 'round', stroke_join: 'round', blend_mode: 'screen',
              stroke_start: `${a0}%`, stroke_end: `${a1}%`,
              opacity: [
                { time: 0, value: '0%', easing: 'quadratic-out' },
                { time: +(life * (0.14 + ei * 0.05)).toFixed(3), value: `${op}%`, easing: 'linear' },
                { time: +(life * 0.84).toFixed(3), value: `${op}%`, easing: 'quadratic-in' },
                { time: life, value: '0%' },
              ],
            });
          });
      ...
      // ---- LIGHTS FLASHING, from 150% up -----------------------------------
      // Josh, after the references: "variety of lights just flashing. doesn't
      // need to be as busy as these refs." So this is the sparse read of them —
      // individual tubes striking on and off in different colours and places,
      // rather than the dense field the references fill the frame with.
      //
      // Each is a short bar, given its own angle with z_rotation, its own
      // colour, its own moment. They BLINK: hard on, hold a beat, hard off. No
      // travel, no fade up — a fade would read as a glow swelling, and what he
      // asked for is switching.
      if (neonExtras && neonMul >= 1.5) {
        const nBars = Math.max(2, Math.min(6, Math.round(neonMul * 2)));
        for (let bi = 0; bi < nBars; bi++) {
          const sd = seed + 70 + bi * 7;
          const on = +(life * (0.06 + pol_rand(sd) * 0.82)).toFixed(3);
          const hold = +(0.08 + pol_rand(sd + 1) * 0.22).toFixed(3);
          if (on + hold >= life) continue;
          const bw = 14 + pol_rand(sd + 2) * 30;          // % of the picture wide
          const bx = 8 + pol_rand(sd + 3) * 84;
          const by = 8 + pol_rand(sd + 4) * 84;
          const rot = Math.round(pol_rand(sd + 5) * 180);
          const op = Math.min(100, Math.round(neonPct * 0.9));
          const bar = (thick, o) => ({
            type: 'shape', path: 'M 0 50 L 100 50',
            x: `${bx.toFixed(2)}%`, y: `${by.toFixed(2)}%`,
            x_anchor: '50%', y_anchor: '50%',
            width: `${bw.toFixed(2)}%`, height: '2%',
            z_rotation: `${rot}\u00B0`,   // \u00B0, never "deg" — see the note on the dust overlay
            fill_color: 'rgba(0,0,0,0)',
            stroke_color: hexOf(bi), stroke_width: thick,
            stroke_cap: 'round', blend_mode: 'screen',
            opacity: [
              { time: 0, value: '0%', easing: 'linear' },
              { time: on, value: '0%', easing: 'linear' },
              { time: +(on + 0.02).toFixed(3), value: `${o}%`, easing: 'linear' },
              { time: +(on + hold).toFixed(3), value: `${o}%`, easing: 'linear' },
              { time: +(on + hold + 0.02).toFixed(3), value: '0%', easing: 'linear' },
              { time: life, value: '0%' },
            ],
          });
          boxes[0].elements.push(bar(px(0.012), Math.round(op * 0.3)));
          boxes[0].elements.push(bar(px(0.004), op));
        }
      }
      ...
      if (neonExtras && neonMul >= 1.5) {
        ...
        for (let fi = 0; fi < nFlash; fi++) {
          ...
          const peak = Math.min(46, Math.round(17 * neonMul));
          boxes[0].elements.push({
            type: 'shape', path: 'M 0 0 L 100 0 L 100 100 L 0 100 Z',
            x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
            width: '100%', height: '100%',
  ...
  if (neonOn && neonPct > 0 && neonExtras && neonMul >= 1.5) {
    // Sparse on purpose. This was up to six per gap, which with the frames and
    // the flashing lights was the "busy" Josh pushed back on.
    const perGap = Math.max(1, Math.min(3, Math.round((neonMul - 1) * 1.5)));
...
// ---- VIDEO GAPS ON EVERY STYLE -----------------------------------------------
//
// The shared slideshow draws a client's video as a keyable full-frame card at
// its place in the timeline (VideoGap-N): Josh keys his real clip into it and
// uses the card to get in and out of the clip. Every OTHER builder — Two
// Panel, Multi Page, the slide family, Glass, Framed Box, the collages, Stills —
// filtered the placeholder out before laying photos, so on those styles the
// video silently vanished from the export. Josh 2026-09-11: "I am not seeing
// the green/magenta place holders for videos on the preview export."
//
// Rather than teach eighteen builders about a slot they were never designed
// around, the sequence is CUT at each video: each run of photos is built by the
// style's own builder exactly as it would be on its own, the runs are laid end
// to end, and the same keyable card goes in each cut. A card in the middle of a
// Two Panel pair or a ribbon would break the layout — this way a run ends where
// the video starts, and the style begins again after it with its own entrance.
//
// What each run keeps: the style's title cards only on the first and last run
// (the route injects the key-colour bookends as items, so those land at the
// true head and tail too), the background and watermark per run, and length
// mode shared out by photo count so the whole still lands on totalSeconds.
// Everything time-based in a run's top-level elements is resolved with the
// same track arithmetic the overlays use, then shifted — so nothing depends on
// auto-sequencing across the cut.
const VIDEO_GAP_STYLES_HANDLED_INLINE = (S) => !usesOwnBuilder(S);
function usesOwnBuilder(S) {
  return !!((S.duotone && S.frantic) || S.twoPanel || S.multipage || S.polaroid || S.collage || S.epic || S.story
    || S.trendy || S.stills || S.framedBox || S.basicCut || S.slidePush || S.slideRail || S.multiSlide || S.glass
    || S.ribbon || S.neon || S.comic);
}
function buildWithVideoGaps(opts = {}) {
  const S = STYLES[opts.style] || STYLES.hollywood;
  const seq = Array.isArray(opts.items) && opts.items.length ? opts.items : null;
  const hasGap = !!(seq && seq.some((it) => it && it.type === 'placeholder'));
  if (!hasGap || VIDEO_GAP_STYLES_HANDLED_INLINE(S)) return buildMontageSourceInner(opts);

  // Runs of non-placeholder items; consecutive videos collapse to one gap that
  // carries both names.
  const runs = [];
  let cur = null;
  for (const it of seq) {
    if (it && it.type === 'placeholder') {
      if (cur && cur.items.length) { cur.after.push(it.name || 'VIDEO'); }
      continue;
    }
    if (!cur || cur.after.length) { cur = { items: [], after: [] }; runs.push(cur); }
    cur.items.push(it);
  }
  const live = runs.filter((r) => r.items.length);
  if (live.length < 2) return buildMontageSourceInner({ ...opts, items: live[0] ? live[0].items : seq.filter((it) => it.type !== 'placeholder') });

  const W = opts.width || 1920, H = opts.height || 1080;
  const nPhotos = Math.max(1, seq.filter((it) => it && it.type !== 'placeholder' && !it.green).length);
  const total = opts.totalSeconds && Number(opts.totalSeconds) > 0 ? Number(opts.totalSeconds) : null;
  const perPhoto = total ? total / nPhotos : null;
  const includeCards = opts.includeCards !== false;

  const out = { output_format: 'mp4', width: W, height: H, frame_rate: 30, elements: [] };
  let t = 0;
  live.forEach((run, ri) => {
    const first = ri === 0, last = ri === live.length - 1;
    const runPhotos = Math.max(1, run.items.filter((it) => !it.green).length);
    const sub = buildMontageSourceInner({
      ...opts,
      items: run.items,
      // Cards: opening on the first run, closing on the last. The builders make
      // both or neither, so a middle run gets none and the first/last runs get
      // both and have the wrong one removed below by position.
      includeCards: includeCards && (first || last),
      totalSeconds: perPhoto ? perPhoto * runPhotos + (includeCards && (first || last) ? 2 * CARD_S : 0) : null,
    });
    if (!sub || !Array.isArray(sub.elements)) return;
    const spans = ovlResolveTimes(sub.elements);
    let runEnd = 0;
    spans.forEach(([, e]) => { runEnd = Math.max(runEnd, e); });
    // Drop the card this run should not have: the OPENING card on a later run,
    // the CLOSING card on an earlier run. Cards are the shared card() element;
    // they are recognised by name and by sitting at the very start or end.
    const isCard = (el) => /^(Opening|Closing)/i.test(el.name || '');
    const keep = [];
    let head = 0, tail = runEnd;
    for (const el of sub.elements) {
      const sp = spans.get(el);
      if (isCard(el) && sp) {
        const atStart = sp[0] <= 0.01, atEnd = sp[1] >= runEnd - 0.01;
        // Trim the run to the card that was removed, so the next run follows
        // the last photo directly. Only when a card really was there — Two
        // Panel and Multi Page never draw cards, and must not lose 4s of
        // pictures to a trim for a card that does not exist.
        if (atStart && !first) { head = Math.max(head, sp[1]); continue; }
        if (atEnd && !last) { tail = Math.min(tail, sp[0]); continue; }
      }
      keep.push(el);
    }
    for (const el of keep) {
      const sp = spans.get(el);
      if (!sp) { out.elements.push(el); continue; }
      const start = sp[0], end = sp[1];
      const nStart = Math.max(head, start), nEnd = Math.min(tail, end);
      if (nEnd - nStart <= 0.01) continue;
      const copy = { ...el, time: +(t + (nStart - head)).toFixed(3), duration: +(nEnd - nStart).toFixed(3) };
      // An element that started before the trimmed head (a run-long background
      // or watermark) keeps its keyframes as they were — they are relative to the
      // element's own start, so a trim at the head would shift them. Only the
      // background/watermark span the whole run and neither is keyframed.
      out.elements.push(copy);
    }
    t += (tail - head);
    if (!last) {
      const names = run.after.length ? run.after.join(' · ') : 'VIDEO';
      // Same card the slideshow draws: full-frame key colour, the clip's name
      // shown only while the frame is fully key colour, so no text ever sits on
      // a transition frame. applyKeyColor turns CHROMA_GREEN into the chosen key.
      const holdText = Math.max(0.4, PLACEHOLDER_S - 0.6);
      out.elements.push({
        name: `VideoGap-${ri + 1}`, type: 'composition', track: 2, time: +t.toFixed(3), duration: PLACEHOLDER_S,
        elements: [
          { type: 'shape', track: 1, path: 'M 0 0 L 100 0 L 100 100 L 0 100 Z', width: '100%', height: '100%', fill_color: CHROMA_GREEN },
          {
            type: 'text', track: 2, time: 0.3, duration: holdText,
            text: names, font_family: S.font || 'Montserrat', font_weight: '700',
            font_size: `${Math.round(H * 0.05)} px`, fill_color: '#FFFFFF',
            x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%', x_alignment: '50%',
            opacity: [{ time: 0, value: '0%' }, { time: 0.2, value: '100%' }, { time: Math.max(0.2, holdText - 0.2), value: '100%' }, { time: holdText, value: '0%' }],
          },
        ],
      });
      t += PLACEHOLDER_S;
...
export function buildMontageSource(opts = {}) {
  const src = applyKeyColor(buildWithVideoGaps(opts), opts.keyColor);
