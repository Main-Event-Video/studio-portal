...
function framedBoxSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  items.forEach((it, i) => {
    ...
    // Its travelling size is the same shape, a little smaller, so the forward
    // move still has somewhere to go.
    const travelW = tall ? endBox.w * 0.86 : BW[0];
    const travelH = tall ? endBox.h * 0.86 : BH[0];
    ...
    const wSeq = [travelW, travelW, tall ? endW * 0.9 : BW[2], endW].map((v) => v * 100);
    const hSeq = [travelH, travelH, tall ? endH * 0.9 : BH[2], endH].map((v) => v * 100);
    ...
    // The whole scene slides in over the previous shot. No dissolve.
