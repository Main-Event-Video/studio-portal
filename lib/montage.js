...
function epicVintageSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, assetBase, perPhoto = null }) {
  ...
  const D = BEAT * 1.4;                                                          // each unit lives a bit past the next cut
  const cardS = includeCards ? CARD_S : 0;
  ...
  const fadedCard = (url, x, y, w, tilt) => ({
    ...
    elements: [
      ...
      { type: 'image', source: url, x: '50%', y: '50%', width: '94%', height: '90%', x_anchor: '50%', y_anchor: '50%', fit: 'cover', blur_radius: 16, blur_mode: 'stack', color_filter: 'grayscale', color_filter_value: '72%', opacity: '90%' },
    ],
  });
  ...
  photos.forEach((it, i) => {
    ...
    let ph = 52, pw = ph * A * HW;
    if (pw > 66) { pw = 66; ph = pw / (A * HW); }
    if (ph > 72) { ph = 72; pw = ph * A * HW; }
    ...
    const unit = {
      ...
      x_scale: [{ time: 0, value: '230%', easing: 'quadratic-out' }, { time: kLand, value: '104%' }, { time: kDwell, value: '100%' }, { time: kL1, value: '55%' }, { time: kL2, value: '30%' }, { time: D, value: '14%' }],
      y_scale: [{ time: 0, value: '230%', easing: 'quadratic-out' }, { time: kLand, value: '104%' }, { time: kDwell, value: '100%' }, { time: kL1, value: '55%' }, { time: kL2, value: '30%' }, { time: D, value: '14%' }],
      ...
      elements: [
        fadedCard(it.url, 22, 26, 150, -8), fadedCard(it.url, 78, 24, 154, 7),
        fadedCard(it.url, 24, 76, 154, 6), fadedCard(it.url, 78, 78, 150, -6),
        fadedCard(it.url, 70, 16, 50, 9), fadedCard(it.url, 22, 82, 52, 8),
        { type: 'shape', x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', path: rect, fill_color: '#2a1a0a', blend_mode: 'multiply', opacity: '20%' },
        {
  ...
  elements.push({ name: 'Warm', type: 'shape', track: 60, time: bStart, duration: bDur, path: rect, width: '100%', height: '100%', fill_color: S.warm || '#C9865A', opacity: '18%', blend_mode: 'multiply' });
  ...
  elements.push({ name: 'Vignette', type: 'shape', track: 60, time: bStart, duration: bDur, path: rect, width: '150%', height: '150%', x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%', fill_color: '#000000', opacity: '26%', blend_mode: 'multiply', blur_radius: 70, blur_mode: 'stack' });
  if (assetBase) {
