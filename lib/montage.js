  ...
  gaps.forEach(([t0, t1], gi) => {
    ...
    if (neonOn && neonPct > 0) {
      ...
      for (let k = 0; k < N; k++) {
        ...
        const start = +(pol_rand(seed + 1)).toFixed(3);
        ...
        const arc = (width, opacity) => ({
          name: `OvlNeon-${gi + 1}-${k + 1}${opacity === neonPct ? '' : `-h${width}`}`,
        ...
        add.push(arc(px(0.026), Math.round(neonPct * 0.26)));
        add.push(arc(px(0.012), Math.round(neonPct * 0.5)));
        add.push(arc(px(0.005), neonPct));
      }
    }
  });
