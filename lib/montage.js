...
function photoFramingBias(it) {
  ...
  let v = 0, h = 0.5;
  if (typeof fr === 'number' && isFinite(fr)) v = Math.min(1, Math.max(0, fr / 100));
...
// only the box ASPECT, so any resolution works. Needs it.w/it.h (probed dims);
// falls back to landscape if missing.
function coverBox(it, boxWpx, boxHpx) {
  const ar = (it && it.w > 0 && it.h > 0) ? it.w / it.h : 1.5;
  const boxAspect = boxHpx > 0 ? boxWpx / boxHpx : 1.5;
  const { v: vBias, h: hBias } = photoFramingBias(it);
  if (ar <= boxAspect) {
    // taller than box → fill width, crop top/bottom by vBias
    const H = Math.max(100, (boxWpx / ar) / boxHpx * 100);
    const y = 50 + ((H - 100) / 2) * (1 - 2 * vBias);
    return { x: '50%', y: `${y.toFixed(1)}%`, x_anchor: '50%', y_anchor: '50%', width: '100%', height: `${H.toFixed(1)}%`, fit: 'cover' };
  }
  // wider than box → fill height, crop sides by hBias
  const W = Math.max(100, (boxHpx * ar) / boxWpx * 100);
  const x = 50 + ((W - 100) / 2) * (1 - 2 * hBias);
  return { x: `${x.toFixed(1)}%`, y: '50%', x_anchor: '50%', y_anchor: '50%', width: `${W.toFixed(1)}%`, height: '100%', fit: 'cover' };
}
...
export function multiPageLayout({ items, width = 1920, height = 1080 } = {}) {
  const AREA = ['a', 'b', 'c', 'd'];
...
function multiPageSource({ S, seq, watermarkUrl, width, height, mp = {} }) {
  ...
  const framingBias = (it) => {
    const fr = it && it.framing;
    let v = 0, h = 0.5;                                       // default: top vertically, centered horizontally
    if (typeof fr === 'number' && isFinite(fr)) v = Math.min(1, Math.max(0, fr / 100));
    else if (fr === 'center') { v = 0.5; }
    else if (fr === 'bottom') { v = 1; }
    else if (fr === 'top') { v = 0; }
  ...
  const headSafe = (it, c) => {
    ...
    const { v: vBias, h: hBias } = framingBias(it);
    if (ar <= cellAr) {
      // taller-than-cell: full width, real proportional height (element matches the
      // photo → 'cover' can't re-crop), clip the overflow. vBias slides which slice
      // shows: 0 = top (heads), 1 = bottom. Centre-anchored so the math is symmetric.
      const hPct = cellHpx > 0 ? (cellWpx / ar) / cellHpx * 100 : 100;
      const H = Math.max(100, hPct);
      const y = 50 + ((H - 100) / 2) * (1 - 2 * vBias);       // anchor-centre offset in % of cell height
      return { x: '50%', y: `${y.toFixed(1)}%`, x_anchor: '50%', y_anchor: '50%', width: '100%', height: `${H.toFixed(1)}%`, fit: 'cover' };
    }
    // wider-than-cell: full height, real proportional width, clip the sides. hBias
    // slides which slice shows: 0 = left, .5 = centre, 1 = right.
    const wPct = cellWpx > 0 ? (cellHpx * ar) / cellWpx * 100 : 100;
    const W = Math.max(100, wPct);
    const x = 50 + ((W - 100) / 2) * (1 - 2 * hBias);
    return { x: `${x.toFixed(1)}%`, y: '50%', x_anchor: '50%', y_anchor: '50%', width: `${W.toFixed(1)}%`, height: '100%', fit: 'cover' };
  };
