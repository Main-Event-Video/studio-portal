...
function glassDrift(el, k, dur, isHero) {
  const dir = k % 2 ? -1 : 1;
  if (isHero) {
    el.x_scale = [{ time: 0, value: '100%', easing: GLASS_EASE }, { time: dur, value: '101.6%' }];
    el.y_scale = [{ time: 0, value: '100%', easing: GLASS_EASE }, { time: dur, value: '101.6%' }];
    return el;
  }
...
function glassSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null }) {
  const items = seq.filter((it) => it.type !== 'placeholder');
  ...
  const elements = slideFamilyShell({ S, background, total, includeCards, title, subtitle, watermarkUrl, H });
  glassBeams(S, total, W, H, 40).forEach((b) => elements.push(b));
  ...
  items.forEach((it, i) => {
    ...
    if (it.green) {
      elements.push({
        name: `Green-${i + 1}`, type: 'composition', track: 2, time: +t0.toFixed(3), duration: +step.toFixed(3),
        ...(i > 0 ? { animations: transitionIn(S, i) } : {}),
    ...
    L.panes.filter((p) => p.behind).forEach((p, k) => inner.push(glassDrift(glassPane(p, it, W, H, S), k, step, false)));
    inner.push(glassDrift(glassPane(L.hero, it, W, H, S), 0, step, true));
    L.panes.filter((p) => !p.behind).forEach((p, k) => inner.push(glassDrift(glassPane(p, it, W, H, S), k + 1, step, false)));

    elements.push({
      name: `Glass-${i + 1}`, type: 'composition', track: 2,
...
export function buildMontageSource({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, totalSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true, assetBase = null, mpTransition = null, mpStagger = null, mpHold = null, mpSpeed = null, duoPalette = null, duoTreatment: duoTreat = null }) {
  ...
  if (S.glass) return finish(glassSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background, perPhoto }));
