...
function photoFramingBias(it) {
  ...
  if (fr && typeof fr === 'object') {
    ...
    if (isFinite(fr.z)) z = Math.min(3, Math.max(1, fr.z / 100));
...
function coverBox(it, boxWpx, boxHpx) {
  ...
  W = Math.max(100, W * z); H = Math.max(100, H * z);                   // zoom crops in
...
export function buildMontageSource({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, totalSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true, assetBase = null, mpTransition = null, mpStagger = null, mpHold = null, mpSpeed = null }) {
  ...
  gseq.forEach((it, i) => {
    ...
    if (S.pan && eff === 'fill') {
      ...
      const { v: pv } = photoFramingBias(it);          // honor the Fix-framing up/down (object, number or preset)
      const yA = `${Math.round(pv * 100)}%`;           // 0% top … 100% bottom
      photoEl = {
