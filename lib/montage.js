...
export function applyOverlays(source, {
  ...
  const neonPct = Math.min(100, Math.round(70 * pc(neon && neon.intensity, 100)));
  ...
  gaps.forEach(([t0, t1], gi) => {
    ...
    if (neonOn && neonPct > 0) {
      // SQUIGGLES. Short arcs of a big rounded outline, drawn well outside the
      // frame so only a piece of each crosses it, then trimmed with
      // stroke_start/stroke_end and travelled with stroke_offset — the same path
      // trimming Neon Frame uses, which the SDK confirms (stroke_start /
      // stroke_end / stroke_offset are "relative to its total length").
      //
      // NOT BLURRED. A stroked shape carrying blur_radius renders as NOTHING in
      // Creatomate — measured 139 against a 137 wall on the Glass probe. The
      // glow is a stack of crisp strokes at falling opacity instead, which is
      // how Neon Frame was fixed after the same trap.
      //
      // Deterministic: pol_rand, never Math.random, because Export Full Rez
      // rebuilds from the snapshot and must land on the same picture.
      // DENSITY scales with the span. Three arcs across a whole montage reads as
      // the odd accident, not "squiggly animations throughout the screen".
      // Roughly one every two and a half seconds, floor 3, ceiling 10.
      const N = Math.max(3, Math.min(10, Math.round(span / 2.5)));
      for (let k = 0; k < N; k++) {
        const seed = gi * 7 + k * 13;
        // THE RING MUST BE SMALLER THAN THE FRAME. This is what made two
        // renders come back empty, and it is arithmetic, not Creatomate.
        //
        // The path is a rounded RECTANGLE drawn on the edge of its own box. A
        // box at 128% — or at 104%, which was my "fix" — is centred on the
        // frame and therefore has ALL FOUR of its sides beyond the frame edges:
        // at 104% the outline sits at -2% and 102% in both axes, so not one
        // pixel of it is ever inside the picture. Every arc was rendering
        // perfectly, off screen. The neon probe drew because its rings were
        // 14% x 46% — well inside.
        //
        // Neon Frame gets away with 100% because its ring is sized to the
        // PHOTO's box, which is itself smaller than the frame. A full-frame
        // overlay has no such box, so the ring has to be explicitly small and
        // scattered instead — which also gives the squiggles somewhere to be
        // other than the border of the picture.
        const scale = 22 + Math.round(pol_rand(seed) * 48);          // 22-70% of frame
        const cx = 18 + pol_rand(seed + 8) * 64;                     // 18-82%
        const cy = 18 + pol_rand(seed + 9) * 64;
        // stroke_start / stroke_end are each REQUIRED to be 0-100. Creatomate
        // rejects the whole render otherwise:
        //   "OvlNeon-1-4.stroke_end: Expected a value between 0 and 100".
        // The start used the full 0-1 range and the arc length was ADDED to it,
        // so any start above about 0.72 pushed the end past 100 — five arcs in
        // thirty, which is why it took until a seed that produced one to show
        // up. The path is a closed loop, so a trim that wraps past the end is a
        // perfectly sensible thing to want; Creatomate simply does not accept
        // it. Pick the start from the room that is actually left.
        const len = 0.12 + pol_rand(seed + 2) * 0.16;
        const start = +(pol_rand(seed + 1) * (1 - len)).toFixed(3);
        const dir = pol_rand(seed + 3) > 0.5 ? 1 : -1;
        const delay = +(span * (0.08 + pol_rand(seed + 4) * 0.55)).toFixed(3);
        const life = +Math.min(span - delay, 1.6 + pol_rand(seed + 5) * 1.4).toFixed(3);
        if (life < 0.4) continue;
        const path = roundedRect(6 + pol_rand(seed + 6) * 10, 6 + pol_rand(seed + 7) * 10);
        // Deterministic cycle: arc index across the whole montage, so two arcs
        // alive at once are never the same colour when more than one is chosen.
        const neonHex = neonPalette[(gi * N + k) % neonPalette.length];
        const arc = (width, opacity, layer) => ({
          // Layer index, not the stroke width — the width carries a space and a
          // unit ("28 px"), which made element names like "OvlNeon-1-4-h28 px".
          name: `OvlNeon-${gi + 1}-${k + 1}-L${layer}`,
          type: 'shape', path, track: OVL_TRACK_NEON,
          time: +(at + delay).toFixed(3), duration: life,
          x: `${cx.toFixed(2)}%`, y: `${cy.toFixed(2)}%`,
          width: `${scale}%`, height: `${scale}%`,
          x_anchor: '50%', y_anchor: '50%',
          fill_color: 'rgba(0,0,0,0)', stroke_color: neonHex, stroke_width: width,
          stroke_cap: 'round', stroke_join: 'round', blend_mode: 'screen',
          stroke_start: `${(start * 100).toFixed(2)}%`,
          stroke_end: `${((start + len) * 100).toFixed(2)}%`,
          stroke_offset: [
            { time: 0, value: '0%', easing: 'linear' },
            { time: life, value: `${(dir * 26).toFixed(0)}%` },
          ],
          opacity: [
            { time: 0, value: '0%', easing: 'quadratic-out' },
            { time: +(life * 0.22).toFixed(3), value: `${opacity}%`, easing: 'linear' },
            { time: +(life * 0.68).toFixed(3), value: `${opacity}%`, easing: 'quadratic-in' },
            { time: life, value: '0%' },
          ],
        });
        // Widths in PIXELS, not vmin. Neon Frame uses vmin and has never been
        // confirmed to render its glow either — one unproven unit under two
        // unproven effects is a bad bet, and px is what the frame stroke uses
        // in Framed Box, which IS render-proven ("border test was a success").
        const px = (v) => `${Math.max(1, Math.round(Math.min(W, H) * v))} px`;
        add.push(arc(px(0.026), Math.round(neonPct * 0.26), 1));
        add.push(arc(px(0.012), Math.round(neonPct * 0.5), 2));
        add.push(arc(px(0.005), neonPct, 3));
      }
    }
  });
