...
function bgLayers(background, S, { track = 1, time = 0, duration }) {
  ...
  if (b.videoUrl) {
    const out = [base('#000000')];
    out.push({
      name: 'Backdrop', type: 'video', source: b.videoUrl, track, time, duration,
      x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%',
      fit: 'cover', loop: true, volume: '0%',
      ...(Number.isFinite(Number(b.trimStart)) && Number(b.trimStart) > 0 ? { trim_start: Number(b.trimStart) } : {}),
...
function fullFrameMotion(S, it, { zoomIn, sizePct, amp, drift }) {
...
function duotoneShot(S, it, photoCount) {
  ...
  els.push({
    ...
    elements: [{ type: 'image', source: url, ...coverBox(it, DUO_HERO.w * 1920, DUO_HERO.h * 1080) }],
  });
  return els;
}
...
function polaroidStackSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background, perPhoto = null }) {
  ...
  if (!isGreen) {
    if (bgVideoUrl) {
      elements.push({ name: 'Backdrop', type: 'video', source: bgVideoUrl, track: 2, time: stackStart,
  ...
  pagesArr.forEach((group, pi) => {
    ...
    keys.forEach((k, idx) => {
      ...
      if (per) {
        ...
        photoEls.push(mpPhotoEnter(cm, c, st, MOVE, FADE, inner));
      } else {
...
function slidingImagesSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null }) {
  ...
  plan.forEach((p, i) => {
    ...
    const el = {
      ...
      elements: [applyPhotoColor({ type: 'image', source: p.it.url, ...coverBox(p.it, p.box.w * W, p.box.h * H) }, p.it)],
    };
    ...
    const el = {
      ...
      elements: [
          ...
          path: roundedRect(1.6, 1.6 * (cw * W) / (ch * H)), fill_color: '#FAF7F0' },
        {
          ...
          elements: [applyPhotoColor({ type: 'image', source: it.url, ...coverBox(it, ph.w * W, ph.h * H) }, it)],
        },
      ],
    };
  ...
  if (photos.length) {
    ...
    const cards = photos.map((it, i) => {
      ...
      const el = {
        ...
        elements: [applyPhotoColor({ type: 'image', source: it.url, ...coverBox(it, b.w * W, b.h * H) }, it)],
      };
      if (!noShadow) { el.shadow_color = '#00000055'; el.shadow_blur = '3vmin'; el.shadow_x = '0vmin'; el.shadow_y = '1vmin'; }
      return el;
    });
    ...
    elements.push({
      ...
      elements: [
        ...
        { type: 'composition', clip: true, width: '100%', height: '100%', x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
          elements: [applyPhotoColor({ type: 'image', source: it.url, ...coverBox(it, box.w * W, box.h * H) }, it)] },
...
function twoPanelSource({ S, seq, watermarkUrl, width, height, mp = {} }) {
  ...
  cards.forEach((group, ci) => {
    ...
    const photoEls = placed.map((pl, idx) => {
      ...
      const inner = applyPhotoColor({ type: 'image', source: pl.it.url, ...coverBox(pl.it, (c.w / 100) * width, (c.h / 100) * height) }, pl.it);
...
export function buildMontageSource({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, totalSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true, assetBase = null, mpTransition = null, mpStagger = null, mpHold = null, mpSpeed = null }) {
  ...
  gseq.forEach((it, i) => {
    ...
    if (S.pan && eff === 'fill') {
      ...
      photoEl = fullFrameMotion(S, it, { zoomIn, sizePct, amp, drift: photoCount });
      ...
      photoEl = fullFrameMotion(S, it, { zoomIn, sizePct, amp, drift: null });
    } else if (eff === 'fill') {
  ...
  // ---- Cast-free hold-and-fade (holdFade styles, e.g. Hollywood) ----
  // The default fade transition is a SYMMETRIC cross-dissolve: mid-transition BOTH
  // photos are partly transparent, so the green backdrop shows through and tints
  // the blend — a cast that survives chroma keying. Here we re-lay the same
  // play-sequence elements on two alternating tracks with an asymmetric dissolve:
  // odd items fade IN over the held (opaque) even item beneath, and fade OUT over
  // the next held even item. The image area therefore only ever blends image→image
  // (no green cast); Fit margins stay green (still keyable).
  //
  // OPT-IN, and ONLY for styles whose transition is a pure fade/dissolve (set
  // `holdFade: true` on the style — currently Hollywood + Timeless). This replaces
  // the transition with a clean cross-dissolve, which is the SAME look those styles
  // already intended — just cast-free. Do NOT enable it on styles that use
  // slide / wipe / scale movement (Party, Party 2, Duotone…): the dissolve would
  // flatten their movement. Those keep their original transitions (the green bleed
  // there is a separate, per-transition-type problem — see notes to Josh).
  if (S.holdFade) {
    ...
    seqEls.forEach((el, k) => {
      ...
      el.time = starts[k];
      el.track = 3 + (k % 2);               // 3 = held layer, 4 = fading layer (always above 3)
      const dur = el.duration || photoS;
      const isLast = k === seqEls.length - 1;
      if (k % 2 === 1) {
        // Odd item sits on top (track 4): fade IN over the previous held item; and
        // fade OUT over the next held item (unless it's the very last element).
        const kf = [{ time: 0, value: '0%' }, { time: fadeS, value: '100%' }];
        if (!isLast) kf.push({ time: Math.max(fadeS, dur - fadeS), value: '100%' }, { time: dur, value: '0%' });
        el.opacity = kf;
      }
      // Even items stay fully opaque (held) — their neighbours' fades do the blend.
    });
  ...
  if (includeCards) {
    elements.push({
      ...
      track: 2,
      duration: CARD_S,
      ...(S.holdFade ? { time: Math.max(0, total - CARD_S) } : {}),
