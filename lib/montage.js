...
function glassPane(p, it, W, H, S) {
  ...
  // A pane that stands IN FRONT of the photograph is the only kind that veils
  // it. Measured off render 005: a front blade lifted the dark floor of a photo
  // from 45/255 to 128/255 — a third of the way to white, across a wide band of
  // the picture. That is not glass, that is paint. Front panes get a fraction of
  // the fill so you look THROUGH them; their edge and their shadow carry them.
  const front = !p.behind && !p.hero;
  const veil = front ? 0.40 : 1;
  ...
  if (gentle) {
    ...
    const frost = (p.frost == null ? 0.20 : p.frost) * 1.45 * veil;
    inner.push({
  ...
  // A soft sheen — the thing that stops an EMPTY pane reading as a flat card.
  //
  // It used to go on every pane, the hero included, and that was a mistake I
  // could only see in a real render. Screen-blending white at 0.16/55% over a
  // photograph lifts every pixel about 9% toward white and puts a floor under
  // the blacks: measured on render 005, nothing inside the hero came in below
  // ~22/255, so the picture could never be fully black anywhere. That is a
  // permanent white film on the photograph, and Robyn asked for crisp and
  // clean. A pane holding a picture does not need help reading as a pane — the
  // picture does that. So: sheen on glass, never on a photograph.
  if (!gentle) {
    inner.push({
      type: 'shape', path: GLASS_RECT, width: '100%', height: '100%',
      x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
      fill_color: 'rgba(255,255,255,0.16)', blend_mode: 'screen',
      opacity: `${Math.round(55 * veil)}%`,
    });
  }
...
function glassSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null, light = true }) {
  ...
  const shotEl = (i, name, inner) => {
    ...
    const el = {
      ...
      duration: +((last ? step : step + fade)).toFixed(3),
      elements: inner,
    };
    if (odd) {
      const kf = [
      ...
      if (!last) kf.push(
        { time: +step.toFixed(3), value: '100%', easing: 'linear' },
