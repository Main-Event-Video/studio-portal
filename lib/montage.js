...
function framedBoxSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  items.forEach((it, i) => {
    ...
    // The box is pinned to the frame edge it grows from, so only its far edge
    // travels — which is exactly what the measurements show.
    const pin = {
    ...
    const wPct = horiz ? traced(SWEEP0 * 100, SWEEP1 * 100) : kf(`${(BOX_W0 * 100).toFixed(2)}%`, `${(BOX_W1 * 100).toFixed(2)}%`);
    const hPct = horiz ? kf(`${(BOX_H0 * 100).toFixed(2)}%`, `${(BOX_H1 * 100).toFixed(2)}%`) : traced(SWEEP0 * 100, SWEEP1 * 100);
    const box = {
      ...
      elements: [
        ...
        applyPhotoColor({ type: 'image', source: it.url, ...coverBox(it, W * BOX_W1, H * BOX_H1) }, it),
  ...
  if (base) {
    ['dust1.png', 'dust2.png'].forEach((d, k) => {
      elements.push({ name: `Dust-${k + 1}`, type: 'image', track: 30 + k, source: `${base}/overlays/${d}`,
        ...
        z_rotation: [{ time: 0, value: '0deg', easing: 'linear' }, { time: bodyD, value: `${k ? -6 : 6}deg` }] });
    });
    items.forEach((it, i) => {
      if (it.green) return;
