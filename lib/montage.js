...
function framedBoxSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  const STROKE = Math.max(6, Math.round(Math.min(W, H) * 0.0112));   // ~12px at 1080, ~6px visible
  // Effects live above every shot track. Shots start at 4 and climb one per
  // photo so each slides in OVER the last, so this must clear all of them.
  const fxBase = 4 + n + 2;
  ...
  items.forEach((it, i) => {
    ...
    const endBox = tall ? nativeBox(it, W, H, 0.95, 1.0) : null;
    const endW = tall ? endBox.w : 1;
    const endH = tall ? endBox.h : 1;

    const at = (k) => +(U[k] * dur).toFixed(3);
    // Josh: "any way to make the movement smoother?" The blow-up used
    // quadratic-in, which starts at zero and only accelerates — it arrives at
    // the frame edge at full speed and stops dead. sinusoid-in-out eases in AND
    // out, so the push begins gently out of the hold and settles rather than
    // slamming. The slide keeps quadratic-out: it is already decelerating into
    // its stop, which is the smooth direction.
    const ease = ['quadratic-out', 'linear', 'sinusoid-in-out', null];
    const kfs = (vals, unit = '%') => vals.map((v, k) => ({
      time: at(k), value: `${v.toFixed(3)}${unit}`,
      ...(ease[k] ? { easing: ease[k] } : {}),
    }));

    // Size across the four keyframes.
    // THE FRAME MUST HUG THE PICTURE. Josh: "The frame is not married to the
    // image edge." On a tall photo the picture is contained inside the box, so
    // the moment the box's aspect drifts from the photograph's the picture
    // letterboxes and the white frame stands off it. BW and BH are different
    // ratios (0.801 vs 0.735), so the box was changing shape all the way
    // through the move and the frame could only touch the picture at the end.
    //
    // A tall photo therefore scales UNIFORMLY — one ratio on both axes — so the
    // box stays exactly the photograph's shape and the frame sits on its edge
    // for every frame. Landscape keeps the measured pair, because there the
    // picture COVERS its box and fills it whatever the box's shape.
    const wSeq = (tall ? BW.map((r, k) => BW[k]) : BW).map((r) => r * endW * 100);
    const hSeq = (tall ? BW : BH).map((r) => r * endH * 100);
    // ONE FLUID MOVE. Josh: "The movement of the hero shot is jerky. stop and
    // go - needs to be one fluid movement with a speed change (as we talked
    // about previously)."
    //
    // The cause was the easing chain on POSITION: quadratic-out decelerated the
    // box to a dead stop at 29%, linear started it again, sinusoid-in-out
    // stopped it a second time at 60%. Three legs, two full stops — velocity hit
    // zero twice in the middle of a move that should never pause. Same mistake
    // as Glass, where an ease-in-out on every keyframe made one continuous
    // travel read as three separate gestures.
    //
    // So position now follows the curve I traced off the reference in the first
    // place — fast at the start, settling to a steady drift, measured samples
    // with linear interpolation between them. Velocity changes continuously and
    // never reaches zero until the shot ends. That is the "speed change" without
    // the stop and go.
    // ...but linear interpolation between eleven samples is itself the jerk.
    // A linear leg holds ONE constant speed, so the hero's velocity was a
    // staircase, and the risers are big: measured off these very samples the
    // speed goes 1.84 -> 1.26 (a 31% drop at 0.10) and 0.76 -> 1.29 (a 71%
    // jump at 0.83), each riser landing as an instantaneous change held for
    // ~0.3s. Ten of those in a three-second move is exactly what reads as
    // "stop and go" — the shape was right, the quantisation was not.
    //
    // Fix: keep the traced shape, resample it. A Fritsch-Carlson monotone
    // cubic through the same eleven points, evaluated at 40 steps, passes
    // through every measured sample (so the reference's timing is untouched)
    // and cannot overshoot or reverse. The largest remaining speed change is
    // ~23% spread over 1/40th of the shot (~75ms at 3s) instead of 71% held
    // for 300ms: continuous acceleration rather than a lurch.
    const FB_CURVE = [[0, 0], [0.10, 0.184], [0.21, 0.323], [0.31, 0.429],
      [0.41, 0.512], [0.52, 0.581], [0.62, 0.645], [0.72, 0.710],
      [0.83, 0.793], [0.93, 0.922], [1, 1]];
    const FB_STEPS = 40;
    const FB_SMOOTH = (() => {
      const xs = FB_CURVE.map((p) => p[0]);
      const ys = FB_CURVE.map((p) => p[1]);
      const n = FB_CURVE.length;
      const h = [];
      const d = [];
      for (let i = 0; i < n - 1; i += 1) { h.push(xs[i + 1] - xs[i]); d.push((ys[i + 1] - ys[i]) / h[i]); }
      const m = new Array(n);
      m[0] = d[0];
      m[n - 1] = d[n - 2];
      ...
      const out = [];
      for (let k = 0; k <= FB_STEPS; k += 1) {
        const u = k / FB_STEPS;
        out.push([u, Math.max(0, Math.min(1, at(u)))]);
      }
      return out;
    })();
    const travelled = (from0, to0) => FB_SMOOTH.map(([u, y], k) => ({
      time: +(u * dur).toFixed(3),
      value: `${(from0 + (to0 - from0) * y).toFixed(3)}%`,
      ...(k < FB_SMOOTH.length - 1 ? { easing: 'linear' } : {}),
    }));
    const startC = (back ? (1 - CX[0]) : CX[0]) * 100;
    const cSeq = travelled(startC, 50);
    const midSeq = '50%';

    const box = {
      type: 'composition', clip: true,
      width: kfs(wSeq), height: kfs(hSeq),
      x: horiz ? cSeq : midSeq, y: horiz ? midSeq : cSeq,
      x_anchor: '50%', y_anchor: '50%',
      elements: [
    ...
    // and grows.
    const inS = +(dur * 0.45).toFixed(3);
    // 8% of the frame, comfortably inside the 11%-per-side overscan.
    const bedPan = (back ? 1 : -1) * 8;
    // Josh: "the first frame entrance is wrong (doesn't happen)." It did not —
    // the first shot was pinned. Every shot enters now; the first one slides in
    // over the background rather than over a previous picture, which is the only
    // difference.
    // The bed arrives by dissolve. Short, so it is under the box quickly.
    const bedIn = i === 0 ? '100%' : [
      { time: 0, value: '0%', easing: 'linear' },
      { time: +(dur * 0.22).toFixed(3), value: '100%' },
    ];
    ...
    elements.push({
      ...
      time: t0, duration: +(i === items.length - 1 ? dur : dur + dur * U[1]).toFixed(3),
      width: '100%', height: '100%',
      x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
      elements: [
        ...
        { type: 'composition', width: '100%', height: '100%', clip: true,
          x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%', opacity: bedIn,
          // THE BED HOLDS STILL. Josh: "you can see the image blurred behind it
          // lined up on top of itself." It only lines up if it does not drift:
          // a bed creeping 114% -> 126% while the hero holds pulls the two out
          // of register over the shot, which is what stopped it reading as one
          // photograph behind itself. Both are centred crops of the same image,
          // so a fixed magnification keeps them aligned by construction.
          x_scale: '122%', y_scale: '122%',
          elements: [(() => {
            const cb = coverBox(bedShot(it, W, H), W, H);
            // Animate AROUND whatever the face-centred crop chose, so the pan
            // adds to that framing instead of replacing it.
            const base = (v) => (parseFloat(v) || 50);
            const pan = (v) => ([
              { time: 0, value: `${(base(v) + bedPan).toFixed(3)}%`, easing: 'quadratic-out' },
              { time: inS, value: `${base(v).toFixed(3)}%` },
            ]);
            return applyPhotoColor({
              type: 'image', source: it.url, ...cb,
              ...(horiz ? { x: pan(cb.x) } : { y: pan(cb.y) }),
              blur_radius: Math.round(Math.min(W, H) * 0.030), blur_mode: 'stack',
            }, it);
          })()] },
        // The darkening rides the bed's fade, or it would multiply over the
        // PREVIOUS shot's picture during the dissolve and darken it.
        { type: 'shape', path: rect, width: '100%', height: '100%',
          x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
          fill_color: '#0B0B10', blend_mode: 'multiply',
          opacity: i === 0 ? '22%' : [
            { time: 0, value: '0%', easing: 'linear' },
            { time: +(dur * 0.22).toFixed(3), value: '22%' },
          ] },
        box,
      ],
    });
  ...
  if (base) {
    ['dust1.png', 'dust2.png'].forEach((d, k) => {
      elements.push({ name: `Dust-${k + 1}`, type: 'image', track: fxBase + k, source: `${base}/overlays/${d}`,
    ...
    items.forEach((it, i) => {
      if (it.green) return;
      const at2 = +(cardS + i * step).toFixed(3);
      const lk = +Math.min(step, 1.9).toFixed(3);
      elements.push({ name: `Leak-${i + 1}`, type: 'image', track: fxBase + 2,
        source: `${base}/overlays/leak_epic${(i % 3) + 1}.png`,
        time: at2, duration: lk,
        width: '150%', height: '150%', x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
        blend_mode: 'screen',
        opacity: [{ time: 0, value: '0%', easing: 'quadratic-out' },
          { time: +(lk * 0.45).toFixed(3), value: '34%', easing: 'quadratic-in' },
          { time: lk, value: '0%' }] });
    });
  }
