...
export function buildMontageSource({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, includeCards = true, width = 1920, height = 1080 }) {
  ...
  seq.forEach((it, i) => {
    ...
    elements.push({
      ...
      // Framing anchor only bites on cover-crop: with contain the whole photo is
      // visible, so shifting it would only push it into the letterbox bars.
      ...(!isFit ? (FRAMING[it.framing] || {}) : {}),
      duration: S.photoS,
