...
function framedBoxSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  const BW = [0.801, 0.801, 0.826, 1.00];    // box width
  ...
  const BH = [0.735, 0.735, 0.757, 1.00];    // box height
  // Centre of the box along the axis it travels, measured for a rightward move.
  const CX = [0.2655, 0.4035, 0.4660, 0.50];
  // Josh: "All images need to have a thicker border." Was ~3px at 1080.
  const STROKE = Math.max(3, Math.round(Math.min(W, H) * 0.0056));   // ~6px at 1080
  ...
  items.forEach((it, i) => {
    ...
    const endW = tall ? endBox.w : BW[3];
    const endH = tall ? endBox.h : BH[3];
    // ITS TRAVELLING SIZE. Measured on render 022, a tall photo went 31.6% ->
    // 36.7% of frame width — a 16% growth on something a third of the frame
    // wide, which is no forward move at all. The reference grows 80.1% -> 100%,
    // a quarter, and ends filling the screen. A vertical picture cannot end
    // full-frame without being cropped, so it gets the growth instead: it now
    // starts at 60% of its final size and arrives whole, which is a 67% push.
    const travelW = tall ? endBox.w * 0.60 : BW[0];
    const travelH = tall ? endBox.h * 0.60 : BH[0];

    const at = (k) => +(U[k] * dur).toFixed(3);
    const ease = ['quadratic-out', 'linear', 'quadratic-in', null];
    ...
    const wSeq = [travelW, travelW, tall ? endW * 0.72 : BW[2], endW].map((v) => v * 100);
    const hSeq = [travelH, travelH, tall ? endH * 0.72 : BH[2], endH].map((v) => v * 100);
    // Centre along the travelling axis: the measured march toward the middle,
    // mirrored for the reverse directions.
    const cSeq = CX.map((c) => (back ? (1 - c) : c) * 100);
    const midSeq = [50, 50, 50, 50];

    const box = {
      type: 'composition', clip: true,
      width: kfs(wSeq), height: kfs(hSeq),
      x: kfs(horiz ? cSeq : midSeq), y: kfs(horiz ? midSeq : cSeq),
      x_anchor: '50%', y_anchor: '50%',
      elements: [
        // The picture is sized to the FINAL box in pixels and held still while
        // the box travels over it, so the box reveals rather than squeezes it.
        // Pixels because a % child scales with its animating parent.
        applyPhotoColor({
    ...
    elements.push({
      ...
      x_anchor: '50%', y_anchor: '50%',
      elements: [
        // the blown-up blurred bed, drifting evenly and against the sweep
        { type: 'composition', width: '100%', height: '100%', clip: true,
          x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
          x_scale: [{ time: 0, value: '114%', easing: 'linear' }, { time: dur, value: '126%' }],
          y_scale: [{ time: 0, value: '114%', easing: 'linear' }, { time: dur, value: '126%' }],
