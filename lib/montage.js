...
export function applyOverlays(source, {
  ...
  if (neonOn && neonPct > 0) {
    ...
    for (const e of source.elements) {
      ...
      const neonHex = neonPalette[shotIdx % neonPalette.length];
      // One light per shot, running most of the way round the picture. Length
      // and start are kept inside 0-100 because Creatomate rejects the whole
      // render otherwise.
      const len = 0.18 + pol_rand(seed + 2) * 0.10;
      const from = +(pol_rand(seed + 1) * (1 - len)).toFixed(3);
      const px = (v) => `${Math.max(1, Math.round(Math.min(W, H) * v))} px`;
      const ring = (width, opacity) => ({
        type: 'shape', path,
        x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
        width: '100%', height: '100%',
        fill_color: 'rgba(0,0,0,0)',
        stroke_color: neonHex, stroke_width: width,
        stroke_cap: 'round', stroke_join: 'round', blend_mode: 'screen',
        stroke_start: `${(from * 100).toFixed(2)}%`,
        stroke_end: `${((from + len) * 100).toFixed(2)}%`,
        // The light travels the perimeter once across the shot. Offset is a
        // share of the path length, so it reads the same on a wide print and a
        // tall one.
        stroke_offset: [
          { time: 0, value: '0%', easing: 'linear' },
          { time: life, value: '100%' },
      ...
      e.elements.push(ring(px(0.022), Math.round(neonPct * 0.24)));
      e.elements.push(ring(px(0.010), Math.round(neonPct * 0.48)));
      e.elements.push(ring(px(0.004), neonPct));
    }
  }
