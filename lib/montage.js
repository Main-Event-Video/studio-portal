...
export const STYLES = {
  ...
  glass: {
    ...
    photoS: 4.6, fadeS: 1.40,   // the prototype's TRANS is 1.15s; 0.9 measured 0.40-0.57s on render 018, about half the reference's 0.83-1.00s
    transitions: ['fade'], zoom: ['100%', '100%'],
    glass: true,
  },
  sliding_images: {
...
function glassSolvePos(p, it, f) {
  ...
  // THE FOREHEAD GUARD. visW/visH are the fractions of the image this pane
  // shows, so a face f.w wide occupies f.w / visW of the pane. Past 62% it is
  // no longer a person in a pane of glass, it is a magnified face — which is
  // exactly what render 017 looked like. Reject this face and let the caller
  // try another one; if none fit, the pane is plain glass, as before.
  if ((f.w > 0 && f.w / visW > GLASS_MAX_FACE)
   || (f.h > 0 && f.h / visH > GLASS_MAX_FACE)) return null;
...
function glassSweep(name, at, dur, W, H) {
  // 019 measured a +27..+34 lift where the prototype reaches roughly double
  // that, so the bar goes to full and widens a little.
  const peak = 100;
