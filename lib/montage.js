...
export function applyOverlays(source, {
  ...
  if (neonOn && neonPct > 0) {
    ...
    for (const e of source.elements) {
      ...
      // picture, which is what it should have been from the start. Capped at
      // six so a wall style cannot turn into a light show.
      const boxes = [];
      const findBoxes = (node, depth) => {
        if (!node || typeof node !== 'object' || boxes.length >= 6 || depth > 6) return;
