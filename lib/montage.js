...
function glassRoom(S, total, W, H) {
  ...
  return [{
    ...
    elements: [
      ...
      //
      // BIGGER AND WEAKER THAN THE FIRST ATTEMPT, for two measured reasons. At
      // 116% x 104% the ellipse's own lower edge fell inside the frame and, even
      // with a 140px blur, showed as a visible crescent across the bottom third
      // of render V4 — a shape in the room that is not part of the room. It now
      // runs well past every edge so there is no edge to see.
      //
      // And at 86% it drove the middle of the wall to 240/255. A white pane rim
      // has nowhere to be brighter than that, which is half of why the glass
      // never read. The prototype's own note says it: "Pure white walls leave no
      // room for white glass edges or white beams to register." At 34% the wall
      // sits around 204 at the corners and 221 in the middle, and white has
      // somewhere to go.
      { type: 'shape', path: 'M 50 0 C 77.6 0 100 22.4 100 50 C 100 77.6 77.6 100 50 100 C 22.4 100 0 77.6 0 50 C 0 22.4 22.4 0 50 0 Z',
        ...
        blur_radius: Math.round(H * 0.10), blur_mode: 'stack', opacity: '34%' },
...
function glassPane(p, it, W, H, S) {
  ...
  const rim = (mult, op) => ({
    ...
    stroke_width: `${Math.max(1, Math.round(edgePx * mult))} px`,
    opacity: `${Math.max(1, Math.round(op * dim))}%`,
  });
  body.push(rim(7, 20));      // the soft outer part of the bloom
  body.push(rim(3.5, 34));    // its core
  body.push(rim(1, 98));      // the hot line itself
  ...
  const wrap = {
    ...
    x_anchor: '50%', y_anchor: '50%',
    elements: [glowOuter, glowInner, bodyComp, lead('left', 96), ...(p.hero ? [lead('right', 67)] : [])],
  };
