...
function polaroidStackSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background, perPhoto = null }) {
  ...
  if (bgUrl) {
    ...
    photosOnly.forEach((it, k) => {
      ...
      const life = (k === n - 1 ? MOVE + HOLD : HOLD) + 0.6;
  ...
  photosOnly.forEach((it, k) => {
    ...
    const print = {
      ...
      x_scale: [{ time: 0, value: '104%', easing: 'quadratic-out' }, { time: MOVE, value: '100%' }],
      y_scale: [{ time: 0, value: '104%', easing: 'quadratic-out' }, { time: MOVE, value: '100%' }],
