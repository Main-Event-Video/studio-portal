...
function multiPageSource({ S, seq, watermarkUrl, width, height }) {
  const rect = 'M 0 0 L 100 0 L 100 100 L 0 100 Z';
  const AREA = ['a', 'b', 'c', 'd'];
  ...
  const STAG = 0.24, FADE = 0.5, HOLD = 1.3, MOVE = 0.72;
  ...
  pagesArr.forEach((group, pi) => {
    ...
    keys.forEach((k, idx) => {
      ...
      if (per) {
        // pivot ON around the cell's bottom-left corner, staggered
        photoEls.push({
          type: 'composition',
          x: `${c.x.toFixed(2)}%`, y: `${(c.y + c.h).toFixed(2)}%`, width: `${c.w.toFixed(2)}%`, height: `${c.h.toFixed(2)}%`,
          ...
          elements: [applyPhotoColor({ type: 'image', source: it.url, x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', fit: 'cover' }, it)],
        });
      } else {
        // reveal in place: scale + fade, staggered
    ...
    if (per) {
      ...
      // the WHOLE page pivots OFF around the frame's bottom-left hub
      const exit = revealDur + HOLD;
      pageComp.x = '0%'; pageComp.y = '100%'; pageComp.x_anchor = '0%'; pageComp.y_anchor = '100%';
      pageComp.z_rotation = [{ time: 0, value: '0°' }, { time: exit, value: '0°', easing: 'quadratic-in' }, { time: pageDur, value: '92°' }];
...
export function buildMontageSource({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, totalSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true, assetBase = null }) {
  ...
  if (S.multipage) return multiPageSource({ S, seq, watermarkUrl, width, height });
