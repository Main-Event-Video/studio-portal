...
function epicVintageSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, assetBase, perPhoto = null }) {
  ...
  for (let k = 1; k < n; k++) {
    ...
    // Soft WARM corner glow: circles centered ON the frame corner so most of each
    // sits off-frame — only a quarter-glow spills inward and fades (radial feel),
    // NOT a hard disc washing the whole frame. Warm, not white.
    // wide amber halo
    elements.push({ name: `Bloom-halo-${k}`, type: 'shape', track: 20, time: t0, duration: dur, path: circle, width: '86%', height: '86%', x: `${c.x}%`, y: `${c.y}%`, x_anchor: '50%', y_anchor: '50%', fill_color: '#FFCF90', blend_mode: 'screen', blur_radius: 70, blur_mode: 'stack', opacity: ramp(64) });
    // warm-white core (contained near the corner)
    elements.push({ name: `Bloom-core-${k}`, type: 'shape', track: 21, time: t0, duration: dur, path: circle, width: '40%', height: '40%', x: `${c.x}%`, y: `${c.y}%`, x_anchor: '50%', y_anchor: '50%', fill_color: '#FFF3DC', blend_mode: 'screen', blur_radius: 40, blur_mode: 'stack', opacity: ramp(80) });
    // faint expanding ring
    elements.push({
      name: `Bloom-ring-${k}`, type: 'shape', track: 22, time: t0, duration: dur, path: circle, x: `${c.x}%`, y: `${c.y}%`, x_anchor: '50%', y_anchor: '50%',
      fill_color: '#FFE6B4', blend_mode: 'screen', blur_radius: 3, blur_mode: 'stack', opacity: ramp(42),
      width: [{ time: 0, value: '16%', easing: 'ease-out' }, { time: dur, value: '40%' }],
      height: [{ time: 0, value: '16%', easing: 'ease-out' }, { time: dur, value: '40%' }],
    });
    // thin light streak across the corner's row
    elements.push({ name: `Bloom-streak-${k}`, type: 'shape', track: 23, time: t0, duration: dur, path: rect, width: '150%', height: '1.6%', x: '50%', y: `${c.y}%`, x_anchor: '50%', y_anchor: '50%', fill_color: '#FFECC8', blend_mode: 'screen', blur_radius: 6, blur_mode: 'stack', opacity: ramp(68) });
  }
