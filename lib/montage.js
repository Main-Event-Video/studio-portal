...
function polaroidStackSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background, perPhoto = null }) {
  ...
  photosOnly.forEach((it, k) => {
    ...
    const print = {
      ...
      x_anchor: '50%', y_anchor: '50%', width: '40%', height: '78%',   // TALL portrait print (was smooshed 46\u00D766)
      ...
      elements: [
        ...
        applyPhotoColor({ type: 'image', source: it.url, width: '88%', height: '74%', fit: 'cover', ...printFraming(it) }, it),
      ],
    };
...
function collageWallSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  // NATIVE SHAPE (Josh): every photo keeps its real proportions — a portrait gets
  // a tall, narrow cell, a landscape a wide one — laid out in JUSTIFIED ROWS (each
  // row a uniform height, cells scaled to fill the row width, same idea as a
  // Google-Photos wall). Photos are PLACED in a shuffled order so that resting on
  // them 1..N in order still sends the camera darting around (haphazard whip
  // preserved); every photo keeps its own focused rest.
  ...
  const ROWH = featured ? 460 : 300;            // base row height in wall px
  const TARGETW = featured ? 2400 : 2600;       // width each row fills
  const G = scatter ? 26 : 22;                   // gap between cells
  const rows = []; let cur = [], sumA = 0;
  place.forEach((pIdx) => {
    const it = photos[pIdx]; const a = aspOf(it);
    cur.push({ pIdx, it, a }); sumA += a;
    if (sumA * ROWH + G * (cur.length - 1) >= TARGETW) { rows.push(cur); cur = []; sumA = 0; }
  });
  ...
  rows.forEach((row, ri) => {
    ...
    if (isLast && (row.length <= 1 || h > ROWH * 1.5)) h = ROWH;   // don't let a short last row blow up
    h = Math.max(ROWH * 0.7, Math.min(h, ROWH * 1.6));
    let x = G;
    row.forEach((c) => {
      const w = h * c.a;
      const cell = { x, y: wy, w, h, it: c.it };
      if (scatter) cell.tilt = +rnd(-8, 8).toFixed(1);
      cellOfPhoto[c.pIdx] = cells.length; cells.push(cell);
      x += w + G;
    });
...
function epicVintageSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, assetBase, perPhoto = null }) {
  ...
  // Length mode paces exactly to the per-photo budget; default keeps the longer
  // hold that keeps the light-leak blowouts from washing out the runtime.
  const HOLD = perPhoto != null ? Math.max(0.6, perPhoto) : S.photoS + 1.6;
  const cardS = includeCards ? CARD_S : 0;
  ...
  photos.forEach((it, i) => {
    ...
    const dur = HOLD;
    // NATIVE SHAPE: the hero print takes the photo's real aspect (a portrait photo
    // becomes a tall print), same base height, width derived from the aspect. The
    // 0.5625 factor converts the pixel aspect into %-of-frame (16:9 frame is wider
    // than tall). Clamp so a panorama/very-tall shot still fits the frame.
    const A = (it.w > 0 && it.h > 0) ? it.w / it.h : 16 / 9; // default landscape when unknown
    let printW = 62 * A * 0.5625, printH = 62;
    if (printW > 90) { printW = 90; printH = printW / (A * 0.5625); }
    if (printH > 74) { printH = 74; printW = printH * A * 0.5625; }
    ...
    elements.push({
      ...
      // The WHOLE pile (bg + hero) moves as ONE: a slow drift, then in the last
      // beat it RECEDES + spins + drifts off and fades — so the entire background
      // leaves WITH the print (Josh: nothing peels off on its own; the tan base
      // must not be left behind). The leak flash covers the hand-off to the next
      // pile.
      x: [{ time: 0, value: '52%', easing: 'linear' }, { time: dur - eOut, value: '48%', easing: 'quadratic-in' }, { time: dur, value: `${(48 + spinDir * 12).toFixed(1)}%` }],
      y: [{ time: 0, value: '48%', easing: 'linear' }, { time: dur - eOut, value: '52%', easing: 'quadratic-in' }, { time: dur, value: '40%' }],
      z_rotation: [{ time: 0, value: `${camR0}°`, easing: 'linear' }, { time: dur - eOut, value: `${camR1}°`, easing: 'quadratic-in' }, { time: dur, value: `${(Number(camR1) + spinDir * 16).toFixed(1)}°` }],
      x_scale: [{ time: 0, value: '106%', easing: 'linear' }, { time: dur - eOut, value: '100%', easing: 'quadratic-in' }, { time: dur, value: '56%' }],
      y_scale: [{ time: 0, value: '106%', easing: 'linear' }, { time: dur - eOut, value: '100%', easing: 'quadratic-in' }, { time: dur, value: '56%' }],
      opacity: [{ time: 0, value: '100%' }, { time: dur - eOut * 0.6, value: '100%' }, { time: dur, value: '0%' }],
      elements: [
        ...
        {
          type: 'composition', width: `${printW.toFixed(1)}%`, height: `${printH.toFixed(1)}%`, x_anchor: '50%', y_anchor: '50%',
          shadow_color: '#000000', shadow_blur: '5vmin', shadow_x: '0vmin', shadow_y: '1.5vmin',
          // FALLS ON (drops + settles), then a gentle dwell drift — and HOLDS. No
          // fly-off: the pile's recede/fade (above) carries the print away with the
          // whole scene, so nothing peels off the background.
          x: '50%',
          y: [{ time: 0, value: '38%', easing: 'quadratic-out' }, { time: eIn, value: '50%' }],
          z_rotation: [{ time: 0, value: `${rIn}°`, easing: 'quadratic-out' }, { time: eIn, value: `${rot0}°`, easing: 'linear' }, { time: dur, value: `${rot1}°` }],
          x_scale: [{ time: 0, value: '132%', easing: 'quadratic-out' }, { time: eIn, value: '112%' }],
          y_scale: [{ time: 0, value: '132%', easing: 'quadratic-out' }, { time: eIn, value: '112%' }],
          opacity: [{ time: 0, value: '0%' }, { time: eIn * 0.55, value: '100%' }],
          elements: [
            { type: 'shape', x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', path: printPath, fill_color: '#F6EFE0' },
            // contain (not cover): the print already matches the photo's aspect, so
            // the whole photo shows with a thin even cream border — nothing cropped.
            applyPhotoColor({ type: 'image', source: it.url, x: '50%', y: '50%', width: '96.5%', height: '95%', x_anchor: '50%', y_anchor: '50%', fit: 'contain' }, it),
          ],
        },
      ],
    });
