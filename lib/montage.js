...
function polaroidStackSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background, perPhoto = null }) {
  ...
  photosOnly.forEach((it, k) => {
    ...
    const print = {
      ...
      duration: life,
      // Drop: falls STRAIGHT DOWN (no sideways drift/bounce \u2014 Josh), decelerating
      // the whole way on a single long ease-out so it comes to rest SOFTLY (not
      // abrupt). Only a whisper of rotation settle (~3\u00B0) and a small scale-down as
      // it lands. Each print keeps its scattered resting spot (settleX/settleY).
      x: `${settleX.toFixed(1)}%`,
      ...
      z_rotation: [{ time: 0, value: `${(rTilt - 3).toFixed(1)}\u00B0`, easing: 'ease-out' }, { time: MOVE, value: `${rTilt.toFixed(1)}\u00B0` }],
