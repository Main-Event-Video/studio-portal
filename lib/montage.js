...
function collageWallSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  if (featured) {
    ...
    // Enough rows for EVERY photo to get its own cell (so every one can be
    // featured); no upper clamp — the camera flies over the taller wall.
    ...
    // Enough rows for EVERY photo to get its own cell (see scatter note).
    const ROWS = Math.max(4, Math.ceil(n / COLS));
  ...
  // UNIVERSAL RULE (Josh): the camera must REST on EVERY included image, one at a
  // time, in CHRONOLOGICAL order — even an image that also sits in the wall as an
  // accent. Keep the haphazard whip + transitions; only choose WHICH cell is
  // featured next. So: place every photo in the wall once as a SHUFFLED scatter,
  // then rest on them in numbered order. Because consecutive numbers live in
  // scattered cells, the camera still darts back and forth exactly like before —
  // it just now lands on 1, 2, 3 … N instead of a sampled handful.
  const nCells = cells.length;
  const perm = [...Array(nCells).keys()].map((i) => i % n);   // every photo covered ≥ once
  for (let i = perm.length - 1; i > 0; i--) { const j = Math.floor(rnd(0, i + 1)); [perm[i], perm[j]] = [perm[j], perm[i]]; }
  cells.forEach((c, i) => { c.it = pk(perm[i]); });            // scatter all photos across the wall
  const cellOfPhoto = new Array(n).fill(-1);                   // first cell holding each photo → its rest
  cells.forEach((c, i) => { if (cellOfPhoto[perm[i]] < 0) cellOfPhoto[perm[i]] = i; });
  const visitCells = [];
  for (let p = 0; p < n; p++) { const ci = cellOfPhoto[p]; if (ci >= 0) visitCells.push(ci); }
