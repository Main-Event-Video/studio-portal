...
function framedBoxSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  // MEASURED on the reference: the stroke is 7px on a 1006-wide frame at the
  // hold = 0.696% of frame width. Creatomate centres a stroke on its path and
  // this one is drawn inside the clipping box, so the outer half is trimmed and
  // the VISIBLE line is half the number below -- hence 2 x 0.696% x (W/1080)
  // expressed against the short side. Josh: "double the size of the boarder to
  // match closer to ref"; the trace says 2.2x, which is what this is.
  //
  // KNOWN DIFFERENCE, not worth a failed render to chase: the reference's
  // border GROWS with the box (7px at the hold, 10px at full frame), because
  // theirs is a proportional stroke. Ours is a pixel length inside an animating
  // composition, and pixel children do not scale with their parent. Matching
  // that needs stroke_width in '%', which I have never rendered and which
  // errored the whole job last time I guessed at a unit ('6deg'). Sized to the
  // hold, which is the state on screen longest.
  const STROKE = Math.max(6, Math.round(Math.min(W, H) * 0.0247));   // ~27px at 1080, ~13px visible
  ...
  const FB_END = 0.978;      // hero's final size, fraction of frame
  ...
  items.forEach((it, i) => {
    ...
    const FB_MOTION = 0.674;   // motion occupies this much of the shot, then rest
    const FB_WIPE = 0.34;      // card has covered the frame by here
    const FB_GROW = 0.34;      // and the hero starts growing at the same instant
    const FB_TRAVEL = 38;      // hero's on-screen travel, % of frame
    ...
    // Reference growth measured slow-fast-slow (height 414 -> 570 px, per-frame
    // deltas 1.5 / 10 / 20 / 8), so sinusoid-in-out is the right shape -- and it
    // begins from rest at the hold and settles at rest on arrival, so neither
    // end of the growth steps the hero's apparent speed.
    //
    // A LANDSCAPE BOX IS NOT FRAME-SHAPED. Measured, the reference holds at
    // 801x414 = 1.95:1 inside a 1.77:1 frame, then grows to fill it -- so the
    // box does change shape, by about 9%, and the two axes need their own
    // ratios. That is safe here because a landscape photo COVERS its box and
    // fills whatever shape it is. A TALL photo is contained, so its box must
    // stay exactly the photograph's shape or the picture letterboxes and the
    // white rule stands off the edge ("The frame is not married to the image
    // edge") -- tall therefore scales UNIFORMLY, on the smaller ratio.
    const sizeSeq = (full, hold) => [
      { time: 0, value: `${(hold * full * 100).toFixed(3)}%`, easing: 'linear' },
      { time: +(dur * FB_GROW).toFixed(3), value: `${(hold * full * 100).toFixed(3)}%`, easing: 'sinusoid-in-out' },
      { time: +(dur * FB_MOTION).toFixed(3), value: `${(full * 100).toFixed(3)}%`, easing: 'linear' },
      { time: dur, value: `${(full * 100).toFixed(3)}%` },
    ];
    const holdW = (tall ? FB_HOLD_H : FB_HOLD_W) / FB_END;
    const holdH = FB_HOLD_H / FB_END;
