...
const POL = { HOLD: 6.0, MOVE: 1.6, KEEP: 4, FADE: 0.65 }; // HOLD = slow deliberate cadence; MOVE = slow floaty toss (eases to rest)
...
function polaroidStackSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background, perPhoto = null }) {
  ...
  photosOnly.forEach((it, k) => {
    ...
    if (S.wholePhoto) {
      ...
      cardW = 30; cardH = cardW * (width / height) * 1.068;
      cardEls = [
        { type: 'shape', x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', path: roundedRect(2.3, 2.1), fill_color: '#FCFBF7' },
        applyPhotoColor({ type: 'image', source: it.url, x: '50%', y: '1.3%', width: '97.2%', height: '91.0%', x_anchor: '50%', y_anchor: '0%', fit: 'cover' }, it),
      ];
    }
    const print = {
      name: `Print-${k + 1}`, type: 'composition', track: 4 + k, time: t,
      duration: life + (willFade ? POL.FADE : 0),
      // APPROVED toss (polaroid-stack-preview): a SLOW, FLOATY toss that eases
      // smoothly to rest \u2014 decelerates the whole way (quadratic-out), monotonic (no
      // gravity-accelerate, no dip). Enters from well above at 112% scale rotated
      // the OTHER way, settling to its resting size + tilt.
      x: [{ time: 0, value: `${startX.toFixed(1)}%`, easing: 'quadratic-out' }, { time: MOVE, value: `${settleX.toFixed(1)}%` }],
      y: [{ time: 0, value: '-120%', easing: 'quadratic-out' }, { time: MOVE, value: `${settleY.toFixed(1)}%` }],
      ...
      z_rotation: [{ time: 0, value: `${(-rTilt).toFixed(1)}\u00B0`, easing: 'quadratic-out' }, { time: MOVE, value: `${rTilt.toFixed(1)}\u00B0` }],
...
function epicVintageSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, assetBase, perPhoto = null }) {
  ...
  // LARGER entry (300%, fills well past the frame on the hard cut) with a LONG,
  // gentle deceleration (0→0.30 of D) so the slow-down isn't abrupt, a brief glide
  // (the view), then accelerate off. Rotation/drift stay continuous.
  const OFF   = [0, 0.30, 0.46, 0.72, 0.88, 1];
  const SC     = [300, 108, 100, 60, 33, 15];
  const ROT    = [-7, -3, 3, 22, 44, 64];                // ×dir — smooth, always turning
  const TX     = [0, 4, 9, 28, 54, 78];                   // ×dir — always drifting
  const TY     = [0, -3, -6, -14, -24, -34];             // fixed (up)
  ...
  photos.forEach((it, i) => {
    ...
    // SOFT blurred background field (Josh: "not blurred enough, strange shadow
    // squares"). Only the 4 BIG oversized patches (edges off-frame), heavily
    // blurred, NO white frame + NO drop shadow → a dreamy soft bed, not hard
    // framed squares. The 2 small in-frame framed prints are gone (they were the
    // "shadow squares").
    const bgPrints = SPOTS.filter((s) => s.w > 100).map((s) => {
      ...
      return {
        ...
        z_rotation: `${s.r}°`,
        elements: [
          { type: 'image', source: it.url, x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', fit: 'cover', color_filter: 'sepia', color_filter_value: '64%', blur_radius: 34, blur_mode: 'stack' },
          { type: 'shape', x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', path: rect, fill_color: '#2A1B0C', blend_mode: 'multiply', opacity: '10%' },
        ],
      };
    });
    ...
    const heroWrap = {
      ...
      // Minimal own-motion so the hero RIDES the scene's smooth spin instead of
      // fighting it (the "shift in the spin" was the hero's own rotation/scale
      // compounding with the scene's).
      x_scale: [{ time: 0, value: '100%', easing: 'linear' }, { time: D, value: '105%' }],
      y_scale: [{ time: 0, value: '100%', easing: 'linear' }, { time: D, value: '105%' }],
      z_rotation: `${(dir * 2).toFixed(1)}°`,
    ...
    elements.push({
      ...
      elements: [
        ...
        { type: 'image', source: it.url, x: '50%', y: '50%', width: '128%', height: '128%', x_anchor: '50%', y_anchor: '50%', fit: 'cover', color_filter: 'sepia', color_filter_value: '70%', blur_radius: 32, blur_mode: 'stack', opacity: '100%' },
  ...
  // --- LIGHT LEAK on every cut: a warm DIRECTIONAL leak (real edge-bleed texture,
  // NOT a centered orb) sweeps in from a frame edge, ramping up over the outgoing
  // image and peaking on the hard cut, with a gentle warm flash. Cycles the three
  // edge directions. No circle "orbs".
  for (let k = 1; k < n; k++) {
    ...
    // gentle warm flash so the cut blooms softly (not a full-frame white-out)
    elements.push({ name: `Leak-flash-${k}`, type: 'shape', track: 19, time: t0, duration: dur, path: rect, width: '100%', height: '100%', x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%', fill_color: '#FFEAC0', blend_mode: 'screen', blur_radius: 10, blur_mode: 'stack', opacity: ramp(16) });
    // directional warm leak bleeding in from an edge, drifting slightly
    if (assetBase) {
      const EP = ['leak_epic1.png', 'leak_epic2.png', 'leak_epic3.png'];
      const d2 = k % 2 ? 1 : -1;
      elements.push({
        name: `Leak-${k}`, type: 'image', track: 20, source: `${assetBase}/overlays/${EP[(k - 1) % EP.length]}`,
...
function storyBuilderSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null }) {
  ...
  cards.forEach((c, i) => {
    ...
    elements.push({
      ...
      opacity: op,
      shadow_color: '#00000099', shadow_blur: '3vmin', shadow_x: '0vmin', shadow_y: '1.4vmin',
      elements: [
        { type: 'shape', x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', path: cardPath, fill_color: '#FFFFFF' },
        applyPhotoColor({ type: 'image', source: it.url, x: '50%', y: '50%', width: '98%', height: '98%', x_anchor: '50%', y_anchor: '50%', fit: 'contain' }, it),
      ],
    });
  });
