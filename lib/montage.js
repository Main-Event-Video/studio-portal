...
export function applyOverlays(source, {
  ...
  if (!source || !Array.isArray(source.elements) || !assetBase || (!atmoOn && !neonOn)) return source;
  ...
  gaps.forEach(([t0, t1], gi) => {
    ...
    if (neonOn && neonPct > 0) {
      ...
      for (let k = 0; k < N; k++) {
        ...
        const scale = 128 + Math.round(pol_rand(seed) * 46);
        const start = +(pol_rand(seed + 1)).toFixed(3);
        const len = 0.05 + pol_rand(seed + 2) * 0.05;
        ...
        add.push(arc('2.4vmin', Math.round(neonPct * 0.22)));
        add.push(arc('1.1vmin', Math.round(neonPct * 0.45)));
        add.push(arc('0.4vmin', neonPct));
      }
    }
  });
