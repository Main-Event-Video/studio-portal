...
export function buildMontageSource({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, totalSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true, assetBase = null }) {
  ...
  gseq.forEach((it, i) => {
    ...
    // The photo itself (contain keeps the whole image; cover crops to fill).
    const photoEl = {
      type: 'image',
      source: it.url,
    ...
    if (isFit) {
      // A Fit photo keeps its whole frame (no cropped heads) but would otherwise
      // show dead style-colour bars around it — especially PORTRAIT photos, which
      // pillar-box to a thin strip. Fill that dead space with a soft, blurred copy
      // of the same photo behind it, so the frame is always full and nothing is
      // lost. Wrapped in a composition so the transition applies to the pair.
      const blurBg = applyPhotoColor({
        type: 'image', source: it.url, fit: 'cover',
        x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%', width: '110%', height: '110%',
        blur_radius: 42, blur_mode: 'stack',
      }, it);
      elements.push({
        name: `Photo-${photoCount}`, type: 'composition', track: 2, duration: S.photoS,
        ...(i > 0 ? { animations: transitionIn(S, i) } : {}),
        elements: [blurBg, photoEl],
      });
    } else {
      // Fill photos already cover the frame — emit the single image (as before).
