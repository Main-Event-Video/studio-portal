...
function glassSolvePos(p, it, f) {
  const pAR = (p.w * GLASS_FW) / (p.h * GLASS_FH);
  const iAR = (it.w > 0 && it.h > 0) ? it.w / it.h : 1.5;
  const visW = pAR > iAR ? 1 : pAR / iAR;      // fraction of the image's width shown
  const visH = pAR > iAR ? iAR / pAR : 1;      // ...and of its height
...
function glassSafeCrop(p, it) {
  const faces = Array.isArray(it.faces) ? it.faces : null;
  if (!faces || !faces.length) return null;                 // rule 2
  ...
  for (const i of order) {
    const pos = glassSolvePos(p, it, faces[i]);
    if (pos) return pos;
  }
  return null;
}
...
function glassPane(p, it, W, H, S) {
  ...
  if (wantsPhoto && !p.hero && seenFrac >= GLASS_MIN_VISIBLE) {
    crop = glassSafeCrop(p, it);
    if (crop) {
      const z = photoFramingBias(it).z * 100;
      shot = { ...it, framing: { x: crop.x, y: crop.y, z } };
    }
  }
...
function glassLayout(it, variant) {
  ...
  if (land && variant % 2 === 0) {
    ...
    L = { kind: 'land', hero: { x: hx, y: hy, w: hw, h: hh, ry: -3, photo: true, hero: true }, panes: [
      ...
      { x: inL(40, 200), y: 110, w: 200, h: 930, ry: 15, photo: true, op: 0.40, accent: true, face: 0 },
      { x: inL(75, 165), y: 240, w: 165, h: 700, ry: 10, photo: true, op: 0.55, accent: true, face: 1 },
      { x: inR(30), y: 150, w: 220, h: 820, ry: -12, frost: 0.18 },
      { x: inR(70), y: 60, w: 210, h: 1000, ry: -17, photo: true, op: 0.45, accent: true, face: 0 },
    ] };
  } else if (land) {
    ...
    L = { kind: 'land', hero: { x: hx, y: hy, w: hw, h: hh, ry: -3, photo: true, hero: true }, panes: [
      ...
      { x: 150, y: 120, w: 300, h: 840, ry: 30, photo: true, op: 0.34, accent: true, behind: true, face: 1 },
      { x: F - 450, y: 120, w: 300, h: 840, ry: -30, photo: true, op: 0.34, accent: true, behind: true, face: 0 },
    ...
    L = { kind: 'port', hero: { x: hx, y: hy, w: hw, h: hh, ry: -2, photo: true, hero: true }, panes: [
      ...
      { x: F - 590, y: 130, w: 470, h: 830, ry: -36, photo: true, op: 0.40, accent: true, behind: true, face: 0 },
      ...
      { x: 250, y: 300, w: 190, h: 560, ry: 22, photo: true, op: 0.30, accent: true, behind: true, face: 1 },
    ...
    L = { kind: 'port', hero: { x: hx, y: hy, w: hw, h: hh, ry: 3, photo: true, hero: true }, panes: [
      ...
      { x: 60, y: 400, w: 940, h: 330, ry: 8, photo: true, op: 0.38, accent: true, behind: true, face: 0 },
      { x: 40, y: 90, w: 340, h: 900, ry: 24, frost: 0.18, behind: true },
      { x: hx + hw + 10, y: 260, w: 200, h: 640, ry: -18, photo: true, op: 0.34, accent: true, behind: true, face: 1 },
    ...
    L = { kind: 'port', hero: { x: hx, y: hy, w: hw, h: hh, ry: -3, photo: true, hero: true }, panes: [
      ...
      { x: hx + hw - 40, y: 400, w: Math.min(940, F - (hx + hw - 40) - 20), h: 330, ry: -8, photo: true, op: 0.38, accent: true, behind: true, face: 0 },
      { x: F - 380, y: 90, w: 340, h: 900, ry: -24, frost: 0.18, behind: true },
      { x: hx - 210, y: 260, w: 200, h: 640, ry: 18, photo: true, op: 0.34, accent: true, behind: true, face: 1 },
