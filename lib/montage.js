...
function buildMontageSourceInner({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, totalSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true, assetBase = null, mpTransition = null, mpStagger = null, mpHold = null, mpSpeed = null, duoPalette = null, duoTreatment: duoTreat = null, glassLight = true, glassRefl = true, atmosphere = true, atmoOpts = null, frameW = null, frameColor = null, stillsOpts = null }) {
  ...
  gseq.forEach((it, i) => {
    ...
    if (S.pan && eff === 'fill') {
      ...
      // ALWAYS build the box when the photo's shape is known, border or not.
      // It used to be built only for a border, so a borderless Fit photo (every
      // 9:16 on Party 2 / Party 3, every Fit photo elsewhere) went out as a
      // bare <image> at the top level — and the universal neon can only trace
      // a COMPOSITION that holds a picture, so those photos never got a light.
      // Josh: "the neon selector isn't hitting on every image." The box is the
      // picture's exact rect with the photo filling it, so the look is the same.
      const rect = containRectPct(it, width, height);
      if (rect) {
        photoEl = {
          type: 'composition',
