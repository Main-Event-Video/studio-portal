...
function framedBoxSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  items.forEach((it, i) => {
    ...
    // The slide runs LONGER than the size-hold phase (0.45 of the shot against
    // 0.29) so that its deceleration overlaps the drift instead of switching
    // off into it. Ending them together left a step in the composed velocity —
    // 1.72 down to 0.34 per frame across the join — because the slide's tail and
    // the drift never coexisted. Overlapped, one hands over to the other.
    // It still completes well before the hero starts to blow up at 0.60, which
    // is the order Josh described: background fills the screen, then the hero
    // grows.
    const inS = +(dur * 0.45).toFixed(3);
    const from = (back ? 1 : -1) * 38;
    ...
    // difference.
    const slideIn = [
      { time: 0, value: `${50 + from}%`, easing: 'quadratic-out' },
      { time: inS, value: '50%' },
    ];
    ...
    elements.push({
      ...
      width: '100%', height: '100%',
      x: horiz ? slideIn : '50%', y: horiz ? '50%' : slideIn,
      x_anchor: '50%', y_anchor: '50%',
      elements: [
