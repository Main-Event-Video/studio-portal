  ...
  pagesArr.forEach((group, pi) => {
    ...
    keys.forEach((k, idx) => {
      ...
      if (per) {
        ...
        const inner = applyPhotoColor({ type: 'image', source: it.url, x: '50%', y: '0%', width: '108%', height: '132%', x_anchor: '50%', y_anchor: '0%', fit: 'cover' }, it); // overscan (taller than the cell) + top-anchor inside the clipping cell → cover crops the BOTTOM, keeps heads
        ...
        photoEls.push({
          ...
          elements: [applyPhotoColor({ type: 'image', source: it.url, x: '50%', y: '0%', width: '108%', height: '132%', x_anchor: '50%', y_anchor: '0%', fit: 'cover' }, it)],
        });
      }
    });
    const pageComp = {
