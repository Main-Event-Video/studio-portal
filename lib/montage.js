...
export const STYLES = {
  ...
  glass: {
    ...
    photoS: 4.6, fadeS: 1.15,   // the prototype's TRANS is 1.15s; 0.9 measured 0.40-0.57s on render 018, about half the reference's 0.83-1.00s
    transitions: ['fade'], zoom: ['100%', '100%'],
    glass: true,
  },
  sliding_images: {
...
//
// 0.30 means a pane must show at least a third of its photograph: a reframing,
// not a magnification, and enough panes keep pictures that the room still reads
// as full of photographs.
const GLASS_MIN_VISIBLE = 0.30;
...
function glassSweep(name, at, dur, W, H) {
  const peak = Math.max(1, Math.round(95 * 0.95));
...
function glassSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null, light = true }) {
  ...
  items.forEach((it, i) => {
    ...
    // copy of each blade.
    behindPanes.forEach((p, k) => inner.push(glassDrift(glassPane(p, it, W, H, S), k, step, p)));
