  ...
  pagesArr.forEach((group, pi) => {
    ...
    keys.forEach((k, idx) => {
      ...
      if (per) {
        ...
        const inner = applyPhotoColor({ type: 'image', source: it.url, x: '50%', y: '0%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '0%', fit: 'cover' }, it); // top-anchored cover — crops the bottom, keeps heads
        ...
        photoEls.push(applyPhotoColor({
          ...
          x: `${(c.x + c.w / 2).toFixed(2)}%`, y: `${c.y.toFixed(2)}%`, width: `${c.w.toFixed(2)}%`, height: `${c.h.toFixed(2)}%`,
          x_anchor: '50%', y_anchor: '0%', fit: 'cover', // top-anchored cover — crops the bottom, keeps heads
