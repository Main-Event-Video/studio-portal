    ...
    const bgPrints = SPOTS.map((s) => {
      ...
      return {
        ...
        elements: [
          ...
          { type: 'image', source: it.url, x: '50%', y: '50%', width: '97.4%', height: '97.4%', x_anchor: '50%', y_anchor: '50%', fit: 'cover', color_filter: 'grayscale', color_filter_value: '66%', blur_radius: 12, blur_mode: 'stack' },
...
export function buildMontageSource({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, totalSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true, assetBase = null }) {
  ...
  gseq.forEach((it, i) => {
    ...
    // FIT (default) vs FILL, per the photo's editor setting:
    ...
    //    palette"). Only a very gentle zoom so it stays whole.
    //  • FILL → cover-crops to fill the frame (110% box + framing / drag / pan).
    // Party 2 (S.pan) is a MOVEMENT style — it always Fills + zooms; every other
    // style Fills only when the photo is set to Fill, else Fits.
    const isFill = it.fit === 'fill' || S.pan;
    let photoEl;
    if (isFill && S.pan) {
      // PARTY 2: cover-fill with a TOP-ANCHORED push/pull (Josh: the zoom must
      // keep the faces up top, not crop them from the centre). The element is
      // pinned to the top of the frame and taller than it, so the frame always
      // shows the TOP of the photo; the zoom grows downward off-frame and a gentle
      // horizontal drift adds energy.
      ...
      photoEl = {
        ...
        width: '112%', height: '120%', y: '0%', x_anchor: '50%', y_anchor: '0%',
