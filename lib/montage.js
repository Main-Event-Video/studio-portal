...
function framedBoxSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  items.forEach((it, i) => {
    ...
    const travelled = (from0, to0) => FB_CURVE.map(([u, y], k) => ({
    ...
    // THE BED NO LONGER SLIDES, and this fixes two complaints at once.
    //
    ...
    //
    // So the bed CROSS-FADES instead of sliding — no edge to see — and the box
    // becomes the only thing that moves. It still slides in over the previous
    // picture, which is what Josh asked for ("Transitions are a slide over the
    // last image"): at 88% of the frame wide starting at centre 8%, its
    // trailing edge begins well off-screen and it travels in over whatever is
    // still on screen behind it. One motion, one curve, no seam.
      ...
      elements: [
        ...
        { type: 'composition', width: '100%', height: '100%', clip: true,
          ...
          elements: [applyPhotoColor({
            type: 'image', source: it.url, ...coverBox(bedShot(it, W, H), W, H),
