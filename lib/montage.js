...
// A pane may carry a picture only if a cover fit shows at least this much of it.
//
// THIS NUMBER HAS A HISTORY. It was 0.55 because an early version cropped accent
// panes to hardcoded percentages like '64% 50%' and on a tall photo of a
// standing child that isolated her torso. 0.55 rejected everything narrow, which
// is why renders 005-007 came back with most panes empty — the reference has
// pictures in five panes the port turned to glass.
//
// What makes 0.20 defensible where 0.55 was needed before is that the crop is no
// longer arbitrary. coverBox crops exactly ONE axis and anchors the vertical at
// the TOP, so a tall pane shows a full-height slice and a wide pane shows the
// top band — head and shoulders. Neither can isolate a region the way a centred
// percentage crop did.
//
// It is still not as safe as the prototype, which anchors every accent crop on
// YuNet-detected EYES and degrades to plain glass when no face can be reached.
// That needs face detection in the render path, which does not exist yet. Until
// it does this is a geometric guarantee, not a semantic one.
const GLASS_MIN_VISIBLE = 0.20;
  ...
  return [{
    ...
    elements: [
      ...
      { type: 'shape', path: rect, width: '100%', height: '100%',
        x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%', fill_color: '#CCD3DC' },
      ...
      { type: 'shape', path: 'M 50 0 C 77.6 0 100 22.4 100 50 C 100 77.6 77.6 100 50 100 C 22.4 100 0 77.6 0 50 C 0 22.4 22.4 0 50 0 Z',
        ...
        x: '50%', y: '34%', x_anchor: '50%', y_anchor: '50%',
        fill_color: '#FDFDFE', blend_mode: 'screen',
        blur_radius: Math.round(H * 0.13), blur_mode: 'stack', opacity: '86%' },
      ...
      { type: 'shape', path: rect, width: '100%', height: '14%',
        ...
        fill_color: [{ offset: '0%', color: '#C9D0D9' }, { offset: '35%', color: '#DDE2E8' }, { offset: '100%', color: '#E9EDF1' }] },
...
//
// STILL MISSING vs the prototype: .refl, the mirrored blurred reflection under
// each pane. It needs a gradient-masked mirror image and nothing in this engine
// has proven that construction in a render yet. Noted rather than guessed at.
function glassPane(p, it, W, H, S) {
  ...
  const gentle = wantsPhoto && seenFrac >= GLASS_MIN_VISIBLE;
  // A pane that WANTED the picture and could not have it legibly does not go
  // back to being empty glass — that is why renders 005-007 had panes with
  // nothing in them. It takes the picture SOFT: the same crop, blurred past the
  // point where any part of a body is identifiable, at low opacity. What lands
  // in the pane is the photograph's colour and light rather than a region of it.
  // The safety argument is that there is nothing left to recognise; the design
  // argument is that a pane full of a picture's colour belongs in the room and
  // an empty one does not.
  const soft = wantsPhoto && !gentle && seenFrac > 0;
  ...
  if (gentle) {
    ...
    if (p.accent) {
      body.push({
        ...
        fill_color: [{ offset: '0%', color: 'rgba(196,214,232,0.55)' }, { offset: '100%', color: 'rgba(228,236,244,0.30)' }],
        blend_mode: 'multiply',
      });
    }
  } else if (soft) {
    body.push(applyPhotoColor({
      type: 'image', source: it.url, ...coverBox(it, boxW, boxH),
      // Blurred by a sixth of the pane's short side. At that radius a face is a
      // smudge and so is everything else — there is no crop to get wrong.
      blur_radius: Math.max(18, Math.round(Math.min(boxW, boxH) * 0.16)), blur_mode: 'stack',
      opacity: `${Math.round((p.op == null ? 0.40 : p.op) * 78)}%`,
    }, it));
    body.push({
      type: 'shape', path: GLASS_RECT, width: '100%', height: '100%',
      x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
    ...
    // Plain glass. The prototype fills it with a 150-degree gradient, white to a
    // cool grey at just over half the opacity — not a flat wash.
  ...
  // .rim, inner half: a 2px white line and a 22px bloom inside it.
  const edgePx = Math.max(2, Math.round(H * 0.0022));
  body.push({
    type: 'shape', path: GLASS_RECT, width: '100%', height: '100%',
    x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
    fill_color: 'rgba(0,0,0,0)', stroke_color: '#FFFFFF', stroke_width: `${edgePx * 6} px`,
    blur_radius: Math.round(H * 0.02), blur_mode: 'stack', blend_mode: 'screen',
    opacity: `${Math.round(55 * dim)}%`,
  });
  body.push({
    type: 'shape', path: GLASS_RECT, width: '100%', height: '100%',
    x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
    fill_color: 'rgba(0,0,0,0)', stroke_color: '#FFFFFF', stroke_width: `${edgePx} px`,
    opacity: `${Math.round(98 * dim)}%`,
  });
  ...
  // .rim's THIRD shadow — the outer white glow, the layer whose absence made the
  // edges read as black. It lives outside the clip so it can reach the wall.
  const glow = {
    type: 'shape', path: GLASS_RECT, width: '100%', height: '100%',
    x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
    fill_color: 'rgba(0,0,0,0)', stroke_color: '#FFFFFF',
    stroke_width: `${Math.max(3, Math.round(H * 0.012))} px`,
    blur_radius: Math.round(H * 0.024), blur_mode: 'stack', blend_mode: 'screen',
    opacity: `${Math.round(85 * dim)}%`,
  };
  ...
  const wrap = {
    ...
    x_anchor: '50%', y_anchor: '50%',
    elements: [glow, bodyComp, lead('left', 96), ...(p.hero ? [lead('right', 67)] : [])],
  };
  if (p.ry) {
    wrap.y_rotation = `${p.ry}°`;
    // Camera distance. Larger = flatter. The prototype's stage uses
    // perspective:1600px on a 1920-wide frame, so it scales with width.
...
function glassLayout(it, variant) {
  ...
  if (land && variant % 2 === 0) {
    ...
    L = { kind: 'land', hero: { x: hx, y: hy, w: hw, h: hh, ry: -3, photo: true, hero: true }, panes: [
      { x: inL(60, 250), y: 0, w: 250, h: 1080, ry: 20, frost: 0.22 },
      { x: inL(40, 200), y: 110, w: 200, h: 930, ry: 15, photo: true, op: 0.40, accent: true },
      { x: inL(75, 165), y: 240, w: 165, h: 700, ry: 10, photo: true, op: 0.55, accent: true },
      { x: inR(30), y: 150, w: 220, h: 820, ry: -12, frost: 0.18 },
      { x: inR(70), y: 60, w: 210, h: 1000, ry: -17, photo: true, op: 0.45, accent: true },
    ] };
  } else if (land) {
    ...
    const hx = Math.round((F - hw) / 2), hy = Math.round((G - hh) / 2);
    L = { kind: 'land', hero: { x: hx, y: hy, w: hw, h: hh, ry: -3, photo: true, hero: true }, panes: [
      { x: -40, y: 20, w: 560, h: 1040, ry: 44, frost: 0.16, behind: true },
      { x: F - 520, y: 20, w: 560, h: 1040, ry: -44, frost: 0.16, behind: true },
      { x: 150, y: 120, w: 300, h: 840, ry: 30, photo: true, op: 0.34, accent: true, behind: true },
      { x: F - 450, y: 120, w: 300, h: 840, ry: -30, photo: true, op: 0.34, accent: true, behind: true },
      { x: hx - 150, y: 60, w: 200, h: 960, ry: 15, frost: 0.24 },
      { x: hx + hw - 50, y: 60, w: 200, h: 960, ry: -15, frost: 0.24 },
    ] };
  } else if (variant % 3 === 0) {
    ...
    L = { kind: 'port', hero: { x: hx, y: hy, w: hw, h: hh, ry: -2, photo: true, hero: true }, panes: [
      { x: 120, y: 130, w: 470, h: 830, ry: 36, frost: 0.16, behind: true },
      ...
      { x: F - 590, y: 130, w: 470, h: 830, ry: -36, photo: true, op: 0.40, accent: true, behind: true },
      { x: -10, y: 50, w: 330, h: 980, ry: 26, frost: 0.22, behind: true },
      // ...and the far-right pane sits ON TOP of it.
      { x: F - 320, y: 50, w: 330, h: 980, ry: -26, frost: 0.22, behind: true },
      { x: 250, y: 300, w: 190, h: 560, ry: 22, photo: true, op: 0.30, accent: true, behind: true },
      { x: hx - 130, y: 0, w: 240, h: 1080, ry: 14, frost: 0.24 },
      { x: hx + hw - 110, y: 60, w: 210, h: 960, ry: -14, frost: 0.20 },
    ] };
  } else if (variant % 3 === 2) {
    ...
    L = { kind: 'port', hero: { x: hx, y: hy, w: hw, h: hh, ry: 3, photo: true, hero: true }, panes: [
      { x: F - 220, y: 150, w: 300, h: 800, ry: -22, frost: 0.20, behind: true },
      ...
      { x: 60, y: 400, w: 940, h: 330, ry: 8, photo: true, op: 0.38, accent: true, behind: true },
      { x: 40, y: 90, w: 340, h: 900, ry: 24, frost: 0.18, behind: true },
      { x: hx + hw + 10, y: 260, w: 200, h: 640, ry: -18, photo: true, op: 0.34, accent: true, behind: true },
      { x: hx + hw - 100, y: 20, w: 220, h: 1040, ry: -13, frost: 0.24 },
      { x: hx - 100, y: 80, w: 200, h: 940, ry: 13, frost: 0.20 },
    ] };
  } else {
    ...
    L = { kind: 'port', hero: { x: hx, y: hy, w: hw, h: hh, ry: -3, photo: true, hero: true }, panes: [
      { x: -80, y: 150, w: 300, h: 800, ry: 22, frost: 0.20, behind: true },
      { x: hx + hw - 40, y: 400, w: Math.min(940, F - (hx + hw - 40) - 20), h: 330, ry: -8, photo: true, op: 0.38, accent: true, behind: true },
      { x: F - 380, y: 90, w: 340, h: 900, ry: -24, frost: 0.18, behind: true },
      { x: hx - 210, y: 260, w: 200, h: 640, ry: 18, photo: true, op: 0.34, accent: true, behind: true },
      { x: hx - 120, y: 20, w: 220, h: 1040, ry: 13, frost: 0.24 },
      { x: hx + hw - 100, y: 80, w: 200, h: 940, ry: -13, frost: 0.20 },
    ] };
  }
...
// A pane in FRONT of the hero. It keeps drifting exactly as it was, and simply
// DISSOLVES AWAY over the picture.
//
// The first attempt slid these panes sideways off the hero and faded them as
// they went. Josh, on render 007: "When I asked for full frame no panes, I
// wanted it to just fade through the panes on it. Not a sudden move of the panes
// to get out of the way." He is right — a pane travelling out of shot is an
// event, and it draws the eye at the exact moment the photograph is supposed to
// be taking it. A pane that simply stops being there is not an event.
//
// So: no positional change at all beyond the drift the pane was already doing,
// and the fade is spread across the whole parting window rather than rushed at
// the end of it.
function glassPart(el, k, dur, acts) {
  glassDrift(el, k, dur);
  el.opacity = [
    { time: 0, value: '100%', easing: GLASS_EASE },
...
function glassSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null, light = true }) {
  ...
  items.forEach((it, i) => {
    ...
    L.panes.filter((p) => !p.behind).forEach((p, k) => inner.push(
      glassPart(glassPane(p, it, W, H, S), k + 1, step, acts)));
