...
function framedBoxSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  const items = seq.filter((it) => it.type !== 'placeholder');
  const n = Math.max(1, items.length);
  const cardS = includeCards ? CARD_S : 0;
  const step = Math.max(1.2, perPhoto != null ? perPhoto : (Number(S.photoS) || 3.4));
  const fade = Math.min(Number(S.fadeS) || 0.5, step * 0.3);
  ...
  // The box, as fractions of the frame. Start and end come straight from the
  // measurements above.
  const BOX_W0 = 0.808, BOX_H0 = 0.747;
  const BOX_W1 = 0.892, BOX_H1 = 0.880;
  const STROKE = Math.max(2, Math.round(Math.min(W, H) * 0.0028));   // ~3px at 1080

  items.forEach((it, i) => {
    const t0 = +(cardS + i * step).toFixed(3);
    const dur = +step.toFixed(3);
    if (it.green) { elements.push(greenSlot(i === 0 ? 'GreenIn' : `Green-${i + 1}`, t0, dur, 4)); return; }

    const kf = (a, b) => ([{ time: 0, value: a, easing: 'linear' }, { time: dur, value: b }]);
    // THE MOVE, TRACED RATHER THAN APPROXIMATED. Josh: "take note of the speed of
    // the movement. it is not consistent. It is fast then slows down... elegantly,
    // continuous drift. Please try to match the move exactly."
    //
    // Tracking the reference's white edge across one shot (66.9% -> 97.0% of the
    // frame over 1.93s) and normalising both axes gives a curve that no standard
    // easing fits — mean absolute error against the measurement:
    //     linear 0.058   sinusoid-out 0.093   quad-out 0.116   cubic-out 0.196
    // It runs well AHEAD of linear through the first half (0.184 at u=0.10,
    // 0.429 at u=0.31) and then settles back onto a near-constant drift, ending
    // at roughly 0.8x the average speed. A quick opening, then elegance.
    //
    // So the measured samples ARE the keyframes. Piecewise-linear between points
    // this close is smooth at 30fps and is a trace of the reference rather than a
    // guess at which named curve it might have been.
    const FB_CURVE = [[0, 0], [0.10, 0.184], [0.21, 0.323], [0.31, 0.429],
      [0.41, 0.512], [0.52, 0.581], [0.62, 0.645], [0.72, 0.710],
      [0.83, 0.793], [0.93, 0.922], [1, 1]];
    const traced = (from, to, unit = '%') => FB_CURVE.map(([u, y], k) => ({
      time: +(u * dur).toFixed(3),
      value: `${(from + (to - from) * y).toFixed(3)}${unit}`,
      ...(k < FB_CURVE.length - 1 ? { easing: 'linear' } : {}),
    }));
    // EACH IMAGE COMES FROM A DIFFERENT DIRECTION. Josh: "each image comes from a
    // different direction - please look at the motion of multiple, not just one."
    // He is right, and only measuring several shots showed it. Tracking the white
    // stroke across the reference:
    //     4.6-6.8s   vertical edge   57% -> 19%   leftward
    //     9.1-11.0s  vertical edge   67% -> 96%   rightward
    //    13.6-15.8s  horizontal edge 65% -> 96%   downward
    //    18.7-20.6s  vertical edge   65% -> 96%   rightward
    //    23.0-25.3s  horizontal edge 36% ->  3%   upward
    //    27.3-29.1s  vertical edge   32% ->  3%   leftward
    // Four directions, and the edge travels for the WHOLE shot rather than
    // wiping on quickly — the box is anchored off one edge of the frame and
    // expands across it. i*3 % 4 rotates through all four without ever
    // repeating one back to back, and is deterministic, which finalize's
    // re-render requires (no Math.random anywhere in a builder).
    const DIRS = ['right', 'left', 'down', 'up'];
    const dir = DIRS[(i * 3) % 4];
    const horiz = dir === 'right' || dir === 'left';
    // The sweep axis opens from a third of the frame to almost all of it; the
    // other axis does the slow grow measured at 74.7% -> 88.0%.
    const SWEEP0 = 0.34, SWEEP1 = 0.95;
    // The bed's own travel, opposed to the sweep so the two separate.
    const bedAway = horiz ? (dir === 'right' ? -1.6 : 1.6) : 0;
    const bedFall = horiz ? 0 : (dir === 'down' ? -1.4 : 1.4);

    // 1. THE BED — the same photograph, blown up and blurred, filling the frame.
    //    It pushes in a little over the shot so the whole picture breathes.
    const bed = applyPhotoColor({
      type: 'image', source: it.url, ...coverBox(it, W, H),
      blur_radius: Math.round(Math.min(W, H) * 0.030), blur_mode: 'stack',
    }, it);
    // 2. THE BOX — clipped, so the sharp photo only shows inside it. The wipe is
    //    the composition's own width opening from the left edge; the grow is the
    //    box scaling about its centre afterwards.
    // A TALL PHOTOGRAPH GETS ITS OWN BOX. Josh, on the first render: "vertical
    // images need to be handled differently. Lets keep the full vertical image
    // but mimic the move with the realestate available in the vertical image -
    // ending up centerd on screen."
    //
    // A landscape-shaped box cover-crops a portrait photo, which is how the
    // first render cut the tops off heads. So for a tall picture the END STATE
    // is the whole photograph at its own aspect, fitted inside the frame and
    // centred, and the sweep happens ACROSS THAT BOX rather than across the
    // frame — same move, played out in the real estate the picture actually
    // has. Nothing is ever cropped: the box only ever reveals more of it.
    const tall = itIsPortrait(it);
    const endBox = tall ? nativeBox(it, W, H, 0.95, 0.88) : null;
    // Where the box finishes, as frame fractions. Landscape keeps the measured
    // frame-relative numbers; a tall photo finishes on its own native box,
    // centred.
    const endW = tall ? endBox.w : (horiz ? SWEEP1 : BOX_W1);
    const endH = tall ? endBox.h : (horiz ? BOX_H1 : SWEEP1);
    // The pin is the edge the box grows FROM. For a tall photo that edge is the
    // end box's own edge, so the box lands centred rather than against the
    // frame; for landscape it is the frame edge, as measured.
    const pin = tall
      ? {
        right: { x: `${((0.5 - endW / 2) * 100).toFixed(3)}%`, y: '50%', x_anchor: '0%', y_anchor: '50%' },
        left:  { x: `${((0.5 + endW / 2) * 100).toFixed(3)}%`, y: '50%', x_anchor: '100%', y_anchor: '50%' },
        down:  { x: '50%', y: `${((0.5 - endH / 2) * 100).toFixed(3)}%`, x_anchor: '50%', y_anchor: '0%' },
        up:    { x: '50%', y: `${((0.5 + endH / 2) * 100).toFixed(3)}%`, x_anchor: '50%', y_anchor: '100%' },
      }[dir]
      : {
        right: { x: '2.5%', y: '50%', x_anchor: '0%', y_anchor: '50%' },
        left:  { x: '97.5%', y: '50%', x_anchor: '100%', y_anchor: '50%' },
        down:  { x: '50%', y: '6%', x_anchor: '50%', y_anchor: '0%' },
        up:    { x: '50%', y: '94%', x_anchor: '50%', y_anchor: '100%' },
      }[dir];
    // The sweep axis follows the traced curve. The cross axis does the slow grow
    // (74.7% -> 88.0%, measured) on a plain linear ramp, so the two axes are not
    // locked together and the box does not read as one scaling rectangle.
    // The sweep axis follows the traced curve, from a sliver to the end box. The
    // cross axis does the slow grow. For a tall photo both are fractions of its
    // own end box, so the move is the same shape at a different size.
    // On a tall photo the CROSS axis does not animate at all. It stays at the end
    // size, because the picture inside is pinned at full size and anything
    // smaller than it on that axis would clip the photograph — which is the one
    // thing this change exists to prevent. The sweep alone carries the move.
    const startW = tall ? (horiz ? endW * SWEEP0 : endW) : (horiz ? SWEEP0 : BOX_W0);
    const startH = tall ? (horiz ? endH : endH * SWEEP0) : (horiz ? BOX_H0 : SWEEP0);
    const wPct = horiz ? traced(startW * 100, endW * 100) : kf(`${(startW * 100).toFixed(2)}%`, `${(endW * 100).toFixed(2)}%`);
    const hPct = horiz ? kf(`${(startH * 100).toFixed(2)}%`, `${(endH * 100).toFixed(2)}%`) : traced(startH * 100, endH * 100);
    const box = {
      type: 'composition', clip: true, width: wPct, height: hPct, ...pin,
      elements: [
        // The sharp picture fills the box, so it grows as the box opens — which
        // is what the reference does: the subject is visibly larger by the end.
        // The picture is sized to the END box IN PIXELS and pinned to the same
        // edge the box grows from. Pixels, not percentages: a % child is
        // relative to its parent, and the parent here is the animating box, so
        // a % image would squeeze along with it instead of being revealed by
        // it. Pinned to the growing edge, it holds still while the box opens
        // over it — and because the end box IS the whole photograph, the last
        // frame shows all of it, centred and uncropped.
        applyPhotoColor({
          type: 'image', source: it.url,
          ...coverBox(it, W * endW, H * endH),
          ...(tall ? {
            width: `${Math.round(endW * W)} px`, height: `${Math.round(endH * H)} px`,
            fit: 'contain',
            ...{
              right: { x: '0%', y: '50%', x_anchor: '0%', y_anchor: '50%' },
              left:  { x: '100%', y: '50%', x_anchor: '100%', y_anchor: '50%' },
              down:  { x: '50%', y: '0%', x_anchor: '50%', y_anchor: '0%' },
              up:    { x: '50%', y: '100%', x_anchor: '50%', y_anchor: '100%' },
            }[dir],
          } : {}),
        }, it),
        // The white outline. A STROKED shape must never carry blur_radius —
        // proved by probe, it renders as nothing. This one has none.
        { type: 'shape', path: rect, width: '100%', height: '100%',
          x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
          fill_color: 'rgba(0,0,0,0)', stroke_color: '#FFFFFF', stroke_width: `${STROKE} px` },
      ],
    };

    elements.push({
      name: `Framed-${i + 1}`, type: 'composition', track: 4 + (i % 2),
      time: t0, duration: +(i === items.length - 1 ? dur : dur + fade).toFixed(3),
      width: '100%', height: '100%', x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
      opacity: i === 0 ? undefined
        : [{ time: 0, value: '0%', easing: 'linear' }, { time: +fade.toFixed(3), value: '100%' }],
      elements: [
        // bed, pushing in slowly behind everything
        // THE BED MOVES INDEPENDENTLY. Josh: "The background and hero almost move
        // independently. elegantly, continuous drift." So it does NOT share the
        // box's traced curve: it holds a slow, perfectly even scale and slides
        // gently AGAINST the sweep. Constant speed against a decelerating one is
        // what reads as two things moving on their own, and the opposed
        // direction is what makes it parallax rather than drift-along.
        { type: 'composition', width: '100%', height: '100%', clip: true,
          x: kf(`${(50 + bedAway).toFixed(2)}%`, `${(50 - bedAway).toFixed(2)}%`),
          y: kf(`${(50 + bedFall).toFixed(2)}%`, `${(50 - bedFall).toFixed(2)}%`),
          x_anchor: '50%', y_anchor: '50%',
          x_scale: kf('114%', '126%'), y_scale: kf('114%', '126%'),
          elements: [bed] },
        // a whisper of darkening so the sharp box reads forward of the bed
        { type: 'shape', path: rect, width: '100%', height: '100%',
          x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
  ...
  // Dust and a light leak, over everything, on their own tracks ABOVE every
  // photo — the mistake that cost Vintage its pictures. 20 is clear of the
  // shot tracks (4/5) and of the cards (20 is the card track, so 30 up).
  const bodyS = cardS, bodyD = +(n * step).toFixed(3);
  if (base) {
    ['dust1.png', 'dust2.png'].forEach((d, k) => {
      elements.push({ name: `Dust-${k + 1}`, type: 'image', track: 30 + k, source: `${base}/overlays/${d}`,
        time: bodyS, duration: bodyD, width: '160%', height: '160%',
        x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
        blend_mode: 'screen', opacity: '26%',
        z_rotation: [{ time: 0, value: '0\u00B0', easing: 'linear' }, { time: bodyD, value: `${k ? -6 : 6}\u00B0` }] });
    });
    items.forEach((it, i) => {
      if (it.green) return;
      const at = +(cardS + i * step).toFixed(3);
      elements.push({ name: `Leak-${i + 1}`, type: 'image', track: 32,
        source: `${base}/overlays/leak_epic${(i % 3) + 1}.png`,
        time: at, duration: +Math.min(step, 1.9).toFixed(3),
        width: '150%', height: '150%', x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
        blend_mode: 'screen',
        opacity: [{ time: 0, value: '0%', easing: 'quadratic-out' },
          { time: +Math.min(step, 1.9).toFixed(3) * 0.45, value: '34%', easing: 'quadratic-in' },
          { time: +Math.min(step, 1.9).toFixed(3), value: '0%' }] });
    });
  }
...
export function buildMontageSource({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, totalSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true, assetBase = null, mpTransition = null, mpStagger = null, mpHold = null, mpSpeed = null, duoPalette = null, duoTreatment: duoTreat = null, glassLight = true, glassRefl = true }) {
  ...
  if (S.framedBox) return finish(framedBoxSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background, assetBase, perPhoto }));
