...
function multiPageSource({ S, seq, watermarkUrl, width, height, mp = {} }) {
  ...
  const headSafe = (it, c) => {
    ...
    const cellWpx = (c.w / 100) * width * (OVER_W / 100);
    const photoHpx = cellWpx / ar;                            // its height at that width
    const cellHpx = (c.h / 100) * height;
    let hPct = cellHpx > 0 ? (photoHpx / cellHpx) * 100 : 140;
    hPct = Math.max(OVER_W, Math.min(hPct, 600));             // always fill; cap extremes
    return { x: '50%', y: '0%', x_anchor: '50%', y_anchor: '0%', width: `${OVER_W}%`, height: `${hPct.toFixed(1)}%`, fit: 'cover' };
  };
