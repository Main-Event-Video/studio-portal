...
function polaroidStackSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height }) {
  ...
  // Blurred backdrop of the CURRENT photo (crossfades each beat) + a tint wash.
  ...
  photosOnly.forEach((it, k) => {
    ...
    const print = {
      ...
      elements: [
        ...
        // Polaroid frame: thin even top/sides (~4%), thick bottom caption strip
        // (~18%). Photo cover-cropped toward the top so heads stay in.
        { type: 'image', source: it.url, x: '50%', y: '43%', width: '92%', height: '78%', x_anchor: '50%', y_anchor: '50%', fit: 'cover' },
      ],
    };
