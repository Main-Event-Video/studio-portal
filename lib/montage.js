...
// Josh: EVERY montage starts and ends with a short chroma-green "fake photo"
// frame — a clean keyable handle to dissolve into/out of when the montage is
// dropped into Premiere. Overlaid on a very high track so it covers everything
// (photos, cards, watermark) for the first/last GREEN_BOOKEND_S seconds.
const GREEN_BOOKEND_S = 1.5;
function addGreenBookends(source) {
  ...
  const green = (name, time) => ({
    name, type: 'shape', track: 100, time, duration: GREEN_BOOKEND_S,
    path: rect, width: '100%', height: '100%', fill_color: CHROMA_GREEN,
  });
  source.elements.push(green('GreenStart', 0));
  source.elements.push(green('GreenEnd', Math.max(0, total - GREEN_BOOKEND_S)));
  return source;
}
...
function collageWallSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background }) {
  const rect = 'M 0 0 L 100 0 L 100 100 L 0 100 Z';
  ...
  let _s = 12345 + n * 97 + (featured ? 7 : 0);
  ...
  if (featured) {
    ...
    [[1, 2], [5, 2], [9, 2], [13, 2], [1, 6], [5, 6], [9, 6], [13, 6]].forEach(([c, r]) => {
      ...
      cells.push({ x, y, w: wpx, h: hpx, it: pk(pi) }); focal.push({ cx: x + wpx / 2, cy: y + hpx / 2, w: wpx, h: hpx }); mark(c, r, 4, 3); pi++;
    });
    ...
    const cw = 256, ch = 144, COLS = 7;            // uniform 16:9 cells
    const ROWS = Math.max(4, Math.min(9, Math.ceil(n / COLS)));
    const px = (c) => gap + (c - 1) * (cw + gap), py = (r) => gap + (r - 1) * (ch + gap);
    let pi = 0;
    for (let r = 1; r <= ROWS; r++) for (let c = 1; c <= COLS; c++) {
      const x = px(c), y = py(r); cells.push({ x, y, w: cw, h: ch, it: pk(pi) });
      if (r >= 2 && r <= ROWS - 1 && c >= 2 && c <= COLS - 1) focal.push({ cx: x + cw / 2, cy: y + ch / 2, w: cw, h: ch });
      pi++;
    }
  ...
  const visit = order.slice(0, Math.min(order.length, featured ? 8 : 10));

  const xK = [], yK = [], sxK = [], syK = [];
  let t = 0;
  visit.forEach((fi, i) => {
    const f = focal[fi];
    const s = rnd(0, 1) < 0.5 ? fillScale(f) : partialScale(f);
    const rest = tfPct(f.cx, f.cy, s), push = tfPct(f.cx, f.cy, s * 1.05);
    const whipS = featured ? rnd(0.52, 0.72) : rnd(0.95, 1.25);
    const dwellS = featured ? rnd(1.75, 2.15) : rnd(1.5, 1.9);
    if (i === 0) {
  ...
  const cellEls = cells.map((c) => applyPhotoColor({
    type: 'image', source: c.it.url,
    x: `${((c.x + c.w / 2) / wallWpx * 100).toFixed(3)}%`, y: `${((c.y + c.h / 2) / wallHpx * 100).toFixed(3)}%`,
    width: `${(c.w / wallWpx * 100).toFixed(3)}%`, height: `${(c.h / wallHpx * 100).toFixed(3)}%`,
    x_anchor: '50%', y_anchor: '50%', fit: 'cover',
  }, c.it));
  ...
  elements.push({
    ...
    elements: [{ type: 'shape', x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', path: rect, fill_color: '#F4F1EA' }, ...cellEls],
  });
...
export function buildMontageSource({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true }) {
  ...
  const wrap = (src) => (greenBookends ? addGreenBookends(src) : src);
  if (S.polaroid) return wrap(polaroidStackSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background }));
  if (S.collage) return wrap(collageWallSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background }));
