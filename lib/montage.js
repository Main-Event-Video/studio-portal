...
export const STYLES = {
  ...
  glass: {
    ...
    kicker: 'MAIN EVENT STUDIO',
    // 4.6s, not the 3.4 this shipped with. Robyn asked for a second and a half
    // of clean, unobstructed picture before each transition; with a 0.9s
    // cross-fade on top of that, a 3.4s shot leaves barely a second for the
    // panes to do anything at all. The three acts need the room. Length mode and
    // the per-photo control still override this.
    photoS: 4.6, fadeS: 0.9,
    transitions: ['fade'], zoom: ['100%', '100%'],
    glass: true,
  },
  sliding_images: {
...
// admin framer so the preview matches. 58% wide × 84% tall.
const DUO_HERO = { w: 0.58, h: 0.84 };
...
function photoSlideSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null }) {
  ...
  items.forEach((it, i) => {
    ...
    const ph = nativeBox(it, W, H, 0.78, 0.72);
    const cw = ph.w + 2 * BW;
    ...
    if (exits) {
      xk.push({ time: +step.toFixed(3), value: '50%', easing: 'quadratic-in' });
...
function photoRibbonSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null }) {
  ...
  const boxes = photos.map((it) => nativeBox(it, W, H, 0.60, 0.78));
  const cx = [];
  ...
  if (photos.length) {
    ...
    const cards = photos.map((it, i) => {
      ...
      if (i === 0) { sk.push({ time: 0, value: '100%', easing: 'linear' }); ok.push({ time: 0, value: '100%', easing: 'linear' }); }
      else {
        sk.push({ time: +Math.max(0, i * step - MOVE).toFixed(3), value: '72%', easing: 'quadratic-in-out' });
        sk.push({ time: +(i * step).toFixed(3), value: '100%', easing: 'linear' });
      ...
      if (i < photos.length - 1) {
        sk.push({ time: +((i + 1) * step - MOVE).toFixed(3), value: '100%', easing: 'quadratic-in-out' });
...
function comicBookSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  const n = items.length;
  const cardS = includeCards ? CARD_S : 0;
  const step = Math.max(1.2, perPhoto != null ? perPhoto : (Number(S.photoS) || 3.0));
  const MOVE = Math.min(0.55, step * 0.22);      // the comic-to-comic transition
  const INK = Math.min(0.42, step * 0.16);       // how long the ink takes to melt
  const HOLD_IN = Math.min(0.5, step * 0.18);    // comic is held before it melts
  const start0 = cardS;
  const total = start0 + n * step + cardS;
  ...
  items.forEach((it, i) => {
    ...
    const box = nativeBox(it, W, H, 0.86, 0.84);
    const last = i === items.length - 1;
    const dur = last ? step : step + MOVE;
    // The real photograph is revealed in the MIDDLE of the beat and re-inked
    // before the panel leaves, so every move happens in comic.
    const onA = HOLD_IN, onB = HOLD_IN + INK;
    const offA = Math.max(onB + 0.1, step - HOLD_IN - INK), offB = Math.max(offA + 0.05, step - HOLD_IN);
    const realOpacity = [
      { time: 0, value: '0%', easing: 'linear' },
      { time: +onA.toFixed(3), value: '0%', easing: 'quadratic-out' },
      { time: +onB.toFixed(3), value: '100%', easing: 'linear' },
      { time: +offA.toFixed(3), value: '100%', easing: 'quadratic-in' },
      { time: +offB.toFixed(3), value: '0%' },
    ];
    // Everything that says "comic" is on while the photograph is off.
    const comicOpacity = (peak) => [
      { time: 0, value: peak, easing: 'linear' },
      { time: +onA.toFixed(3), value: peak, easing: 'quadratic-out' },
      { time: +onB.toFixed(3), value: '0%', easing: 'linear' },
      { time: +offA.toFixed(3), value: '0%', easing: 'quadratic-in' },
      { time: +offB.toFixed(3), value: peak },
    ];

    const inner = [];
    // Base layer = the comic art if we have it, otherwise the photo pushed hard.
    inner.push(applyPhotoColor({
      type: 'image', source: it.altUrl || it.url, ...coverBox(it, box.w * W, box.h * H),
      ...(it.altUrl ? {} : { color_filter: 'contrast', color_filter_value: 55 }),
    }, it.altUrl ? it : null));
    // The real photograph on top, revealed mid-beat.
    inner.push({ ...applyPhotoColor({ type: 'image', source: it.url, ...coverBox(it, box.w * W, box.h * H) }, it), opacity: realOpacity });
    // The photo's own border, above both the comic art and the real photograph so
    // it frames the panel's picture whichever of the two is currently showing.
    const cbBorder = borderFrame(it, box.w * W, box.h * H, Math.min(W, H));
    if (cbBorder) inner.push(cbBorder);
    // Halftone dot screen — only while it is comic.
    if (dots) {
      inner.push({ type: 'image', source: dots, repeat: true, x: '50%', y: '50%', width: '100%', height: '100%',
        x_anchor: '50%', y_anchor: '50%', blend_mode: 'multiply', opacity: comicOpacity('26%') });
    }
    // Comic panel border, also only while it is comic.
    inner.push({ type: 'shape', path: 'M 0 0 L 100 0 L 100 100 L 0 100 Z',
      x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%',
      fill_color: 'rgba(0,0,0,0)', stroke_color: '#141018', stroke_width: '1.5vmin',
      stroke_join: 'miter', opacity: comicOpacity('100%') });

    const el = {
      name: `Comic-${i + 1}`, type: 'composition', track, time: +t0.toFixed(3), duration: +dur.toFixed(3),
      width: `${(box.w * 100).toFixed(2)}%`, height: `${(box.h * 100).toFixed(2)}%`,
      x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%', clip: true,
      // The panel arrives with a movement transition — in cover mode so the move
      // never uncovers the backdrop.
...
function neonFrameSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null }) {
  ...
  items.forEach((it, i) => {
    ...
    const box = nativeBox(it, W, H, 0.80, 0.80);
    const neon = NEON_COLOURS[i % NEON_COLOURS.length];
    const last = i === items.length - 1;
    const dur = last ? step : step + FADE;
    // The tube sits just inside the card edge so the whole stroke stays on the
    // photo rather than half-hanging in the background.
    const ring = (extra) => ({
      type: 'shape', path: roundedRect(2.4, 2.4 * (box.w * W) / (box.h * H)),
      x: '50%', y: '50%', width: '96%', height: '96%', x_anchor: '50%', y_anchor: '50%',
    ...
    elements.push({
      ...
      elements: [
        ...
        // 1. wide blurred colour halo
        ring({ stroke_color: neon, stroke_width: '2.6vmin', blur_radius: 22, blur_mode: 'stack', blend_mode: 'screen', opacity: '62%' }),
        // 2. the tube itself
        ring({ stroke_color: neon, stroke_width: '0.85vmin', blend_mode: 'screen', opacity: '92%' }),
        // 3. near-white core
        ring({ stroke_color: '#FFFFFF', stroke_width: '0.28vmin', opacity: '80%' }),
        // 4. the light running round the perimeter — halo, then core
        ring({ stroke_color: neon, stroke_width: '3.2vmin', blur_radius: 26, blur_mode: 'stack', blend_mode: 'screen',
...
export function styleNeedsDims(st) {
  ...
  return !!(s.collage || s.epic || s.trendy || s.wholePhoto || s.story || s.multipage || s.duotone || s.pan
    || s.slidePush || s.slideRail || s.ribbon || s.neon || s.comic
...
function twoPanelCard(items, variant, W, H) {
  ...
  if (items.length === 1) {
    const it = items[0], p = twoPanelIsP(it), w = p ? 30 : 60;
  ...
  if (pa && pb) { // two portraits — tall pair, offset high/low
    const left = twoPanelBox(26, AR_916, 'left', 23, flip ? 'bottom' : 'top', 5, W, H);
    const right = twoPanelBox(26, AR_916, 'right', 23, flip ? 'top' : 'bottom', 5, W, H);
    return [{ it: a, box: left }, { it: b, box: right }];
  }
  if (!pa && !pb) { // two landscapes — wide pair on a diagonal
    const left = twoPanelBox(53, AR_169, 'left', 5, flip ? 'bottom' : 'top', 7, W, H);
    const right = twoPanelBox(53, AR_169, 'right', 5, flip ? 'top' : 'bottom', 7, W, H);
    return [{ it: a, box: left }, { it: b, box: right }];
  }
  // mixed — landscape high, portrait low; alternate which side each sits on
  const land = pa ? b : a, landLeft = !flip;
  const lBox = twoPanelBox(53, AR_169, landLeft ? 'left' : 'right', 6, 'top', 8, W, H);
  const pBox = twoPanelBox(26, AR_916, landLeft ? 'right' : 'left', 15, 'bottom', 8, W, H);
...
// is ours. The ten rays take 2..11, which is why the shots start at 14.
const GLASS_BEAM_TRACK = 2;
const GLASS_SHOT_TRACK = 14;

// A pane may carry a picture only if it shows at least this much of it. Below
// the line it becomes glass. 0.55 keeps a near-full reframing (the big side
// wings) and rejects a narrow blade or a letterbox band, which are exactly the
// shapes that produced the torso crop.
const GLASS_MIN_VISIBLE = 0.55;
...
// One pane. `p` is in FRACTIONS of the frame: {x,y,w,h} 0..1, plus ry (degrees),
// photo (bool), op, and dim (edge brightness 0..1).
function glassPane(p, it, W, H, S) {
  const px = (v) => `${(v * 100).toFixed(3)}%`;
  const boxW = p.w * W, boxH = p.h * H;

  // Can this pane hold the picture? Gentle crops only.
  const wantsPhoto = !!p.photo && !it.green;
  const gentle = wantsPhoto && glassVisible(boxW, boxH, it.w || 0, it.h || 0) >= GLASS_MIN_VISIBLE;

  const inner = [];
  if (gentle) {
    // coverBox sizes the image to its TRUE aspect and slides it by the photo's
    // framing bias — which defaults to TOP. Same head-keeping behaviour as every
    // other style in this engine.
    // The photo border applies here like any other style: the pane's box IS the
    // visible picture (coverBox oversizes the image and this comp crops it), so
    // the frame drawn at the box edge lands on the picture's edge.
    withBorder([applyPhotoColor({
      type: 'image', source: it.url, ...coverBox(it, boxW, boxH),
      ...(p.op != null ? { opacity: `${Math.round(p.op * 100)}%` } : {}),
    }, it)], it, boxW, boxH, Math.min(W, H)).forEach((el) => inner.push(el));
  } else {
    // Plain glass: a faint pale fill. Also the fallback whenever a photo pane
    // would have had to crop too hard.
    const frost = (p.frost == null ? 0.20 : p.frost) * 1.45;
    inner.push({
      type: 'shape', path: GLASS_RECT, width: '100%', height: '100%',
      x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
      fill_color: `rgba(255,255,255,${frost.toFixed(3)})`,
    });
  }

  // A soft diagonal sheen — the thing that stops a pane reading as a flat card.
  inner.push({
    type: 'shape', path: GLASS_RECT, width: '100%', height: '100%',
    x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
    fill_color: 'rgba(255,255,255,0.16)', blend_mode: 'screen', opacity: '55%',
  });

  // THE EDGE. This is what says "glass" once the photo is opaque. It is dimmed
  // where the pane crosses the photograph: at full strength a white line drawn
  // across a face reads as a scratch on the picture rather than as an edge.
  const dim = p.dim == null ? 1 : p.dim;
  const edgePx = Math.max(1, Math.round(H * 0.0022));
  // A soft bloom OUTSIDE the crisp line. Without it a white stroke on a light
  // wall reads as a hairline and the pane looks like a flat card, which is
  // exactly how the first port came out.
  inner.push({
    type: 'shape', path: GLASS_RECT, width: '100%', height: '100%',
    x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
    fill_color: 'rgba(0,0,0,0)',
    stroke_color: '#FFFFFF', stroke_width: `${edgePx * 5} px`,
    blur_radius: 16, blur_mode: 'stack', blend_mode: 'screen',
    opacity: `${Math.round(62 * dim)}%`,
  });
  inner.push({
    type: 'shape', path: GLASS_RECT, width: '100%', height: '100%',
    x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
    fill_color: 'rgba(0,0,0,0)',
    stroke_color: '#FFFFFF', stroke_width: `${edgePx} px`,
    opacity: `${Math.round(96 * dim)}%`,
  });

  const comp = {
    type: 'composition', clip: true,
    x: px(p.x + p.w / 2), y: px(p.y + p.h / 2),
    width: px(p.w), height: px(p.h),
    x_anchor: '50%', y_anchor: '50%',
    // A pane standing in a room casts a shadow. This is most of what separates
    // "sheet of glass" from "rectangle drawn on the wall".
    shadow_color: 'rgba(45,60,80,0.22)',
    shadow_blur: '5vmin', shadow_x: '0vmin', shadow_y: '2vmin',
    elements: inner,
  };
  if (p.ry) {
    comp.y_rotation = `${p.ry}°`;
    // Camera distance. Larger = flatter. Tied to frame width so the amount of
    // perspective does not change with output size.
    comp.perspective = Math.round(W * 0.85);
  }
  return comp;
}

// ---- LAYOUTS ---------------------------------------------------------------
// All coordinates are FRACTIONS of the frame, so the same numbers work at any
// output size. `photo: true` asks for the picture; the pane only gets it if the
// crop would be gentle (glassPane decides).
function glassLayout(it, variant) {
  const ar = (it.w > 0 && it.h > 0) ? it.w / it.h : 1.6;
  const land = ar >= 1;

  if (land) {
    const hw = 0.80, hh = Math.min(0.86, hw * (16 / 9) / ar * (9 / 16));
    const hHfrac = (hw * 1920 / ar) / 1080;      // hero height as a frame fraction
    const hgt = Math.min(0.88, hHfrac);
    const hx = (1 - hw) / 2, hy = (1 - hgt) / 2;
    const hero = { x: hx, y: hy, w: hw, h: hgt, ry: -3, photo: true, hero: true };

    if (variant % 2 === 0) {
      // L1 — THE RACK. Blades stacked down both margins, positioned by their
      // INNER edge so each clips the hero by a little and everything else lives
      // out in the margin. Placing them by their left edge instead is what once
      // put a blade squarely over a child's face.
      const inL = (over, w) => hx + over - w;
      const inR = (over) => hx + hw - over;
      return { kind: 'land', hero, panes: [
        { x: inL(0.031, 0.130), y: 0.00, w: 0.130, h: 1.00, ry: 20, frost: .22, dim: .30 },
        { x: inL(0.021, 0.104), y: 0.10, w: 0.104, h: 0.86, ry: 15, frost: .16, dim: .30 },
        { x: inL(0.039, 0.086), y: 0.22, w: 0.086, h: 0.65, ry: 10, photo: true, op: .55, dim: .30 },
        { x: inR(0.016), y: 0.14, w: 0.115, h: 0.76, ry: -12, frost: .18, dim: .30 },
        { x: inR(0.036), y: 0.06, w: 0.109, h: 0.93, ry: -17, photo: true, op: .45, dim: .30 },
      ] };
    }
    // L2 — HINGED WINGS. Panes fan from a centre hinge and recede behind the
    // hero, showing mostly lit edge. Quieter, and it reads as architecture.
    return { kind: 'land', hero, panes: [
      { x: -0.02, y: 0.02, w: 0.29, h: 0.96, ry: 44, frost: .16, behind: true },
      { x: 0.73, y: 0.02, w: 0.29, h: 0.96, ry: -44, frost: .16, behind: true },
      { x: 0.08, y: 0.11, w: 0.16, h: 0.78, ry: 30, photo: true, op: .34, behind: true },
      { x: 0.76, y: 0.11, w: 0.16, h: 0.78, ry: -30, photo: true, op: .34, behind: true },
      { x: hx - 0.078, y: 0.06, w: 0.104, h: 0.89, ry: 15, frost: .24, dim: .30 },
      { x: hx + hw - 0.026, y: 0.06, w: 0.104, h: 0.89, ry: -15, frost: .24, dim: .30 },
    ] };
  }

  // ---- PORTRAIT: three rooms --------------------------------------------
  const hgt = 0.92;
  const hwf = (hgt * 1080 * ar) / 1920;
  const v = variant % 3;

  if (v === 0) {
    // P1 — WINGS AND BLADES. The big side wings are the one place a picture can
    // sit at near-full size, so the right one carries it: a second view of the
    // same moment rather than a smaller copy of it.
    const hx = (1 - hwf) / 2, hy = (1 - hgt) / 2;
    return { kind: 'port', hero: { x: hx, y: hy, w: hwf, h: hgt, ry: -2, photo: true, hero: true }, panes: [
      { x: 0.06, y: 0.12, w: 0.245, h: 0.77, ry: 36, frost: .16, behind: true },
      { x: 0.695, y: 0.12, w: 0.245, h: 0.77, ry: -36, photo: true, op: .40, behind: true },
      { x: -0.005, y: 0.05, w: 0.172, h: 0.91, ry: 26, frost: .22, behind: true },
      { x: 0.833, y: 0.05, w: 0.172, h: 0.91, ry: -26, frost: .22, behind: true },
      { x: hx - 0.068, y: 0.00, w: 0.125, h: 1.00, ry: 14, frost: .24, dim: .30 },
      { x: hx + hwf - 0.057, y: 0.06, w: 0.109, h: 0.89, ry: -14, frost: .20, dim: .30 },
    ] };
  }

  if (v === 1) {
    // P2 — OFF-CENTRE LEFT, with a wide pane crossing behind and running toward
    // the right edge. The composition has a direction instead of being a mirror.
    const hx = 0.34 - hwf / 2, hy = (1 - hgt) / 2;
    return { kind: 'port', hero: { x: hx, y: hy, w: hwf, h: hgt, ry: -3, photo: true, hero: true }, panes: [
      { x: -0.04, y: 0.14, w: 0.156, h: 0.74, ry: 22, frost: .20, behind: true },
      // Held INSIDE the frame. When this ran off the right edge, the part still
      // on screen showed a shoulder rather than the subject it was aimed at.
      { x: hx + hwf - 0.021, y: 0.37, w: Math.min(0.49, 0.98 - (hx + hwf - 0.021)), h: 0.31, ry: -8, frost: .14, behind: true },
      { x: 0.80, y: 0.08, w: 0.177, h: 0.83, ry: -24, frost: .18, behind: true },
      { x: hx - 0.063, y: 0.02, w: 0.115, h: 0.96, ry: 13, frost: .24, dim: .30 },
      { x: hx + hwf - 0.052, y: 0.07, w: 0.104, h: 0.87, ry: -13, frost: .20, dim: .30 },
    ] };
  }

  // P3 — P2 handed the other way: hero right, wide pane left. So a run of tall
  // photos does not always lean the same direction.
  const hx = 0.66 - hwf / 2, hy = (1 - hgt) / 2;
  return { kind: 'port', hero: { x: hx, y: hy, w: hwf, h: hgt, ry: 3, photo: true, hero: true }, panes: [
    { x: 0.885, y: 0.14, w: 0.156, h: 0.74, ry: -22, frost: .20, behind: true },
    { x: 0.031, y: 0.37, w: Math.max(0.30, hx - 0.05), h: 0.31, ry: 8, frost: .14, behind: true },
    { x: 0.021, y: 0.08, w: 0.177, h: 0.83, ry: 24, frost: .18, behind: true },
    { x: hx + hwf - 0.052, y: 0.02, w: 0.115, h: 0.96, ry: -13, frost: .24, dim: .30 },
    { x: hx - 0.052, y: 0.07, w: 0.104, h: 0.87, ry: 13, frost: .20, dim: .30 },
  ] };
}

// Slow, unsynchronised drift. Movement is a SCALE and a POSITION shift, never a
// change of box size: the photo is cover-cropped, so resizing its box re-crops
// the picture every frame and the result reads as camera shake.
// EASING, and why it is not linear. A linear keyframe pair moves at a constant
// speed, which means it starts and stops instantly — at this slow a drift that
// snap is visible at both ends of every shot and reads as a jerk. 'sinusoid-in-
// out' eases away from rest and back into it, so a pane is never seen starting
// or stopping. It is the difference between a pane that glides and one that is
// switched on.
...
// The budget. `blend` is the cross-fade into the NEXT shot, which eats the tail
// of this one — so the clean window has to finish before it, not before the raw
// end of the shot, or the "clean" second and a half would have the next
// photograph dissolving through most of it.
//
...
// window is taken FIRST because it is the point of the exercise — but never so
// greedily that the room has no time to be a room. On a shot too short for all
// three the window shrinks rather than swallowing the shot.
function glassActs(step, blend) {
  const usable = Math.max(0.5, step - Math.max(0, blend || 0));
  const part = Math.min(0.85, Math.max(0.30, usable * 0.22));
  const clean = Math.max(0.40, Math.min(GLASS_CLEAN_S, usable - part - 0.55));
  const hold = Math.max(0.12, usable - part - clean);
  return { hold: +hold.toFixed(3), part: +part.toFixed(3), open: +(hold + part).toFixed(3), clean: +clean.toFixed(3) };
}

// Slow, unsynchronised drift — for the panes BEHIND the hero, which never
// obstruct it and so run for the whole shot. Movement is a POSITION shift, never
// a change of box size: the photo is cover-cropped, so resizing its box re-crops
// the picture every frame and the result reads as camera shake.
function glassDrift(el, k, dur) {
  const dir = k % 2 ? -1 : 1;
  const from = parseFloat(el.x), to = from + dir * 1.35;
  el.x = [{ time: 0, value: `${from.toFixed(3)}%`, easing: GLASS_EASE }, { time: dur, value: `${to.toFixed(3)}%` }];
  return el;
}
...
function glassHero(el, dur, acts) {
  const kf = () => ([
...
// A pane in FRONT of the hero: drift, then slide clear of the picture and fade
// out. `shift` is a percentage of the frame width, signed away from the hero.
// The fade finishes slightly before the slide does, so nothing is still visibly
// travelling when the clean window opens.
function glassPart(el, k, dur, acts, shift) {
  const dir = k % 2 ? -1 : 1;
  const from = parseFloat(el.x);
  const drifted = from + dir * 1.35;
  el.x = [
    { time: 0, value: `${from.toFixed(3)}%`, easing: GLASS_EASE },
    { time: acts.hold, value: `${drifted.toFixed(3)}%`, easing: GLASS_EASE },
    { time: acts.open, value: `${(drifted + shift).toFixed(3)}%` },
  ];
  el.opacity = [
    { time: 0, value: '100%', easing: GLASS_EASE },
    { time: acts.hold, value: '100%', easing: GLASS_EASE },
    { time: +(acts.hold + acts.part * 0.82).toFixed(3), value: '0%' },
  ];
  return el;
}

// How far a front pane must travel to be off the photograph: its overlap with
// the hero box, plus a margin, in percent of the frame. The margin has two jobs
// — it absorbs the 1.35% drift the pane may have taken TOWARD the hero before
// it parts, and it covers the perspective skew, which pushes a rotated pane's
// near edge out past its box.
function glassClearShift(p, hero) {
  const side = (p.x + p.w / 2) < (hero.x + hero.w / 2) ? -1 : 1;
  const overlap = side < 0
    ? Math.max(0, (p.x + p.w) - hero.x)
    : Math.max(0, (hero.x + hero.w) - p.x);
  return side * (overlap + 0.055) * 100;
}
...
function glassBeams(S, total, W, H, track) {
  ...
  return defs.map(([a, h, op], i) => ({
    ...
    path: GLASS_RECT,
    x: '50%', y: '96%', x_anchor: '0%', y_anchor: '50%',
    width: `${(72 + (i % 3) * 6)}%`, height: `${(h * 0.09).toFixed(2)}%`,
    fill_color: '#FFFFFF', blend_mode: 'screen',
    blur_radius: 3 + (i % 3) * 2, blur_mode: 'stack',
    opacity: `${op}%`,
...
function glassSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null, light = true }) {
  ...
  const fade = Math.min(1.0, step * 0.30);
  ...
  const acts = glassActs(step, Math.min(Number(S.fadeS) || 0, step * 0.45));

  items.forEach((it, i) => {
    const t0 = cardS + i * step;
    if (it.green) {
      elements.push({
        name: `Green-${i + 1}`, type: 'composition', track: GLASS_SHOT_TRACK, time: +t0.toFixed(3), duration: +step.toFixed(3),
        ...(i > 0 ? { animations: transitionIn(S, i) } : {}),
        elements: [{ type: 'shape', track: 1, path: GLASS_RECT, width: '100%', height: '100%', fill_color: CHROMA_GREEN }],
      });
      return;
    }
    ...
    const inner = [];
    // Panes BEHIND the hero first, then the hero, then the blades in front.
    // Depth in this engine is order, not z, so this ordering IS the depth.
    L.panes.filter((p) => p.behind).forEach((p, k) => inner.push(glassDrift(glassPane(p, it, W, H, S), k, step)));
    inner.push(glassHero(glassPane(L.hero, it, W, H, S), step, acts));
    // The front blades are the only things that ever cross the photograph, so
    // they are the only things that have to leave before the clean window.
    L.panes.filter((p) => !p.behind).forEach((p, k) => inner.push(
      glassPart(glassPane(p, it, W, H, S), k + 1, step, acts, glassClearShift(p, L.hero))));

    elements.push({
      name: `Glass-${i + 1}`, type: 'composition', track: GLASS_SHOT_TRACK,
      time: +t0.toFixed(3), duration: +step.toFixed(3),
      ...(i > 0 ? { animations: transitionIn(S, i) } : {}),
      elements: inner,
    });
  });
