  ...
  if (b.videoUrl) {
    return wrap([
      fill('#000000'),
      {
        ...
        // EXPLICIT duration, always. A video element with no duration takes its
        // MEDIA length, so `loop: true` has nothing to loop into — the clip plays
        // once and stops. (This is exactly what broke when the background layers
        // moved inside a composition: the children stopped carrying their own
        // duration, and a 6-second backdrop under a 40-second montage simply
        // ended. Images do not care; video does.)
        duration,
