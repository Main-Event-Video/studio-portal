...
function storyBuilderSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, perPhoto = null }) {
  ...
  function relayoutSide(side, atT) {
    const dir = (side === up) ? -1 : 1, firstGap = 0.15 * sh;
    let off = 0;
    side.forEach((rc, i) => {
      const scale = 0.5 * Math.pow(SHR, i); off += firstGap * Math.pow(SHR, i);
      const ddy = dir * off, f = scale / 0.5;
      rc.forEach((c) => {
        const ndx = c.dx0 * f;
        push(c, atT, c.dx, c.dy, c.sc, c.tl);
        push(c, atT + RELAY, ndx, ddy, scale, c.tl);
        c.dx = ndx; c.dy = ddy; c.sc = scale;
      });
    });
  }

  // ---- run the choreography, recording every waypoint ----
  let t = stackStart, idx = 0;
  while (idx < n) {
    const c = cards[idx];
    c.landStart = t;
    ...
    if (frac >= TARGET || current.length >= MAXPR) {
      toggle = !toggle;
      const side = toggle ? up : down;
      current.forEach((cc) => { cc.dx0 = cc.dx; });
      side.unshift(current); current = [];
      relayoutSide(side, decideT);
      t = decideT + ROWGAP;
    } else {
      t = decideT;
    }
  ...
  cards.forEach((c, i) => {
    ...
    elements.push({
      ...
      opacity: [{ time: 0, value: '0%' }, { time: +(0.34 * LAND).toFixed(3), value: '100%' }],
