...
function framedBoxSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width: W, height: H, background, assetBase, perPhoto = null }) {
  ...
  items.forEach((it, i) => {
    ...
    // TUNED AGAINST THE PICTURE, NOT THE MATHS. At 60%/0.45 the numbers were
    // all correct — monotone, 3.1x, no reversals — and it still looked wrong on
    // a contact sheet: the card rushed past at 2.8x the hero's speed, so the
    // hero read as PINNED while a wipe swept over it, which is not "the
    // foreground Hero travels with the background". Widening the hero's travel
    // and lengthening the wipe brings them to 1.7:1, close enough that the two
    // read as one object arriving with a natural parallax between its layers.
    const FB_WIPE = 0.55;      // the card has covered the frame by here
    const FB_TRAVEL = 85;      // hero's total on-screen travel, % of frame
    const FB_HOLD = 0.88;      // hero size while the card is still arriving
    const FB_LIN = 0.55;       // share of the hero's travel at constant speed
    const FB_P = 2.6;          // deceleration on the rest of it
    const FB_STEPS = 40;
    // A pure ease-out was the trap. 1-(1-u)^p decelerates beautifully and then
    // stalls: its velocity goes to zero at the end, so at p=4 the hero covered
    // 87% of its travel in the first 40% of the shot and was parked for the
    // last fifth — a lurch and a stop, measured at 246,000:1 fastest to
    // slowest. Blending a linear term against it puts a FLOOR under the
    // velocity, so the hero never stops moving until it lands.
    // Measured on this curve: 3.1x fastest to slowest, monotone throughout.
    const heroF = (u) => (FB_LIN * u) + ((1 - FB_LIN) * (1 - ((1 - u) ** FB_P)));
    const wipeF = (u) => (u >= FB_WIPE ? 1 : 1 - ((1 - (u / FB_WIPE)) ** 2));
    ...
    // The card must cross a whole frame to wipe; the hero crosses 60% of one.
    // Since the hero is a child of the card, the only way its on-screen path
    // can be gentler than its parent's is for its LOCAL keyframes to run
    // against the card during the wipe. That is what a foreground moving
    // slower than a background is, and it is what the reference does — I
    // measured the hero's own travel at about 42% of frame while the plate
    // behind it swept the lot.
    //
    // My direction guard used to forbid exactly this, because it compared a
    // child's sign against its parent's. That rule was wrong: the thing Josh
    // actually asked for is "Everything moves in one consistent direction",
    // which is a statement about what ends up ON SCREEN. The guard now checks
    // the composite path instead, which still catches the reversal it was
    // built for and stops rejecting legitimate parallax.
    ...
    // Sampled, not eased. Two keyframes with easing:'quadratic-out' ought to
    // trace the same curve wipeF describes, but "ought to" is doing real work
    // there: the hero's keyframes are solved AGAINST the card, so if
    // Creatomate's quadratic-out differs from 1-(1-x)^2 by even a little, the
    // composite stops being the curve I designed. Sampling both from the same
    // function makes the composite exact by construction instead of by
    // assumption, and means the guard is checking what actually renders.
    ...
    const sizeSeq = (full) => [
      { time: 0, value: `${(FB_HOLD * full * 100).toFixed(3)}%`, easing: 'linear' },
      { time: +(dur * FB_WIPE).toFixed(3), value: `${(FB_HOLD * full * 100).toFixed(3)}%`, easing: 'sinusoid-in-out' },
      { time: dur, value: `${(full * 100).toFixed(3)}%` },
    ];

    const box = {
      type: 'composition', clip: true,
      width: sizeSeq(endW), height: sizeSeq(endH),
    ...
    elements.push({
      ...
      time: t0, duration: +(i === items.length - 1 ? dur : dur + (dur * FB_WIPE)).toFixed(3),
      ...
      elements: [
        ...
        { type: 'composition', width: '100%', height: '100%', clip: true,
          x: '50%', y: '50%', x_anchor: '50%', y_anchor: '50%',
          // THE BED DOES NOT MOVE INSIDE THE CARD. It does not need to: the
          // card is moving, and the bed is the card. Every previous attempt to
          // give the bed a motion of its own — a slide, then a pan — was
          // solving a problem created by not making the card the transition.
          //
          // Oversized so the blur's own soft edge falls outside the clip.
          // A stack blur has nothing to sample past the element's bounds, so a
          // bed sized exactly to the card feathers at the boundary and the wipe
          // line reads as a smeared band instead of an edge — which is what the
          // "weird stripe / filter shift" was.
          x_scale: '112%', y_scale: '112%',
          elements: [(() => {
