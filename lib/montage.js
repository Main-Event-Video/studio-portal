...
function polaroidStackSource({ S, seq, title, subtitle, watermarkUrl, includeCards, width, height, background, perPhoto = null }) {
  ...
  // The DROP takes a shorter slice of the beat so each print visibly falls, slows,
  // and gently STOPS with a rest before the next drops (matches the approved
  // preview — it was eating ~80% of the beat, so the settle never showed).
  const MOVE = Math.min(1.2, HOLD * 0.6);
  ...
  photosOnly.forEach((it, k) => {
    ...
    const print = {
      ...
      duration: life,
      // APPROVED drop (polaroid-stack-preview): FALLS fast, then SLOWS and gently
      // STOPS. The preview used a snappy front-loaded ease (cubic-bezier .19,.72,.28,1);
      // Creatomate can't take an arbitrary curve, so we shape it: an `ease-out` drop
      // that covers most of the fall by ~45% of MOVE (to just above rest), then a
      // long gentle settle over the rest. Enters from well above at 112% scale,
      // rotated the other way, settling to its resting size + tilt.
      x: [{ time: 0, value: `${startX.toFixed(1)}%`, easing: 'ease-out' }, { time: MOVE, value: `${settleX.toFixed(1)}%` }],
      y: [
        { time: 0, value: '-120%', easing: 'ease-out' },
        { time: MOVE * 0.45, value: `${(settleY - 7).toFixed(1)}%`, easing: 'ease-out' },
        { time: MOVE, value: `${settleY.toFixed(1)}%` },
      ],
      x_scale: [{ time: 0, value: '112%', easing: 'ease-out' }, { time: MOVE, value: '100%' }],
      y_scale: [{ time: 0, value: '112%', easing: 'ease-out' }, { time: MOVE, value: '100%' }],
      x_anchor: '50%', y_anchor: '50%', width: `${cardW.toFixed(1)}%`, height: `${cardH.toFixed(1)}%`,
      z_rotation: [{ time: 0, value: `${(-rTilt).toFixed(1)}\u00B0`, easing: 'ease-out' }, { time: MOVE, value: `${rTilt.toFixed(1)}\u00B0` }],
...
function mpPhotoEnter(motion, c, st, MOVE, FADE, inner) {
  ...
  const base = { type: 'composition', width: `${c.w.toFixed(2)}%`, height: `${c.h.toFixed(2)}%`, opacity: op, elements: [inner] };
  ...
  pagesArr.forEach((group, pi) => {
    ...
    keys.forEach((k, idx) => {
      ...
      if (per) {
        ...
        const inner = applyPhotoColor({ type: 'image', source: it.url, x: '50%', y: '50%', width: '100%', height: '100%', x_anchor: '50%', y_anchor: '50%', fit: 'cover' }, it);
