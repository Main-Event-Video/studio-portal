...
export function buildMontageSource({ photos, items, style = 'hollywood', title, subtitle, watermarkUrl, photoSeconds = null, totalSeconds = null, includeCards = true, width = 1920, height = 1080, background = null, greenBookends = true, assetBase = null, mpTransition = null, mpStagger = null, mpHold = null, mpSpeed = null }) {
  ...
  //
  // DEFAULT for EVERY one-at-a-time slideshow style (and any future one) — this is
  // the studio convention: a transition must never dissolve through the green
  // backdrop. A style can opt OUT with `holdFade: false` (e.g. if it ever wants a
  // literal fade-through-colour look). NOTE: this makes the transition a clean
  // cross-dissolve, so a style's `transitions` list (slide / wipe / scale) no
  // longer drives the move — those variants dissolved through green, which was the
  // bug. Multi-photo / wall / grid / green-screen styles use their own builders
  // above and are NOT affected by this block; fix green there per-builder.
  if (S.holdFade !== false) {
