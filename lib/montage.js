  ...
  gaps.forEach(([t0, t1], gi) => {
    ...
    if (neonOn && neonPct > 0) {
      ...
      for (let k = 0; k < N; k++) {
        ...
        // SIZED TO BE SEEN. The first pass drew a 128-174% outline and trimmed
        // 5-10% of its perimeter, so most arcs were a short piece of a shape
        // mostly OUTSIDE the frame — a good share of them landed entirely off
        // screen, which is the likeliest reason the first neon render showed
        // nothing at all. Now the outline is barely larger than the frame and
        // the arc is two to four times longer, so a piece always crosses it.
        const scale = 104 + Math.round(pol_rand(seed) * 18);
        const start = +(pol_rand(seed + 1)).toFixed(3);
        const len = 0.14 + pol_rand(seed + 2) * 0.10;
